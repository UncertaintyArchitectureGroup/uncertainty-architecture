...
This matrix is the canonical repository record for material source-to-framework decisions. It is a synthesis aid, not a mandatory ledger for every sentence. Material research items that are still under active investigation are tracked separately in the [`Active Research Register`](research-register.md); this file records their relationship to framework decisions when that relationship is material.
...
|---|---|---|---|---|
| Primary system category | Behavioral Software / Behavioral Applications | Thinking Systems | Active | Use Thinking Systems in current framework documents; preserve legacy wording in historical sources and provenance records. Formulation provenance is separately preserved in [`thinking-systems-formulation-provenance-arkadiy-dobkin.md`](notes/thinking-systems-formulation-provenance-arkadiy-dobkin.md). |
| Opposite-side category terminology | `Linear Software` is the current draft-normative glossary term, but external review identified repeated topology ambiguity in publication prose | **Explicitly Authored Software** is being tested only as a paper-level comparative label | Needs Resolution | Keep the canonical glossary unchanged in this PR. Track `TS-TERM-002` in the Active Research Register and require a separate deliberate terminology review before any repository-wide rename. |
...
| Technology scope of the Thinking-System definition | Current public framing could be read as if the category were restricted to LLM-based systems | The definition is technology-neutral: Thinking-System membership is not restricted to LLM-based systems | Active | Preserve this definition-level scope implication as resolved research item `TS-SCOPE-001`; no separate doctrine change is implied because the current definition already carries the technology-neutral boundary. |
| Concrete pre-LLM and low-consequence boundary cases | Technology-neutral applicability could be mistaken for proof that particular historical or low-consequence systems satisfy the category | Specific systems qualify only when causal analysis shows that a Consequential Runtime Responsibility materially depends on their Model Judgment | Needs Resolution | Test concrete cases separately under `TS-CASE-001`. Do not infer historical prevalence or classify examples solely from the presence of a probabilistic component. |
...
- opposite-side terminology: whether `Explicitly Authored Software` should replace, narrow, or remain only a paper-level alternative to `Linear Software`;
...
- systematic bidirectional mapping of the four-horizon model against STAMP/STPA, Simplex/runtime assurance, control-theoretic research, production ML/software engineering, management systems, and mature internal compositions;

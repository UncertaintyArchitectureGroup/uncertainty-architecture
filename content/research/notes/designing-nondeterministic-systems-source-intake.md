...
This note records the presentation deck used as a synthesis source for Uncertainty Architecture so that the Research Track distinguishes source evidence from explicit framework decisions.

## Source state

The maintainer-supplied original PPTX is the working source used for slide-level review and framework extraction.

The repository currently preserves a PDF export under [`content/raw/`](../../raw/Designing%20Non-Deterministic%20Systems%3A%20Maintaining%20Engineering%20Rigor%20in%20the%20AI%20Era.pdf). The PDF provides an archival repository snapshot; it is not a substitute for the original PPTX when slide order, speaker notes, or editable presentation content matter.

The deck has not yet been converted into a complete reviewable Markdown transcript or normalized repository edition. Slide content remains research evidence and does not become normative merely because it has been reviewed or preserved.
...
The maintainer-defined slides 1–6 transfer scope has now been reviewed against the original PPTX and translated into the following repository components:
...
1. Create a complete Markdown transcript or normalized repository edition with slide-level provenance when that work becomes useful.
2. Continue reviewing later deck sections against the broader publication corpus.
3. Record contradictions, superseded claims, and newly extracted entities in the existing research-to-framework traceability matrix.
4. Use worked applications to test whether the current doctrine, patterns, and practical artifact remain sufficiently clear and lightweight for SMB teams.

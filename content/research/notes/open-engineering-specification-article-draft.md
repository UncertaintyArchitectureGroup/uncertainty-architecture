...
This paper makes **Thinking Systems** explicit as an engineering category: software systems in which one or more **Consequential Runtime Responsibilities** depend partly on probabilistic Model Judgment rather than being fully specified through explicitly encoded logic in advance. The systems covered by this category predate LLMs; LLMs made model-mediated judgment far more general-purpose and widespread. The term names the changed engineering object, not a maturity level or architecture style. Fixed or dynamic orchestration, agent labels, autonomy, and delegated authority are separate dimensions; a simple predefined workflow can already be a Thinking System when at least one **Consequential Runtime Responsibility** depends partly on probabilistic Model Judgment.
...
flowchart LR
    ...
    T["Thinking-System engineering<br/> expanded concern: model-mediated runtime judgment inside the object<br/> response: bounded control of the changed object"]
...
Systems with this responsibility structure predate LLMs. Earlier probabilistic systems can qualify whenever at least one Consequential Runtime Responsibility depends partly on their Model Judgment—for example, a credit-scoring system used so that its score materially shapes a lending decision. LLMs did not create this responsibility structure; they made model-mediated judgment far more general-purpose, accessible, and widespread, which is why LLM-based systems are the paper's primary contemporary focus.

**Category membership, consequence severity, and required control depth are separate questions.** An internal summarizer whose output materially influences a reversible human review may fall within the category while requiring only a modest explicit control surface; a system authorized to alter financial or operational state may require much more. Classification identifies the engineering problem, while consequence, reachable authority, reversibility, evidence quality, and operating context determine implementation depth.
...
Thinking-System engineering still requires product discovery, deterministic software engineering, testing, security, deployment discipline, observability, and incident response. The category does not invalidate those practices. It changes the object they are controlling.
...
**Relationship to established systems-theoretic control.** This paper does not claim that hierarchical socio-technical control—or its extension into management and regulatory authority—is new; [STAMP/STPA](https://psas.scripts.mit.edu/home/get_file.php?name=STPA_handbook.pdf) already supports control structures that can span those levels. The narrower working hypothesis is that, for model-judgment-dependent software, the four-horizon model provides a useful specialization by explicitly separating ownership of technical viability, business and authority authorization, release scope, runtime correction, and reassessment when the basis of operation becomes invalid. A later bidirectional comparison tests that hypothesis against STAMP/STPA, Simplex and other runtime-assurance architectures, control-theoretic research, production ML and software engineering, risk and AI management systems, and implementation platforms—including what the four-horizon model itself flattens, renames, duplicates, or misses.
...
## 5. Applying the Map Without Overbuilding

The complete map creates a practical risk of its own: mistaking visibility for required bureaucracy. Once boundaries, evidence, decision authority, corrective action, and reassessment are all made explicit, the easiest implementation mistake is to create a new document, role, service, or gate for each one. That would confuse the **structure of the control problem** with the **shape of its implementation**.

A material relationship does not need a framework-specific artifact merely because the paper names it. It needs a credible **carrier**: some record or operating mechanism through which the relationship remains clear enough to operate. An architecture decision can carry a technical rationale. A product requirement can carry an approved operating contract. An IAM policy or transaction gateway can carry an authority boundary. CI, an evaluation store, and runtime telemetry can carry evidence. A release workflow can carry a bounded decision. An incident process can carry corrective and reassessment paths. Several relationships may share one carrier, and one relationship may span several carriers.

This section therefore proposes a **carrier-sufficiency test**. It is a paper-level application hypothesis under validation, not a new conformance requirement: proportionality is credible when every relationship that is material for the controlled object remains operable and traceable somewhere, while redundant representation is avoided.

Materiality has to be decided **before** choosing a preferred carrier. For this test, a relationship is material when losing, misrepresenting, or changing it could materially alter the intended outcome; satisfaction of an applicable Requirement or Constraint; delegated authority or reachable consequence; a correctness, release, continuation, or stop decision; the ability to correct, contain, verify, or reassess operation; or technical, operational, or control-economic viability. Otherwise a team can make any architecture look lightweight by declaring its inconvenient relationships unimportant after seeing where they would have to live.

A carrier is credible only end to end. The relevant owner and legitimate authority must remain identifiable; the intended outcome and applicable operating contract must be recoverable; source, scope, assumptions, guarantee strength where applicable, active version, and current authorization effectivity must not be ambiguous; changes must have a lifecycle; evidence must have a known consumer and remain fit for that decision; the resulting decision must connect to an effective Actuator; the action must be verifiable; and failure must route to the level whose decision basis is actually challenged. Access, retention, and auditability matter where the decision depends on them. A carrier may be very small. It may not be semantically empty.

### Running Example | Same Business Proposal, Different Candidate Project Scope

Section 4 followed the action-capable support system through authorization, Delivery, and Runtime. To isolate proportionality from maturity, consider a **counterfactual sibling at the same Project / Architecture candidate-design point**.

In the lower-authority candidate, Model Judgment interprets the request and drafts or recommends a resolution, but the system cannot communicate directly with the customer, execute a refund, or change account state. An adequately informed and empowered support agent may reject or replace the recommendation, makes the consequential decision, and executes the downstream action. This does **not** remove the sibling from the Thinking-System category as defined in this paper: the system's recommendation or draft is itself a **Consequential Runtime Responsibility** because it materially influences the downstream human decision, and that software responsibility depends partly on probabilistic Model Judgment. The human remains the consequential decision-maker and executor, so the human path reduces reachable system authority; it does not make the model-mediated consequential responsibility fully specified through explicitly encoded logic.

The canonical candidate keeps the broader scope already used in the paper: direct consequential communication may be allowed, and bounded low-value refunds may eventually execute through delegated authority while higher-value transactions remain reserved to Human Authority. The comparison below is deliberately made **before** final Organizational Business Authorization and Delivery approval on both sides. It compares proposed obligations, not a prototype on one side with a mature production system on the other.

| Comparison dimension | Draft / recommendation + human execution | Direct communication + bounded refund execution |
|---|---|---|
| Lifecycle point | Project / Architecture candidate design before final Business Authorization and Delivery approval | Project / Architecture candidate design before final Business Authorization and Delivery approval |
| Proposed operating-contract basis | Human owns consequential communication and transaction decisions; the candidate must preserve usable recommendations, bounded context, reviewability, and a real human reject/replace path | Candidate may communicate directly and exercise bounded transaction authority; proposed contract must also bound direct customer effects, transaction authority, failure handling, and restoration |
| Model-Judgment-dependent consequential responsibility | Interpretation and recommendation materially shape the human decision even though the human executes it | Interpretation, remedy selection, consequential communication, and selected bounded actions may affect the customer or business state directly |
| Reachable authority and effects | No autonomous send, refund, or account-state mutation | Proposed direct communication plus bounded refund/credit execution; higher authority remains reserved |
| Material boundaries and guarantee claims | Deterministic no-send/no-write and data/identity boundaries may carry Hard claims; recommendation and semantic-policy quality remain Soft where only probabilistic influence is available | Adds a deterministic transaction/approval boundary for the authorized action path; end-to-end semantic resolution correctness—interpretation, grounding, policy mapping, remedy choice, parameters, and communication—remains Soft where Model Judgment is involved |
| Human / decision authority | Human is the consequential decision-maker and must have information, competence, time, capacity, independence, and real power to reject or replace the model recommendation | Human Authority remains substantive for reserved transactions, exceptions, overload, and cases outside delegated scope; fallback and capacity become part of the proposed control perimeter |
| Proposed evidence and corrective action | Recommendation quality, source use, accept/edit/reject behavior, review load, no-write assurance, and scope violations; disable or fall back to ordinary human handling | Adds evidence about direct communication, attempted/blocked transactions, approvals, downstream results, realization health, Actuator execution, fallback, compensation, and restoration |
| Candidate carrier composition | Existing architecture/product decision records, human support workflow, read-only permissions, evaluation evidence, and the planned release/reassessment path may carry most relationships | The same classes of carriers remain useful, but Project must additionally require credible transaction-authorization, direct-communication, evidence, fallback, and corrective-action properties for later Delivery realization |
| Economics | Model cost plus the latency and labor of substantive human decision and execution | Model/platform cost plus transaction and communication controls, evidence, Human Authority for reserved cases, fallback/recovery, incident handling, and residual exposure |
| Reassessment | Review capacity, automation bias, degraded evidence, or pressure to add direct authority can invalidate the candidate basis | New reachable authority, non-credible transaction gating, Human Authority/fallback failure, direct-effect incidents, or changed control economics can invalidate the candidate basis |

The broader candidate therefore creates additional material obligations **because it can cause more directly**, not because the map demands more paperwork. The lower-authority candidate can legitimately use a lighter composition where human execution actually reduces authority, consequence, irreversibility, or automation dependence. If the human merely rubber-stamps suggestions, lacks the information to challenge them, or cannot intervene in time, the apparent reduction is mostly cosmetic.

There is also a separate **lifecycle-maturity delta**. Once Organization chooses to pursue the canonical candidate on a viable production basis and Project issues a production-capable Project Authorization, Delivery turns proposed operating-contract properties into an approved Requirement and Operating Envelope, implements the transaction guard and evidence paths, makes a Release decision, and activates Runtime carriers. Those are consequences of maturing a candidate into authorized operation. They are not evidence that broader authority alone requires more documents. A viable lower-authority candidate taken to production would also acquire Delivery and Runtime carriers appropriate to its own scope.

### From material relationships to credible carriers

The same discipline applies after authorization. The active operating contract, authority, Constraints, evidence, decisions, and reassessment paths may be distributed across existing systems. The question is not whether a single artifact contains the map, but whether the **connected relationship survives across the composition**.

The following rows are only an illustrative excerpt from the canonical action-capable case, not the complete material analysis:

| Material relationship | Possible credible carrier composition | Evidence, decision, action, and reassessment semantics |
|---|---|---|
| Organizational Business Authorization coverage → production Project Reauthorization boundary | Existing business/initiative decision record + Project architecture/authorization record | Project may reauthorize a changed technical design locally only while the resulting production scope remains inside the existing Organizationally owned authorization envelope; crossing that envelope requires renewed or reshaped Organizational authorization before a corresponding production-capable technical baseline |
| Active Requirement and Operating Envelope | Product/requirement record + release configuration + linked Project Authorization | Defines intended outcomes, deterministic and model-mediated obligations, exposure, failure handling, and active scope; Delivery evidence supports correctness and Release decisions, while invalidated Project assumptions route to Project reassessment |
| Organizationally delegated cumulative refund/credit authority and deterministic transaction gate | Organizational authority record + one shared authoritative per-order financial ledger + automated `KTX` and reserved-Human `HATX` execution lanes. The shared ledger atomically enforces the illustrative €50 automated sublimit and €500 total in-object EUR ceiling; non-EUR/above-€500/higher-role cases become `finance-review-required` and stop inside the controlled object | Hard claims apply only to the deterministic authority/transaction path under stated assumptions: current ordinary authorization and protective readiness, shared atomic reservation across automated and Human lanes, idempotency/reconciliation, and fail-closed uncertainty. A Human approval is not a bypass and cannot exceed remaining shared headroom. The controlled object does not emit an executable Finance instruction; external Finance review is independently initiated outside the object. Semantic remedy correctness remains Soft; invalid authority, stale factual/decision basis, failed gating, or ineffective Human Authority triggers containment and the owning reassessment |
| End-to-end semantic resolution correctness | Approved policy/data sources + prompt/context/tool realization + current evaluator-applicability evidence + sampled Human review + runtime complaints/incidents/outcomes | Interpretation, factual grounding, policy mapping, remedy selection, in-range parameters, and consequential communication remain Soft where Model Judgment is involved. Current semantic evidence is nevertheless a required input to autonomous action: stale, unavailable, unsupported, or out-of-envelope evidence routes to Human Authority/fallback rather than being treated as approval; persistent failure can require narrowing, recalibration, redesign, or earlier-horizon reassessment |

The table is intentionally many-to-many. A Requirement record does not become a Constraint Realization because it describes the boundary. An evaluator does not become the release authority because its score is consumed by a gate. An approval workflow does not create the authority it routes. A feature flag provides Actuation but does not authorize itself. Conversely, forcing every distinction into a separate file can make the path harder to reconstruct rather than easier.

The gap-closing rule is therefore conservative: **reuse a credible existing carrier first**. Add a new record only when ownership, lifecycle, scope/version, access, retention, or auditability would otherwise become unclear. Add automation only when a repeatable sensing, checking, routing, bounded-decision, or Actuation path is credible given evidence quality, failure behavior, reversibility, consequence, and delegated authority—and keep that automated path observable and correctable. Add an operating practice only when a material decision or reassessment path would otherwise be absent. Fewer artifacts do not mean fewer decisions; more artifacts do not imply stronger control.

Carrier sufficiency is also not a one-time filing exercise. A changed authoritative source, business authorization, Project scope, model, population, policy, evidence instrument, Delivery realization, or Runtime condition can make a previously adequate carrier incomplete. Renewed inspection is **bidirectional**: it may require stronger control, narrower authority, a different carrier, or evidence-instrument recalibration, but it may also justify consolidating duplicate representation or removing control work whose relationship is no longer material under the changed scope. The mapping therefore has to be versioned and reopenable. In the next section, serious existing methods and platform compositions are tested in both directions: not only against the relationships derived here, but also for **material relationships they contain that this map may have missed**. If reverse mapping exposes such a relationship, the map must change before any whole-system substitution verdict can stand.

> **The paper proposes the following application test: a proportionate implementation is sufficiently represented when every relationship that is material under the stated pre-substitution materiality rule has a credible carrier, and when a changed decision basis or reverse-mapping discovery triggers renewed inspection. This remains a paper-level hypothesis under validation rather than a new conformance requirement.**

Once carrier sufficiency is explicit, the next question is whether current engineering methods, safety and runtime-assurance approaches, standards, platforms, governance systems, and ordinary organizational practice already carry these relationships well enough that any additional operating augmentation is unnecessary.

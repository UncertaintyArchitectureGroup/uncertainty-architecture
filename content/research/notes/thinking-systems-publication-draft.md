...
| **Explicitly authored consequential responsibility** _(descriptive contrast)_ | A Consequential Runtime Responsibility fulfilled entirely through explicit logic rather than depending partly on runtime Model Judgment. This describes the responsibility boundary, not a second canonical system category or a sequential topology. |
...
**Relationship to established systems-theoretic control.** This article does not claim that hierarchical socio-technical control—or its extension into management and regulatory authority—is new; [STAMP/STPA](https://psas.scripts.mit.edu/home/get_file.php?name=STPA_handbook.pdf) already supports control structures that can span those levels. The narrower working hypothesis is that, for model-judgment-dependent software, the four-horizon model provides a useful specialization by explicitly separating ownership of technical viability, business and authority authorization, release scope, runtime correction, and reassessment when the basis of operation becomes invalid. The larger working paper's planned bidirectional comparison tests that hypothesis against STAMP/STPA, Simplex and other runtime-assurance architectures, control-theoretic research, production ML and software engineering, risk and AI management systems, and implementation platforms—including what the four-horizon model itself flattens, renames, duplicates, or misses.

...
2. separates category membership from consequence severity and implementation depth, distinguishes Thinking Systems from agentic applications, and introduces proportionality through one intentionally low-consequence case;
...
### External pre-publication review checkpoint — Maximiliano Armesto

**Maximiliano Armesto** reviewed the complete standalone publication draft before publication. Treat that response as conceptual critique and revision provenance, not endorsement or co-authorship. The review confirmed that the responsibility-following and bounded-control argument was legible while creating four revision obligations for the owning research pair and the publication adaptation:

- replace the publication-facing label `Linear Software`, which repeatedly invited a topology reading, with the research-only comparative label **Explicitly Authored Software**, aligned to explicitly authored consequential responsibilities;
...
- separate category membership, consequence severity, and implementation depth, and introduce one intentionally low-consequence case before the complete map appears;
- position the four-horizon model directly against STAMP/STPA while preserving the broader bidirectional comparison with Simplex/runtime assurance, control theory, production ML/software engineering, risk and management systems, implementation platforms, and mature internal organizational compositions.

**Tracked research items for this reconciliation:** `TS-TERM-002` (opposite-side terminology), `TS-SCOPE-001` (definition scope versus release-contract coherence), `TS-HIST-001` (pre-LLM historical cases), `TS-LOW-001` (low-consequence boundary case), `TS-PROP-001` (proportionality distinction), and `TS-COMP-001` (STAMP/STPA comparison). These IDs are the cross-document identities; this blueprint owns the detailed article-level reasoning for them.
...
This iteration uses **Explicitly Authored Software** as a paper-level comparative label under validation. It does **not** rename the draft-normative glossary entry **Linear Software**; any repository-wide canonical rename requires a separate deliberate framework review with coupled glossary and doctrine changes. The full comparative mapping remains unfinished research. The standalone adaptation should add the minimum direct positioning needed to avoid implying untested novelty, not compress a responsible cross-framework study into a superficial paragraph.
...
→ category membership does not determine consequence severity or maximal control implementation; implementation depth remains proportionate to consequence, authority, exposure, reversibility, uncertainty, feedback latency, Human Authority load, operating capacity, and control economics
...
→ for the motivating class covered by the release-contract deduction, Delivery also places a runtime judgment process into operation that completes part of the consequential mapping; the changed object is the judgment-dependent situation-to-consequence mapping, not necessarily the cardinality of terminal outputs; in LLM-based systems, a few allowed actions may still depend on Model Judgment across a large, context-dependent space of situations, meanings, and evidence that is not exhaustively authored in advance; credible production use requires the surrounding control architecture to keep resulting system operation, reachable authority, and consequential effects within the approved boundary
...
→ runtime negative cases may therefore reveal a local Delivery realization defect, invalidate a Delivery release basis, invalidate a Project / Architecture assumption about judgment, Operating Envelope, Human Authority, or control economics, or invalidate an Organizational authority/business premise; evidence must route to the horizon whose decision basis is challenged
...
**Full-map and proportionality rule.** The article intentionally presents the full decision-and-capability map needed to reason about a complex, high-consequence Thinking System. Article §1 must make the separation visible before the full model appears by using one intentionally low-consequence case and stating that category membership, consequence severity, and implementation depth are distinct. The article must state explicitly that the complete map does **not** mean every system requires every mechanism, role, artifact, approval path, Sensor, or Actuator shown in it. Simpler, lower-consequence systems should use a proportionate subset justified by consequence, authority, exposure, reversibility, uncertainty, feedback latency, realization difficulty, Human Authority load, operating capacity, and control economics. The purpose of teaching the complete map is diagnostic: even when implementation is intentionally lightweight, teams should inspect the whole map first so they do not accidentally build a system with broad authority, slow feedback, hidden cross-level dependencies, or expensive control requirements while treating it as a simple LLM feature. Proportionality may simplify implementation; it must not hide complexity that is actually present.
...
**Early STAMP/STPA positioning rule.** Article §4 must include one bounded paragraph, with direct primary-source links to Leveson's STAMP work and the STPA handbook, acknowledging that STAMP models hierarchical socio-technical control structures extending into management and regulatory authority and that STPA applies that model in systems-theoretic safety analysis. It may present explicit lifecycle decision ownership for model-judgment-dependent software as the four-horizon model's working practitioner hypothesis, but it must not state that delta as established novelty. Article §6 retains the systematic bidirectional verdict and remains broader than STAMP/STPA: it must also compare Simplex and related runtime-assurance architectures, mathematical/control-theoretic research, production ML and software/systems engineering, risk and management systems, implementation tools and integrated platforms, and mature internal organizational compositions. The comparison must record what the derived map flattens, renames, duplicates, or fails to preserve, not only what another approach leaves outside its normal scope.
...
- Explain the release-contract shift: explicitly authored software releases an implementation of an already specified consequential mapping, whereas Thinking-System Delivery also places into operation a judgment process that completes part of that mapping at runtime. Make clear that the general distinction is whether the consequential decision structure is determined through explicitly authored logic before release or partly completed by Model Judgment at runtime, not the number of terminal outputs. In LLM-based systems, a small enumerated action set may still depend on Model Judgment over a large, context-dependent space of situations, meanings, and evidence. Production readiness depends on whether the surrounding control architecture can keep resulting system operation, reachable authority, and consequential effects within the approved boundary despite judgment that remains unresolved until runtime.
- Describe the open judgment-dependent decision mapping rather than equating it with the terminal action set. In motivating LLM cases the space of possible situations, meanings, evidence, and relevant distinctions may be large and only partly characterized even when allowed downstream actions are tightly enumerated. Keep three dimensions separate: behavioral acceptability (desired / accepted only within stated conditions or residual bounds / prohibited), epistemic status (sufficiently characterized / uncertain / unclassified), and authority (within delegated authority / reserved for Human Authority). Do not imply that Thinking-System classification itself requires semantic interpretation, a large terminal output set, or an enumerable or estimable full distribution.
- Connect negative outputs to decision-basis routing: some reveal local Delivery realization defects, some invalidate a Delivery release basis, some invalidate Project / Architecture assumptions or control economics, and some invalidate an Organizational authority/business basis.
...
- [ ] Article §2 explains the release-contract shift without equating Explicitly Authored Software with one execution path, claiming the full outcome distribution is enumerable, or asserting a universal increase in error count; it connects negative outcomes to the decision basis they invalidate.
...
- fixed, linear, iterative, adaptive, or dynamically selected orchestration does not determine whether software is Explicitly Authored Software or a Thinking System;
...
- Show a simple classification boundary: ask whether any **Consequential Runtime Responsibility** depends partly on probabilistic Model Judgment. Route **No** to Explicitly Authored Software and **Yes** to Thinking System. Show orchestration topology, autonomy, and delegated authority as independent dimensions that affect architecture, risk, and control demand but do not decide category membership.
...
Figure 3 uses the **same Mermaid panel-layout pattern already proven by Figure 8/9** rather than relying on disconnected subgraphs or cross-links between internal nodes. The canonical paper source is inline Mermaid in both the long-form manuscript and standalone publication adaptation: top-level `flowchart TB`; an outer transparent `ROW3` subgraph with `direction LR`; inside it, left subgraph `A` (`Explicitly Authored Software`) and right subgraph `B` (`Motivating runtime-judgment class`), each with `direction TB`; and a non-semantic dashed relation between the **subgraphs themselves** (`A -. "responsibility-structure comparison" .- B`) to make the two panels participate in one LR row. The left internal responsibility chain is `A1 --> A2 --> A3`. The right side preserves parallel authored and judgment paths `B1 --> B2 --> B3` and `B1 --> J1 --> B3`. Do **not** add alignment or comparison links between internal nodes of the two panels (for example `A2 ~~~ J1`), because such links can override the local TB directions in GitHub Mermaid. `ROW3` is layout-only and visually transparent. The panel-to-panel dashed relation is a visual comparison relationship, not an execution or control edge. For PDF publication, `quartz/scripts/publication-figure3.mjs` remains the deterministic reviewed rendition contract and may replace this canonical Mermaid at export time, but Markdown must contain only the inline Mermaid representation, not an additional reader-facing SVG. Any future change to Figure 3 must preserve the two side-by-side panels and top-down internal flows or receive explicit visual review.
...
- **Superseded on 2026-08-24 for publication-facing research:** the 2026-08-12 iteration retained the draft-normative label **Linear Software** for the responsibility boundary. External review showed that the label repeatedly invited a topology reading. The paper now uses **Explicitly Authored Software** as a research-only comparative label; the canonical glossary remains unchanged pending separate framework review.
...
**Blueprint-owned canonical working mapping — `support-resolution-canonical-material-map / v12`.**

This is the complete working material-relationship map for the canonical action-capable support-resolution case as revised after the eleventh independent Article §5 review. It is a **research object**, not a conformance table and not evidence from a real deployment. The carrier column describes the operating composition posited by the editorial case; equivalent existing records or mechanisms may substitute where they preserve the relationship semantics. Version `v12` is not yet frozen for a whole-controlled-object substitution verdict. Article §6 must record an immutable mapping-content digest or intentionally preserved repository snapshot after freeze and must name that exact mapping ID/version/snapshot in any verdict.

**Mapping header**

- **Mapping ID / version:** `support-resolution-canonical-material-map / v12`.
- **Controlled object / lifecycle baseline:** the canonical action-capable customer-support-resolution Thinking System after a technically viable design has received Organizational Business Authorization, a production-capable Project Authorization, Delivery approval, and production release. The system may communicate directly with customers and may execute refunds/credits up to the paper's illustrative delegated threshold; higher-value refunds remain reserved to substantive Human Authority. The threshold remains illustrative and non-normative.
- **Logical editorial baseline identifiers:** `ORG-SUP-SOURCES v1` (the applicable Organizational source set), `OBA-SUP-01 v1` (Organizational Business Authorization), `AUTHZSTATE-SUP-01 v1` (operational projection of the currently effective applicable Organizational/Project authorization set), `PROTECTAUTH-SUP-01 v1` (standing bounded protective-control authority for disable/contain/rollback/shutdown independent of ordinary business-action permission), `PROTECTSTATE-SUP-01 v1` (fresh operational projection of protective-authority effectivity plus bound protective-Actuator health), `PCA-SUP-01 v1` (Project viability/control-architecture decision basis), `PA-SUP-PROD-01 v1` (production-capable Project Authorization), `REQ-SUP-01 v1` + `OE-SUP-01 v1` (Delivery-approved Requirement and Operating Envelope), `DOR-SUP-01 v1` (Delivery readiness decision), `DOD-SUP-01 v1` (Delivery completion/evidence decision), `RG-SUP-01 v1` (Release Gate decision), `REL-SUP-01 v1` (released Delivery baseline), `MODEL-SUP-01 v1` (active model), `PROMPT-SUP-01 v1` (active instruction/prompt set), `CTX-SUP-01 v1` (active approved context/retrieval configuration), `TOOLS-SUP-01 v1` (active routing/tool configuration), `KDATA-SUP-01 v1` (automated/application customer-account data-access realization), `HAREAD-SUP-01 v1` (active separately authorized human-support/incident read path for this canonical baseline), `KSEND-SUP-01 v1` (direct-communication realization), `KTX-SUP-01 v1` (automated delegated refund transaction realization), `HATX-SUP-01 v1` (separately authorized Human-Authority transaction execution path for reserved financial actions), `FINLEDGER-SUP-01 v1` (authoritative shared per-order reservation-and-settlement state enforcing the automated sublimit and total in-object financial ceiling across automated and Human lanes), `CTRL-SUP-01 v1` (active Runtime Controller decision/routing logic, including evidence thresholds and escalation conditions), `ACTCFG-SUP-01 v1` (active Actuator-selection/binding configuration), `HA-SUP-01 v1` (active substantive Human Authority decision/approval path), `HAENTITLE-SUP-*` (current per-actor Human entitlement assertion tied to the versioned identity/support-role authority source), `FB-SUP-01 v1` (fallback/recovery realization), `EVAL-SUP-01 v1` (active semantic-evidence instrument set), `EVALVAL-SUP-01 v1` (active evaluator validation/calibration/applicability snapshot), and `AUDIT-SUP-01 v1` (active material-evidence access/retention/integrity policy and retained-record basis). `POP-SUP-01 v1`, `DATA-SUP-01 v1`, and `ENV-SUP-01 v1` identify the bounded customer/account population, approved data classes/sources, and production environment used by the editorial baseline. Per-case `FACTSNAP-SUP-*`, `ELIGDEC-SUP-*`, and `SEMDEC-SUP-*` identify the decision-relevant factual snapshot, eligibility decision, and semantic-resolution acceptance actually consumed by a consequential action. These are synthetic identifiers for the editorial case, not repository artifacts or claims about a real deployment.
- **Source / authorization baseline:** `ORG-SUP-SOURCES v1` contains the applicable legal/contractual/support-policy/data/security/vendor/deployment sources adopted for the case. `OBA-SUP-01 v1` authorizes the bounded support-resolution production envelope. `PCA-SUP-01 v1` records the Project-selected Thinking-System design, its viability assumptions, and required operating-contract/evidence properties. `PA-SUP-PROD-01 v1` carries the exact production-capable technical scope to Delivery. `REQ-SUP-01 v1` + `OE-SUP-01 v1` define the approved bounded operating contract. `DOR-SUP-01 v1`, `DOD-SUP-01 v1`, and `RG-SUP-01 v1` preserve readiness, completion/evidence sufficiency, and release as distinct Delivery decisions. `AUTHZSTATE-SUP-01 v1` is an authenticated monotonic operational projection of the applicable authoritative Organizational/Project decisions; it does not create or reinterpret authority. `PROTECTAUTH-SUP-01 v1` is an independently authenticated, versioned, currently effective protective authorization that separately permits only disable/contain/rollback/shutdown actions needed to reach or preserve an authorized safe/contained state after ordinary business-action permission is suspended; compensation or other new consequential business action is outside this standing protective authority unless separately authorized. `PROTECTSTATE-SUP-01 v1` operationalizes readiness separately from execution: it is an authenticated, freshness-bounded projection of current `PROTECTAUTH-SUP-01 v1` effectivity plus the health/binding of declared protective execution dependencies. **Readiness gating is Hard:** stale, indeterminate, or unhealthy `PROTECTSTATE` deterministically blocks new effectful customer-communication and refund/credit commits; this does not pretend that a failed protective Actuator can execute itself. The separate bounded protective-execution claim applies only while the declared protective Controller/Actuator/Sensor dependencies needed for that action remain available. For this editorial baseline, when those dependencies are available, the Operating Envelope defines **`L_protect = 20 seconds` end to end from the qualifying protective trigger to the verified effect-contained state**. For an Organizational emergency suspension explicitly requiring faster protective cessation, the authenticated authoritative suspension event itself is the trigger and its delivery/ingestion to the protective Controller is included in the 20-second budget; for an internal control-health event, the trigger is the first qualifying Sensor event. The budget allows at most 5 seconds for trigger delivery/detection, at most 5 seconds for Controller decision/dispatch, and the remaining budget for protective Actuation plus verification. If `PROTECTSTATE` becomes unhealthy because a required protective execution dependency itself is unavailable, new effectful commits still fail closed, but this baseline makes **no `L_protect` completion guarantee through that unavailable path**; the failure is retained and escalated for alternate authorized emergency handling and Project reassessment. The 20-second objective and sub-bounds are illustrative local assumptions, not UA defaults. `REL-SUP-01 v1` binds the released scope to `AUTHZSTATE-SUP-01 v1`, `PROTECTAUTH-SUP-01 v1`, `PROTECTSTATE-SUP-01 v1`, `MODEL-SUP-01 v1`, `PROMPT-SUP-01 v1`, `CTX-SUP-01 v1`, `TOOLS-SUP-01 v1`, `KDATA-SUP-01 v1`, `HAREAD-SUP-01 v1`, `KSEND-SUP-01 v1`, `KTX-SUP-01 v1`, `HATX-SUP-01 v1`, `FINLEDGER-SUP-01 v1`, `CTRL-SUP-01 v1`, `ACTCFG-SUP-01 v1`, `HA-SUP-01 v1`, `FB-SUP-01 v1`, `EVAL-SUP-01 v1`, `EVALVAL-SUP-01 v1`, and `AUDIT-SUP-01 v1`. No research-only Project Authorization is active in this baseline. If one is later introduced while production authorization remains active, its stable Bounded Research Authorization, research-only Project Authorization, research DoR/DoD/Release, and experimental release-baseline identifiers plus explicit separation, overlap, nesting, or precedence relationship become part of the material authorization set; a research Release produces a versioned experimental baseline rather than `REL-SUP-01 v1` and cannot promote itself into production. That experimental baseline must bind the applicable research authorization set plus its current `AUTHZSTATE-SUP-*` projection, applicable protective-control authority, active model/configuration, evaluator-validity, Controller/Actuator, Human Authority/fallback, and audit/retention identities needed for Runtime correction, verification, and later reconstruction.
- **Organizational Business Authorization coverage represented by `OBA-SUP-01 v1`:** the authorized support-resolution business outcome/use scope; intended customer population/environment and external exposure owned by Organization; reserved versus delegated communication authority; and, for this editorial baseline, an **illustrative total in-object refund/credit authority of cumulative €500 per order in EUR across automated and designated support-supervisor paths, with an automated subdelegation capped at cumulative €50 per order across all support cases and automated refund/credit paths**. The €500 total includes any amount already reserved or settled through the automated €50 lane; automated and Human paths therefore share one order-level financial headroom rather than additive €50 + €500 authority. Non-EUR actions, amounts above the cumulative €500 in-object ceiling, and actions requiring higher financial authority are outside this controlled object and require an independent Finance process. Project / Architecture may derive and realize technical Controls for these boundaries but may not redefine their aggregation unit, covered financial-action set, currency basis, or reserved/delegated meaning. The authorization also carries approved data/geography/vendor/deployment conditions where these are explicit Organizational premises, material service/value/investment assumptions, and explicit renewal conditions. Project-owned model, prompt, routing, tool, and other technical choices are not frozen unless a specific choice is itself made an Organizational condition. The €50 and €500 amounts are deliberately illustrative and non-normative.
- **Scope boundaries:** `POP-SUP-01 v1` is the bounded customer/account population for the canonical support-resolution service; `DATA-SUP-01 v1` is the approved customer/account/support-policy data-source and data-class set; `ENV-SUP-01 v1` is the production environment in which `REL-SUP-01 v1` is active. Direct consequential communication is permitted only through `KSEND-SUP-01 v1`. **Automated** refund/credit execution is permitted only through `KTX-SUP-01 v1` and inside the cumulative €50 automated subdelegation. **In-object reserved Human** refund/credit execution is permitted only through `HATX-SUP-01 v1` and inside the remaining shared `FINLEDGER-SUP-01 v1` headroom up to the cumulative €500-per-order EUR ceiling. Non-EUR, above-€500, or higher-role financial cases are not executed or externally instructed by this controlled object: it records a bounded `finance-review-required` case state and stops financial Actuation; a Human outside the controlled object independently initiates any Finance transfer, authorization, and execution. Billing-plan changes, account closure, and other higher-authority actions are outside automated scope. Geography is **not a separately constraining dimension in this editorial baseline**. A specific model/provider identity is **not an Organizational Business Authorization condition** in this baseline and remains Project/Delivery-owned unless a later Organizational decision explicitly makes it one.
- **Operational authorization-effectivity assumption:** `AUTHZSTATE-SUP-01 v1` accepts only authenticated, authorized-source updates and advances through a monotonic authorization epoch/version so replay or rollback to an older permission state is rejected. Conflicting source versions, projection partition, unverifiable provenance, or inability to establish the current epoch make the state indeterminate and fail closed for new consequential commits. A gate may either perform authoritative commit-time introspection or consume a signed/attested lease bound to one authorization epoch. If leases are used, this editorial baseline fixes the illustrative Operating-Envelope maximum revocation latency at **`L_rev = 30 seconds`**; the Hard claim is bounded to that latency rather than pretending instantaneous revocation. The value is local to this editorial case and not a UA default. A suspension/supersession creates a higher epoch, prevents issuance/renewal of old-epoch leases, and invalidates uncommitted action no later than `L_rev`; an Organizational emergency suspension explicitly marked for faster protective cessation uses the separately authorized protective path, with the authenticated authoritative suspension event itself starting the **20-second end-to-end `L_protect`** clock declared above. If the emergency signal or required protective execution dependencies are unavailable, the faster protective guarantee is failed/not claimed for that path; the ordinary `L_rev` enforcement still applies and the protective-path failure is control evidence. Where an external effect cannot be committed atomically with the authorization-state check, the guarantee covers only effects whose commit began under a still-valid assertion/epoch; already-committed effects are handled through verification/correction rather than retroactively reclassified.
- **Per-case decision binding:** autonomous consequential Actuation uses a versioned `FACTSNAP-SUP-*` decision-relevant factual snapshot plus `ELIGDEC-SUP-*` eligibility and `SEMDEC-SUP-*` semantic-resolution acceptance. `FACTSNAP-SUP-*` captures or version-binds the material case/order/account/customer-message and other factual state on which eligibility/remedy/payload depends. For any financial remedy or eligibility decision, it also binds the relevant `FINLEDGER-SUP-*` order-state version, prior compensation/reservation/settlement state, and remaining headroom used by that decision. `ELIGDEC-SUP-*` binds the case and `FACTSNAP-SUP-*` to the active `AUTHZSTATE-SUP-*`, applicable release/experimental baseline, model/prompt/context/policy basis, relevant `CTRL-SUP-*` decision logic, and `EVALVAL-SUP-*` applicability snapshot. `SEMDEC-SUP-*` binds the proposed interpretation/remedy/parameters/payload to that same factual snapshot, authorization/baseline, and current `SR-M23` evidence under the active Operating Envelope. A material configuration/evaluator change, material factual-state change, or material `FINLEDGER` state change affecting the remedy/eligibility basis invalidates the affected cached decision state; an execution gate may consume it only while all bound versions remain current or a deterministic commit-time freshness/revalidation check proves that the relevant facts are unchanged. Human Authority decisions are likewise bound to the applicable authorization/baseline, exact consequential action basis, and decision-relevant factual snapshot unless the owning policy explicitly permits a broader durable approval.
- **Human-entitlement effectivity assumption:** `HAENTITLE-SUP-*` is an authenticated per-actor assertion derived from the current versioned identity/support-role authority source and records actor, role/action scope, source/policy version, issue/expiry and revocation-relevant state. Reserved customer send, `HATX`, and `HAREAD` require either authoritative access/commit-time entitlement introspection or a signed non-expired assertion whose **illustrative maximum entitlement staleness is `L_ha = 5 minutes`**. Revocation/scope change prevents renewal; stale, expired, unverifiable, mismatched, or indeterminate entitlement fails closed for the corresponding Human action/access. `L_ha` is a local editorial assumption, not a UA default.
- **Effect-contained state predicate for `SR-M25`:** the support-resolution system is effect-contained when no new customer communication can commit through `KSEND-SUP-*`, no new refund/credit can reserve or commit through `KTX-SUP-*` or `HATX-SUP-*`, and model/Human decisions cannot reach those effectful Actuators. Case ingestion, evidence collection, and read-only automated `KDATA-SUP-*` may remain available only while their own current authorization/identity/data-scope controls remain valid; `HAREAD-SUP-*` remains governed independently by `SR-M26`. Thus “contained” means bounded cessation of the declared external consequential effects, not necessarily process termination or loss of all diagnostic read access.
- **Explicit local assumptions used by this editorial baseline:** `KDATA-SUP-01 v1` is the automated/application customer-account access path and binds authenticated service identity, tenant/customer identity, current `AUTHZSTATE-SUP-*`, and `DATA-SUP-01 v1` source/data-class allowlisting; suspension of the automated Thinking-System production authority therefore blocks new automated KDATA use. `HAREAD-SUP-01 v1` **exists in this canonical baseline** as a distinct human-support/incident read path governed by `SR-M26`: its own versioned security/support/legal access policy, current `HAENTITLE-SUP-*`, authentication, least-privilege subject/data scope, audit, revocation and unavailable/conflict behavior; model-production suspension does not silently grant or revoke that human access, and fallback may rely on it only while its own authority remains effective. `KSEND-SUP-01 v1` rejects autonomous send unless identity, current authorization epoch, applicable release/experimental scope, allowed channel, case/account/destination binding, current `FACTSNAP-SUP-*`, matching `ELIGDEC-SUP-*`, and matching `SEMDEC-SUP-*` inside the active Operating Envelope all remain current at commit; reserved/ambiguous send requires current `HAENTITLE-SUP-*` plus an authenticated `HA-SUP-01 v1` decision bound to current authorization/baseline, factual snapshot, case/account/destination/channel, and exact final payload identity/hash/version. `KTX-SUP-01 v1` is the automated delegated financial lane: it binds identity, customer/account, case, order/eligible transaction target, current factual snapshot, EUR value basis, stable compensation identity, and idempotency key. `FINLEDGER-SUP-01 v1` is the **single authoritative per-order financial ledger for both in-object lanes**: every automated or Human reservation is serialized against the same order state; it atomically enforces automated cumulative reservations/settlements at or below €50 and combined automated + Human cumulative reservations/settlements at or below €500; stable compensation/idempotency identity prevents replay; unknown outcomes keep headroom reserved until reconciliation. Autonomous settlement additionally requires current `AUTHZSTATE-SUP-*`, current healthy `PROTECTSTATE-SUP-*`, `FACTSNAP-SUP-*`, `ELIGDEC-SUP-*`, and `SEMDEC-SUP-*`. Reserved above-automated-boundary actions inside the controlled object require current `HAENTITLE-SUP-*` plus a `HA-SUP-01 v1` decision and execute only through `HATX-SUP-01 v1` for the designated support-supervisor role, consuming only the remaining shared `FINLEDGER-SUP-*` headroom up to the €500 total. Any material shared-ledger reservation/settlement change after a financial `FACTSNAP` was established stales the affected `SEMDEC` or Human financial decision (and `ELIGDEC` where eligibility depends on prior compensation) unless deterministic revalidation re-establishes the semantic/factual basis before reservation/execution. Non-EUR, above-€500, or higher-role cases cannot execute through either in-object lane. The system only records `finance-review-required` and stops financial Actuation; it does not transmit an executable instruction, pre-authorize Finance, or itself initiate the external transfer. A Human outside this controlled object may independently initiate Finance review under that external process. "Reserved to Human Authority" therefore does not mean unlimited financial authority, and absent current `HATX-SUP-*` permission or sufficient shared `FINLEDGER-SUP-*` headroom the Human may decide/escalate but this controlled object cannot execute the transaction. `KDATA-SUP-*`, `KSEND-SUP-*`, `KTX-SUP-*`, and `HATX-SUP-*` fail closed for the action they gate when required authority, identity, factual/decision binding, approval, ledger/idempotency, or realization-health state is unavailable or indeterminate. `PROTECTAUTH-SUP-01 v1` survives suspension of ordinary business-action authority only while its own authenticated version/effectivity and bound protective-Actuator health remain established, and only for bounded protective disable/contain/rollback/shutdown needed to reduce or stop exposure; it cannot authorize new customer communication, refund/credit, or compensation unless another current authority does. Loss or indeterminate state of this protective path suspends new ordinary consequential commits rather than authorizing continued exposure without a credible correction path. Reserved Human Authority demand remains below **15% of cases** and p95 decision latency remains at or below **15 minutes**. `FB-SUP-01 v1` is not a universal recovery path: for model/evaluator/controller failure it may preserve manual/degraded case handling only if `HAREAD-SUP-*` and other required human-support dependencies remain authorized/available; send-gateway failure permits containment/queueing but no claimed delivery through the failed path; payment/ledger uncertainty blocks further transaction pending reconciliation; automated identity/data-binding failure blocks account action rather than being bypassed by fallback. Any alternate human communication or transaction path must be separately authorized/evidenced. Restoration is complete only when direct transaction/communication authority is disabled or returned to the last known authorized applicable release/experimental-baseline state and that state is observed. `EVAL-SUP-01 v1` is used only under active `EVALVAL-SUP-01 v1`, which binds the evaluator to Requirement/OE outcome definition, model, prompt, context/retrieval, tools/routing, relevant Controller thresholds, population, policy/data-distribution, and operating conditions; material basis change stales affected evidence until revalidated/narrowed. `AUDIT-SUP-01 v1` keeps material authorization, Human Authority, release/experiment, evidence-validity, action, financial-settlement, incident, correction, factual-snapshot, and per-case decision records access-controlled, retrievable, and integrity-verifiable through the predeclared active/historical reconstruction horizon. Material events remain correlatable to the logical identifiers above. The percentage, Human-Authority latency, **30-second `L_rev`**, **20-second end-to-end `L_protect`**, automated €50 sublimit, and total in-object €500 financial ceiling are illustrative local assumptions, not UA defaults.
- **Conditional research-equivalence rule:** the current baseline is production-only. `SR-M18` and the research variants embedded in shared DoR, DoD, Release, Runtime-correction, verification, and reconstructability rows are **inactive conditional extensions** for a current-production-only substitution verdict. They become part of the equivalence basis only when the verdict scope explicitly claims the capability to open and carry a reserved-boundary research branch. A substitute must not fail a production-only verdict merely for omitting those inactive research extensions, and must not claim lifecycle-wide equivalence if it omits them while claiming reserved-research capability.
- **Materiality decision rule:** include a relationship when its absence, misrepresentation, or material change could alter the intended outcome; satisfaction of a Requirement or Constraint; delegated authority or reachable consequence; a correctness, release, continuation, or stop decision; the ability to correct, contain, verify, or reassess operation; or technical, operational, or control-economic viability. Apply this rule before judging a carrier or substitute. Current accountable research owner for the v12 materiality judgment is the article maintainer; Article §6 must challenge it through reverse mapping rather than treat the author's partition as self-validating.
- **Change log / supersedes:** `v12` supersedes the unfrozen `v11` working map after the eleventh independent review separated deterministic protective-readiness gating from conditional protective-Actuator latency, aligned the emergency protective clock with the authoritative emergency-suspension event, defined the exact effect-contained target state, materialized Human support/incident read access as `SR-M26`, made Human actor-entitlement freshness explicit across HAREAD/reserved send/HATX, and bound shared financial reservation/settlement state into the factual/semantic decision basis. Git history preserves `v1`–`v11`; no whole-controlled-object verdict was issued against any of them. Any later material change to relationship set, materiality rationale, source/scope/assumptions, guarantee semantics, or substitution equivalence basis requires another version increment.

| Relationship ID | Material relationship | Source / scope / assumptions | Guarantee strength where applicable | Materiality rationale | Active carrier or carrier composition | Owner / authority | Decision / reference semantics | Evidence / intended consumer / action / verification | Lifecycle / change control / reassessment |
|---|---|---|---|---|---|---|---|---|---|
| `SR-M01` | Organizational Business Authorization coverage bounds production Project Reauthorization | `OBA-SUP-01 v1` coverage + `PCA-SUP-01 v1`; only explicit Organizational conditions freeze technical choices | N/A — authority relationship | Crossing this boundary can change whether Project has authority to issue a production-capable technical baseline | Organizational business/initiative decision record + linked Project viability/authorization record | Legitimate Organizational business/authority owner; Project owns technical viability inside that basis | Project may reauthorize a changed technical design only when it remains viable and the resulting production scope stays covered by the applicable existing Business Authorization; otherwise Organization renews, reshapes, or changes the basis first | Project change analysis identifies whether outcome, population/exposure, reserved authority, data/geography/vendor/deployment conditions, service/value/investment assumptions, or renewal conditions are crossed; decision is consumed by Project before technical authorization | Any material production-scope change rechecks coverage; changed coverage triggers Organization → Project reassessment before Delivery proceeds |
| `SR-M02` | Model-Judgment necessity, alternative selection, production viability, and Architectural Veto remain explicit Project decisions | `OBA-SUP-01 v1` standing Organizational outcome/authority basis + `PCA-SUP-01 v1`; material scenarios and candidate alternatives | N/A — decision relationship | If this rationale is wrong, the system may carry unnecessary judgment, authority, cost, or control burden; if no credible bounded control architecture exists, treating that result as ordinary residual risk would bypass the engineering feasibility boundary | `PCA-SUP-01 v1` architecture/project decision basis carrying alternatives, scenarios, control feasibility, Human Authority/fallback, economics, and viability status | Project / Architecture | Project selects the technical design inside the standing Organizational basis, confirms category, and issues the viability conclusion. **Architectural Veto** means no credible complete bounded control architecture exists for the unchanged proposal: Project cannot issue `PA-SUP-PROD-01` for that proposal, and Organization may only change the outcome/scope/authority/assumptions or another owned premise and return the changed proposal for Project reassessment; it may not waive the veto by accepting the same non-credible architecture. Economic non-viability remains a distinct finding because Organization may change business levers | Evaluation, feasibility, capacity, economics, incident, and research evidence consumed by Project; action may narrow/redesign/research or issue Architectural Veto; a selected non-Thinking-System path exits this lifecycle after category confirmation | Reassess when evidence invalidates necessity, feasibility, authority, Human Authority, fallback, evidence sufficiency, or economics; any changed Organizational basis returns to Project before a new technical authorization is issued |
| `SR-M03` | Production-capable Project Authorization defines the exact technical scope Delivery may realize | `OBA-SUP-01 v1` positive Business Authorization on the viable `PCA-SUP-01 v1` basis; Project-selected Thinking-System design | N/A — technical authorization relationship | Delivery release without this boundary could expand authority or exposure beyond the viable/authorized design | Versioned Project authorization/baseline linked to business authorization and Project Constraint Architecture | Project / Architecture within Organizational coverage | Carries exact production technical scope, intended Judgment landscape, required operating-contract properties/evidence, delegated control authority, dependencies, assumptions, and reauthorization triggers | Delivery checks inherited authorization before DoR/Release; violations or contradictory evidence route to Project; Project verifies resulting authorization version before Delivery continues | Versioned; any scope/authority or viability-basis change triggers Project reassessment and possibly Organizational review under `SR-M01` |
| `SR-M24` | Current Organizational/Project authorization effectivity, suspension, expiry, supersession, and revocation latency are enforced at consequential execution paths | Applicable Organizational Business Authorization + Project Authorization set (and bounded-research authorization where conditionally applicable) + authenticated monotonic `AUTHZSTATE-SUP-*` epoch + applicable release/experimental baseline + `SR-M25` protective-readiness relationship | **Hard only within the stated enforcement model:** new ordinary consequential commits require either authoritative commit-time authorization introspection or a non-expired authenticated lease bound to the current monotonic authorization epoch, and require current healthy protective readiness under `SR-M25`. Source updates are authenticated and anti-replay; conflicts/partition/indeterminate provenance fail closed. If leases are used, no old-epoch lease may authorize a new commit after the 30-second `L_rev`; the map does not claim zero-latency revocation | A technically healthy deployment can otherwise keep producing effects after owning authority changes, while a system without a credible current protective path may continue exposure it cannot bound or stop | Authoritative Organization/Project decisions → authenticated monotonic authority epoch/projection → commit-time introspection or bounded leases → ordinary execution gates, jointly conditioned on current `PROTECTSTATE-SUP-*` → retained transition/denial evidence | Organization and Project own authoritative decisions; Delivery owns faithful authenticated projection/enforcement; Runtime consumes but cannot redefine state; protective authority/readiness remains separately bounded under `SR-M25` | Only currently effective ordinary authorization plus current protective readiness may enable new ordinary consequential commits. Suspension/expiry/supersession advances authority state; ordinary action ceases under `L_rev`; protective containment follows `SR-M25` and cannot create new business permission | Source signatures/authorized-writer identity, epoch, projection/partition state, lease issuance/expiry, gate denials, `PROTECTSTATE` freshness, post-change attempts and resulting containment are consumed by Delivery/Runtime/Project/Organization | Basis change advances the effective epoch and invalidates affected execution/decision state. Failure to meet `L_rev` or establish current protective readiness is a control failure requiring suspension/containment and owning reassessment; new scope still requires the owning authority |
| `SR-M25` | Protective readiness deterministically gates new effectful operation, while bounded protective-execution latency is claimed only when the declared protective execution dependencies are available | `PROTECTAUTH-SUP-01 v1` + current `PROTECTSTATE-SUP-*` + protective Controller/Actuator/Sensor bindings + OE `L_protect = 20 seconds` for available-path trigger→verified effect-contained state; Organizational emergency-suspension events themselves start the clock where faster cessation is requested | **Two scoped Hard claims, not one circular claim:** (1) readiness gating: stale/indeterminate/unhealthy `PROTECTSTATE` rejects new `KSEND`/`KTX`/`HATX` effectful commits; (2) when the declared protective execution dependencies remain available, a qualifying trigger must reach the `SR-M25` effect-contained predicate within 20 seconds. If a required protective execution dependency is itself unavailable, no completion-latency guarantee is claimed through that failed path | A failed protective Actuator must not be assumed to execute itself, while loss of correction readiness must still stop accumulation of new external effects | Authenticated/versioned `PROTECTAUTH` + freshness-bounded `PROTECTSTATE` Sensor + effectful execution-gate precondition + protective trigger/Controller/Actuator path + post-action verification | Project owns viability/declared dependencies and target state; Delivery realizes/tests; Runtime consumes readiness and invokes only authorized available protective actions; protective authority cannot create new business effects | Current healthy readiness is required for new external-effect commits. Loss/staleness closes those gates. With dependencies available, emergency Organizational suspension uses the authoritative signed suspension event as trigger; internal health cases use the first qualifying Sensor event. With a failed protective execution dependency, operation remains effect-blocked and the unavailable-path latency claim is failed/not asserted | `PROTECTSTATE` freshness, dependency health, authoritative emergency-trigger timestamp/delivery, Controller dispatch, Actuator execution, verification, gate-denial and unavailable-path evidence are retained for Runtime/Delivery/Project | Readiness-gate bypass, failure of an available path to meet `L_protect`, or protective-dependency unavailability is control evidence requiring continued effect blocking and Project reassessment/alternate authorized emergency handling; changed protective scope/authority follows its owner before operation resumes |
| `SR-M04` | Delivery-approved Requirement and Operating Envelope define acceptable operation for the bounded released scope | `PA-SUP-PROD-01 v1` + `REQ-SUP-01 v1` / `OE-SUP-01 v1` + `REL-SUP-01 v1` + `POP-SUP-01 v1` / `DATA-SUP-01 v1` / `ENV-SUP-01 v1` | N/A as a whole; individual deterministic obligations may have separate Hard realizations | Without an active operating contract, evidence and correctness/release decisions lack a stable reference | Product/requirement record + Delivery review/release record + deployed scope/configuration references | Delivery/release authority within Project Authorization and explicit delegated Delivery change authority | Defines intended outcomes, deterministic obligations, model-mediated expectations, acceptable variation, prohibited outcomes, exposure/resource conditions, evidence expectations, and failure handling | Deterministic tests, evaluation evidence, runtime outcomes, cost/latency/capacity and failure evidence consumed by Delivery/Runtime Controllers; actions include block, limit, release, rollback, narrow, or escalate | Requirement/OE changes are Delivery-local only when they remain inside **both** the active `PA-SUP-PROD-01 v1` and Delivery's explicitly delegated change authority. A change beyond Delivery delegation returns to Project even if a broader Project scope could technically contain it. Changes invalidating Project viability or `OBA-SUP-01 v1` coverage route to the corresponding earlier horizon before a new baseline becomes active |
| `SR-M05` | Automated/application customer-account access is restricted to the currently authorized tenant/customer binding and approved data-source/data-class set; separately governed Human support read access is outside this Hard path | Organizational data/security sources + applicable Project Authorization/OE + `DATA-SUP-*` + current `AUTHZSTATE-SUP-*`; automated service identity/tenant binding assumptions from header; any `HAREAD-SUP-*` path has its own independent authority | **Hard for this scoped automated/application data-access path:** `KDATA-SUP-*` rejects access unless current effective `AUTHZSTATE-SUP-*`, authenticated service identity, tenant/customer binding, and approved source/data-class allowlist all match. Stale/unavailable/indeterminate authorization, identity, binding, allowlist, or realization-health state fails closed. This claim does not cover a separately authorized Human support/incident read path, geography, or unrelated deployment dimensions | Unauthorized automated data access or identity confusion can change reachable consequence; conflating KDATA suspension with independent Human read rights can either create a bypass or remove the access needed for legitimate containment | `AUTHZSTATE-SUP-*` + scoped service identity + tenant/customer-bound retrieval filter + source/data-class allowlist + rejecting automated access layer + audit; `HAREAD-SUP-*` is a separate carrier where authorized | Security/data authority owns source/access rules; Project/Delivery realize automated access inside delegation; Human support/security owners govern any HAREAD path separately | Automated subjects, bindings, sources or data classes are rejected unless current authority is effective. Model/tool availability cannot create access authority. Suspension of automated production permission blocks new KDATA use but neither grants nor silently revokes independently governed Human support access | Authorization-state/epoch traces, denial logs, cross-tenant tests, allowlist tests, identity/configuration health consumed by security/Runtime/Delivery; Actuators revoke/narrow credentials or disable automated path; resulting access state verified | Organizational data/security-source change may activate Organization directly; technical identity/platform defects remain Delivery/Project as appropriate; Human-read authority changes follow their own owning source rather than being inferred from KDATA state |
| `SR-M26` | Human support/incident read access is independently authorized, scoped, current, revocable and evidenced rather than inferred from automated KDATA or production authorization | Versioned security/support/legal access source + approved Human-readable data scope + authenticated actor identity + current `HAENTITLE-SUP-*` + `HAREAD-SUP-*`; illustrative `L_ha = 5 minutes` maximum cached entitlement staleness where introspection is not used | **Hard for this scoped Human read-access path:** HAREAD rejects access when actor identity, entitlement role/action scope, source/policy version, approved subject/data scope or realization health is stale, expired, revoked, mismatched, unavailable or indeterminate. Current entitlement is established by authoritative access-time introspection or a non-expired signed assertion within `L_ha`; this claim is independent of KDATA and ordinary Thinking-System production authorization | Human fallback/incident handling depends on this access, while treating it as merely “available” or reconstructable can hide stale Human privilege or unauthorized data scope | Versioned Human-access policy + identity provider/role source + `HAENTITLE` + least-privilege HAREAD realization + revocation/denial/audit evidence | Security/support/legal access authority owns policy and entitlements; Delivery integrates but cannot expand them; Human operators consume only their current entitlement | Human support/incident reads are allowed only for the approved actor/subject/data purpose and current entitlement. Production-model suspension neither grants nor silently revokes this separate path. Entitlement/source/scope change is enforced on its own authority timeline | Actor/policy/entitlement version, issue/expiry/revocation state, subject/data scope, access/denial logs and realization health are consumed by security/support/incident owners and retained for Project/audit reconstruction | Revocation/source/scope change invalidates stale Human access. Unavailable/conflicting authority or inability to enforce the declared bound fails closed for new HAREAD access and can invalidate fallback viability, routing to Project while the access owner resolves entitlement/source issues |
| `SR-M06` | Direct customer communication executes only for the authorized case/account, destination, channel, identity, currently effective authorization/baseline, current protective readiness, and a current factual + autonomous-or-human decision basis | Applicable Project Authorization + Requirement/OE + population + `AUTHZSTATE-SUP-*` + current healthy `PROTECTSTATE-SUP-*` + current `FACTSNAP-SUP-*` + `SR-M22` / `SR-M23`; autonomous communication uses matching `ELIGDEC-SUP-*` and `SEMDEC-SUP-*`; reserved communication additionally requires current `HAENTITLE-SUP-*` and a scoped Human Authority send decision | **Hard for the send-authority/execution-precondition path:** autonomous or reserved send rejects stale/indeterminate ordinary authorization or protective readiness. Autonomous send additionally requires matching baseline/scope, identity, channel, case/account/destination, factual snapshot, eligibility and semantic acceptance; reserved send requires current Human entitlement plus a Human decision bound to current authorization/baseline, factual snapshot and exact payload hash/version. Semantic correctness remains Soft | External communication can continue after authority, facts, Human entitlement or correction readiness becomes invalid unless those states are joined at commit | Outbound gateway + authority epoch + `PROTECTSTATE` + scoped identity/destination + factual/eligibility/semantic preconditions for autonomous send + `HAENTITLE` + exact Human-approved payload/fact binding for reserved send + audit | Runtime/Delivery operate delegated path; Runtime Controller authorizes only while current preconditions hold; legitimate Human Authority owns reserved decision only while its entitlement and higher authorization are current | Autonomous send requires current authority + protective readiness + facts + eligibility + semantic acceptance. Reserved send additionally requires current Human entitlement and valid Human decision. Material authority/protective/factual/payload/entitlement change invalidates pending execution | Denials, authority epoch/lease, `PROTECTSTATE`, factual freshness, eligibility/semantic decisions, Human entitlement/approval/payload hash, gateway health and delivery result are consumed by Runtime/Delivery/Project; resulting state retained | New channel/population/authority, protective-readiness failure, stale Human entitlement, stale fact/decision reuse, propagation failure or decision/payload-binding bypass triggers suspension/containment and owning reassessment |
| `SR-M07` | Refund/credit execution preserves the Organizationally defined shared cumulative in-object financial authority and revalidates semantic/factual decisions when shared financial state changes | OBA defines €500 total and €50 automated sublimit; OE + `AUTHZSTATE-SUP-*` + healthy `PROTECTSTATE-SUP-*` + authoritative shared `FINLEDGER-SUP-*`; financial `FACTSNAP` binds the relevant ledger version/prior compensation/headroom; automated path additionally consumes current `ELIGDEC`/`SEMDEC`; Human path uses current `HAENTITLE` + `HA-SUP` + `HATX` | **Hard for financial-authority/execution preconditions:** `FINLEDGER` serializes all in-object reservations, enforces automated cumulative ≤€50 and combined automated+Human ≤€500, and holds unknown outcomes until reconciliation. `KTX` requires current authorization/protective/factual/eligibility/semantic state; `HATX` requires current authorization/protective readiness, Human entitlement, bound Human decision and supervisor scope. A material ledger-state change that affects remedy appropriateness stales the corresponding semantic/Human decision unless deterministic revalidation re-establishes it. Semantic correctness itself remains Soft | Atomic authority bounds alone do not prevent a semantically stale refund after another case has already changed the order’s compensation state | Shared `FINLEDGER` + `AUTHZSTATE` + `PROTECTSTATE`; financial factual snapshot/version; automated `KTX` + `ELIGDEC`/`SEMDEC`; Human `HAENTITLE` + decision + `HATX`; stable compensation/idempotency identity and reconciliation | Organization owns total/subdelegated authority; Project derives architecture; Runtime may authorize only current eligible/semantically accepted automated transactions; Human Authority acts only inside current entitlement/role limits and remaining shared headroom; Delivery operates/evidences both | Automated settlement requires current authority + readiness + current financial facts/ledger basis + eligibility + semantic acceptance + shared atomic reservation. Human settlement requires current authority + readiness + current entitlement + bound decision based on current financial facts + shared reservation. Non-EUR/above-€500/higher-role cases stop at `finance-review-required` | Shared ledger versions/reservations/settlements/headroom, factual/semantic revalidation, authority/protective state, Human entitlement/approval, idempotency, gateway health, reconciliation and audit are consumed by Runtime/Delivery/Project; resulting financial state verified | Ledger-state change invalidates affected cached semantic/Human decisions where compensation history/headroom is decision-relevant. Failure of either shared bound, entitlement, revalidation, readiness or reconciliation triggers suspension/containment and Project reassessment; authority-meaning change requires Organization |
| `SR-M22` | Only cases with a current factual-state-bound semantic eligibility decision may reach autonomous consequential Actuation; financial eligibility also remains current to any decision-relevant shared compensation state | Reserved/delegated authority + Project/PA autonomy boundary + Requirement/OE + `AUTHZSTATE`; active model/prompt/context/policy/controller/evaluator + `EVALVAL`; per-case `FACTSNAP` + `ELIGDEC`; for financial eligibility, `FACTSNAP` includes the relevant `FINLEDGER` version/prior compensation state | **Soft for semantic eligibility; Hard only for downstream freshness/binding/execution preconditions.** Autonomous send/transaction cannot execute without an allowed eligibility decision whose case, factual snapshot, authorization, baseline, behavioral/configuration basis, Controller logic and `EVALVAL` remain current, or deterministic revalidation proves relevant facts unchanged. A decision-relevant `FINLEDGER` change invalidates financial eligibility where prior compensation/headroom is part of the eligibility basis | Eligibility can become wrong after customer/order/compensation state changes even when model/prompt/evaluator versions do not | Eligibility policy/context + model/evaluator evidence + `EVALVAL` + factual/financial snapshot + `ELIGDEC` + Runtime Controller + deterministic execution preconditions + Human Authority queue | Organization owns reserved/delegated basis; Project derives eligibility boundary; Delivery realizes/evidences; Runtime consumes only current state; Human Authority owns reserved decisions | Autonomous Actuation requires current facts and allowed eligibility. Ambiguous/reserved/unsupported/evaluator-invalid/authorization-stale/fact-stale or decision-mismatched cases are not autonomous; financial eligibility is re-established when relevant ledger state changes | Eligibility outputs, fact/ledger versions and freshness checks, `ELIGDEC`, false-eligible/false-reserved review, Human decisions, complaints/incidents, denials/outcomes consumed by Runtime/Delivery/Project | Material authorization, Requirement/OE, policy, model, prompt/context, evaluator/applicability, population, Controller, baseline, factual or decision-relevant financial-state change invalidates affected cached eligibility; systematic false eligibility/HA overload may invalidate Project viability |
| `SR-M23` | Model-Judgment-dependent interpretation, grounding, policy mapping, remedy selection, consequential parameters/payload and communication remain consistent enough with the active operating contract **for the current factual and, where financial, compensation state** | Support-policy/data sources + Requirement/OE + data scope + `AUTHZSTATE`; active model/prompt/context/tools + evaluator + `EVALVAL`; `FACTSNAP` + `SEMDEC` bind interpretation/remedy/parameters/payload to current material facts, and financial decisions additionally bind the relevant `FINLEDGER` version/prior compensation/headroom | **Soft for semantic correctness. Hard only for routing/freshness:** autonomous Actuation is unavailable unless current semantic acceptance shows required evidence usable/in-envelope and the decision-relevant factual/financial snapshot still matches, or deterministic revalidation re-establishes the material basis. Stale/out-of-envelope evidence or changed case/order/account/customer-message/decision-relevant compensation state routes to Human Authority/fallback | A remedy can become semantically inappropriate after another case changes prior compensation even while the financial ceiling still has remaining headroom | Approved case/account/order/policy context + factual/financial snapshot + prompt/tool realization + evaluator under active applicability + `SEMDEC` + sampled Human review + complaint/incident/post-action evidence | Delivery/Runtime Controllers operate within current PA; policy/data owners own source meaning; Project owns semantic-evidence/correction viability; Human Authority handles unsupported/reserved cases inside current higher authorization | Semantic evidence is mandatory input but not correctness proof. Autonomous send/settlement requires current factual/financial basis + semantic acceptance + eligibility + Hard authority gate. Material fact or decision-relevant ledger-state change invalidates pending semantic acceptance until re-established | Intent/fact/policy/remedy/amount/payload evidence, fact/ledger versions/freshness, `SEMDEC`, Human edits/rejections, complaints/reversals/outcomes consumed by Runtime/Delivery/Project; resulting state verified | Material authority, policy/data/model/prompt/context/tool/population/OE/evaluator-applicability, factual state, or decision-relevant `FINLEDGER` state invalidates affected semantic acceptance until re-established/narrowed; persistent wrong resolution or inability to evidence decisions may invalidate Project viability |
| `SR-M09` | Human Authority is substantive for reserved decisions/exceptions; each consequential decision is bound to current higher authority, exact action basis, decision-relevant factual state, and **current actor entitlement** | Reserved decision rights + Project viability assumptions about information, competence, time, capacity, independence and power + current `AUTHZSTATE` / baseline + `FACTSNAP`; `HAENTITLE-SUP-*` derives current actor role/action scope from the versioned identity/support-role source; financial Human decisions also bind current `FINLEDGER` factual state/headroom | N/A as a human-capability relationship; deterministic Human execution/read gates may separately be Hard | A nominal approval can fail because the human lacks substance, facts/financial state changed, approval is replayed, or the actor’s entitlement was revoked while a stale session remains usable | Human Authority workflow + authenticated reviewer identity + `HAENTITLE` + decision context/factual snapshot + action-specific decision record + fallback/escalation; reserved financial execution through `HATX`/`FINLEDGER` | Legitimate Human Authority owns reserved decision only inside current higher authorization and current role/action entitlement; Project owns capacity/viability; Delivery/Runtime operate path | Human may approve/reject/modify/escalate with real power. Consequential approval records higher authorization/baseline, factual snapshot and exact action basis. Financial approval binds the relevant shared-ledger state; execution authority arises only after current entitlement check and successful reservation. Material fact/ledger/basis/action/entitlement change invalidates pending approval unless owning policy explicitly permits a broader durable approval | Decision/reject/edit records, authority/baseline, factual/financial snapshot, entitlement source/version/assertion, role/limit, ledger reservation, queue/load/latency, competence and execution evidence consumed by Runtime/Project | Local queue issues may be Delivery; material basis/factual/financial/entitlement change invalidates affected approvals; persistent capacity/latency or entitlement/authority mismatch triggers Project viability reassessment and possibly Organization |
| `SR-M10` | Material Sensors/evaluators remain fit for the decisions they support, with applicability state versioned independently from evaluator identity | `EVAL-SUP-01 v1` + `EVALVAL-SUP-01 v1` + active Requirement/Operating-Envelope outcome/correctness basis + model + prompt/instruction + context/retrieval + tools/routing + relevant Controller eligibility/decision thresholds + population + active policy/data-distribution/operating-condition basis for the applicable production or research experimental baseline | N/A — evidence-sufficiency relationship | An unchanged evaluator implementation can become invalid after its decision basis changes; reconstructing only evaluator identity can make stale evidence look valid | Evaluation store + evaluator/rubric/Golden Set version records + **versioned validation/calibration/applicability snapshot** + monitoring | Delivery/evaluation owner operates the instrument and applicability record; Project owns evidence-feasibility assumptions; each relevant Controller consumes evidence only under an applicable snapshot | Evidence is usable only with known coverage, uncertainty, latency, blind spots, active evaluator version, and an active `EVALVAL-SUP-*` snapshot binding it to the decision-relevant baseline. Material basis change stales the prior applicability snapshot for affected decisions even if `EVAL-SUP-01 v1` itself is unchanged | Drift/change signals and calibration/validation checks consumed by Delivery/Runtime/Project; actions recalibrate, create a new applicability snapshot, replace/narrow use, change threshold/rubric, or escalate; decisions record which evaluator **and applicability snapshot** supported them | Material Requirement/OE outcome definition, model, prompt/instruction, context/retrieval, tool/routing, Controller threshold/decision semantics, population, policy/data-distribution, or operating-condition change rechecks instrument applicability. Until a new `EVALVAL-SUP-*` snapshot exists, affected evidence is narrowed, marked stale, or withheld; lost evidence feasibility can invalidate Project basis |
| `SR-M20` | Definition of Ready remains a bounded decision about whether implementation or an authorized experiment may begin | **applicable Project Authorization type/scope** + proposed bounded Delivery scope + inherited Constraints/operating-contract properties/evidence obligations; production work uses `PA-SUP-PROD-01 v1`, while a reserved-boundary experiment would instead require the applicable specific Bounded Research Authorization + research-only Project Authorization for that experiment | N/A — readiness-decision relationship | Starting work with unresolved authority, Judgment scope, realization feasibility, evidence, Human Authority, fallback, or escalation semantics can create implementation that cannot be validly completed or released | Delivery planning/review workflow + linked Project authorization and architecture/evidence records | Delivery/engineering readiness authority within Project Authorization; contradictions to Project basis route upward | Ready means authority, scope, Judgment Nodes, required realizations, evidence plan, Human Authority/fallback, assumptions, and reassessment path are explicit enough to begin the bounded work. Outcomes include ready, ready with conditions/bounded experiment where authorized, revise locally, or Project reassessment; DoR does not create broader Project or Organizational authority | Readiness evidence is consumed before implementation starts; unresolved items trigger local clarification or Project reassessment rather than being deferred silently; any authorized experiment remains inside its applicable authorization type/scope | Re-run DoR when the bounded work scope or inherited basis changes materially; Project contradiction cannot be accepted locally; DoR remains distinct from DoD and Release |
| `SR-M21` | Definition of Done remains an implementation-and-evidence-completeness decision distinct from Release for the **applicable authorization type/scope** | Applicable Project Authorization + bounded Requirement/OE + applicable DoR + implemented realizations/evidence for that scope. Production uses `PA-SUP-PROD-01 v1` / `DOR-SUP-01 v1`; reserved-boundary research uses its specific Bounded Research Authorization + research-only Project Authorization and research DoR | N/A — completion/evidence-decision relationship | Conflating completion with Release can authorize exposure before required realization/evidence is complete, while treating research completion as production completion can silently promote an experiment | Versioned Delivery DoD decision carried by the review/change workflow with links to the applicable authorization, implementation, realization, evaluation, fallback, Human Authority, and baseline evidence | Delivery completion/evidence authority only inside the applicable Project Authorization type/scope; Project owns contradictions to that basis | DoD asks whether implementation/experiment obligations and required evidence are complete for the **authorized bounded scope**. Production DoD may proceed only to the production Release Gate. Research DoD establishes completion/evidence sufficiency for the declared experiment and may proceed only to the research Release Gate; it does not become production DoD, authorize production exposure, or widen either Project Authorization | Deterministic tests; realization/bypass/failure/degradation/unavailability evidence; active technical/baseline identities; behavioral evidence and limitations; Sensor/Controller/Actuator evidence; Human Authority/fallback evidence; experiment stopping/evidence obligations where applicable; and confirmation that Delivery evidence does not invalidate Project assumptions are consumed before the corresponding Release Gate | Re-run DoD after material implementation, realization, evaluator, model/configuration, fallback, or bounded-scope/authorization change. Incomplete work returns to Delivery; evidence invalidating Project basis routes to Project; research completion returns evidence to Project viability after its research Release/experiment and never promotes itself into the production chain |
| `SR-M11` | Release remains a bounded exposure decision distinct from completion and bound to the **applicable authorization type/scope** | Applicable Project Authorization + bounded Requirement/OE + corresponding DoD + Release Gate. Production uses `PA-SUP-PROD-01 v1` and `RG-SUP-01 v1` leading to `REL-SUP-01 v1`; reserved-boundary research uses its specific Bounded Research Authorization + research-only Project Authorization and a versioned research Release Gate leading only to a versioned experimental release baseline/record | N/A — release-decision relationship | A complete implementation or experiment may still be unacceptable for the requested exposure; collapsing research and production Release can silently convert evidence generation into production permission | Existing release/change/experiment workflow + linked applicable authorization and evidence references | Delivery/release authority only within the applicable Project Authorization type/scope | Outcomes may release the authorized production scope, release/limit the declared experimental exposure, condition the requested exposure, return to rework, **block or defer this release/exposure**, or route to Project when its basis is challenged. A research Release cannot create or reuse `REL-SUP-01 v1`, cannot promote research authorization into production, cannot expand Project Authorization, and cannot change an Organizational basis | Corresponding DoD, control health, residual exposure, capacity/economics, stopping conditions for research, and active-version evidence consumed by the appropriate Release Controller; deployment/feature flags are Actuators; post-release/experiment state verified | New evidence inside Delivery authority may trigger rework/containment; challenged Project basis routes to Project. Research Release evidence returns to Project viability and the experimental baseline is closed/retained as evidence; production requires its own viable basis, production-capable PA, production DoD, and production Release decision |
| `SR-M12` | Runtime correction uses current ordinary authorization or separate standing protective authority, with readiness gating distinct from any protective-execution latency guarantee | Active Requirement/OE + Constraints + authorization set + `AUTHZSTATE` + `PROTECTAUTH` + current `PROTECTSTATE` + applicable production/research baseline where conditional + Controller/Actuator identities | N/A as composite; `SR-M25` carries the Hard readiness gate and conditional available-path `L_protect` claim | If protective dependency failure both closes ordinary gates and is incorrectly assumed to execute the same failed path, correction semantics become circular | Runtime/experiment Controller or Human Authority + ordinary authorization projection + protective authorization + freshness-bounded `PROTECTSTATE` + effectful gate closure + any still-available authorized protective Actuators; compensation only when separately authorized | Ordinary business correction remains inside current ordinary authority. Protective authority survives ordinary suspension only for bounded protective actions. Unhealthy readiness always blocks new effectful commits; protective Actuation is attempted only through dependencies that remain available | Suspension removes new ordinary effects but retains available protective actions. Loss/staleness of readiness closes effectful gates. If the lost readiness reflects a failed protective execution dependency, the system does not claim that dependency will satisfy `L_protect`; it remains effect-blocked and escalates/uses an independently authorized alternate if one exists | Authority epoch, protective authority/state, dependency health, gate closure, trigger/dispatch/Actuator/verification timestamps, unavailable-path and resulting-state evidence return to Controller and are retained | Readiness bypass, failed available-path containment, unavailable protective dependency, or need for new business/compensation authority routes to Project/owning authority while effectful operation stays blocked |
| `SR-M13` | Material corrective action is verified against the resulting state, including the explicit `SR-M25` effect-contained predicate where protective containment is claimed | Actuator/gate command + expected authorized production/research state or explicit `SR-M25` effect-contained state + relevant Sensor path | N/A — verification relationship | A rollback/block/disable request can fail silently, and an undefined “contained” label can hide continued external effects | Actuator/gate logs + configuration/deployment/experiment state + outbound/financial effect state + downstream Sensor + retained authorization/baseline identity | Runtime/Delivery or experiment Controller consuming correction evidence | Action is successful only when the specific resulting production/degraded/research/stopped state is observed. For protective containment, verify no new `KSEND`/`KTX`/`HATX` effect can commit while any retained read-only processing stays within its own authority | Post-action permission/gate state, deployment/experiment exposure, transaction/outbound state, fallback/stopping evidence; Controller may retry another available Actuator or escalate; result retained under audit | Failed verification is a control failure; inability to establish the declared effect-contained predicate or restore an authorized state can invalidate Project viability and reactivate Organization where its basis is implicated |
| `SR-M14` | Fallback, containment, degraded operation, human support access, and recovery are credible only for the **specific failure class, dependency set, and function claimed** | Project/OE + `FB-SUP-01 v1`; automated `KDATA-SUP-*` and independently governed `HAREAD-SUP-*`; failure-class dependency/capacity/transition/restoration assumptions | N/A as a composite resilience claim unless separately deterministic | Calling every secondary path fallback can hide common-mode dependencies, and blocking the automated production path can accidentally be described as removing the human read access needed for incident/manual handling | Fallback/degraded case workflow + `HAREAD-SUP-*` where separately authorized + dependency/failure-class map + direct-action disable/containment + queueing/reconciliation + capacity/transition/restoration evidence | Project owns viable degraded functions; security/support/legal owners govern any human read path; Delivery realizes/tests; Runtime/Human Authority select only responses inside their respective current authority | Model/evaluator/controller failure may preserve manual handling **only if** the separately authorized human read/support dependencies remain available. Automated KDATA suspension does not imply human access, and human access cannot bypass its own revocation. Send failure permits queueing not delivery; payment uncertainty blocks transaction; identity/data authority failure may require stop. Protective disable/contain remains available under `PROTECTAUTH` | Dependency/common-mode and failure-injection tests, human-read authorization/health, queue/capacity, blocked-action evidence, reconciliation and restored-state evidence are consumed by Delivery/Runtime/Project; resulting degraded/restored state verified | Saturation, common-mode failure, unavailable human-support access, inability to contain, or inability to restore an authorized state triggers Project reassessment; Organization only where its basis is implicated |
| `SR-M15` | Material evidence remains retrievable, integrity-verifiable, and reconstructable against effective ordinary/protective authority, protective readiness, factual/financial state, behavioral/control state, evaluator validity, **Human entitlement**, shared financial state and per-case decision state | Production reconstruction includes Organization/Project authority + `AUTHZSTATE` + `PROTECTAUTH`/`PROTECTSTATE` + Requirement/OE/DoR/DoD/Release/baseline + model/prompt/context/tools/realizations + `KDATA` + `HAREAD` policy/`HAENTITLE` + `KSEND`/`KTX`/`HATX` + authoritative `FINLEDGER` + Controller/Actuator/Human/fallback + evaluator/`EVALVAL` + population/data/environment/audit. Material actions retain `FACTSNAP`, `ELIGDEC`, `SEMDEC` or Human-decision identities; research variants are conditional | N/A — provenance/reconstructability relationship | Stable component IDs alone cannot show whether authority/readiness/entitlement were effective, whether financial state changed after a decision, or whether the shared bounds and Human access were valid | Linked release/config/evaluation/observability/audit records + authority/protective histories + factual/financial snapshots + decisions + `FINLEDGER` history + HAREAD policy/entitlement history + retention/integrity controls | Delivery/Runtime record owners maintain; Project/Organization/audit/dispute owners consume; research obligations activate only when in verdict scope | Reconstruct authoritative epoch, protective readiness, factual/financial snapshot, evaluator applicability, autonomous/Human decision, Human entitlement, shared financial headroom/reservation and control configuration governing each material effect/correction/access | Correlated authority/protective timestamps, fact/ledger versions, decisions, entitlement source/assertion, Human role/limits, shared ledger, HAREAD access, payload/transaction, event and integrity evidence remain retrievable through historical horizons | Failure to reconstruct authority/revocation, protective readiness, financial decision freshness/headroom, Human entitlement/read access, factual freshness or decision state triggers containment and Project reassessment when trust becomes unreliable |
| `SR-M16` | Complete solution + control-perimeter economics, capacity, and latency remain part of viability without transferring business-attractiveness authority to Project | `PCA-SUP-01 v1` viability basis + `OBA-SUP-01 v1` service/value/investment assumptions + applicable production or research runtime evidence | N/A — viability-decision relationship | A technically controlled system can still become operationally/control-economically non-viable, while business attractiveness remains an Organizational decision | Project viability/control-economic analysis + runtime/research cost/capacity/latency/Human Authority/fallback evidence + business decision record | Project owns technical/control viability and characterizes lifecycle control burden against stated assumptions; Organization owns business outcome/basis and continue/reshape/defer/stop attractiveness decisions | Project concludes whether the control architecture remains technically/operationally viable and characterizes the material cost/capacity/latency/maintenance burden; it may narrow/redesign or return an economics finding. Organization decides whether that viable burden is attractive enough for the business basis and changes business levers where needed | Cost, latency, queue/capacity, false blocks, fallback load, incident/maintenance burden consumed by Project; Organization consumes the Project finding when business assumptions or continue/reshape/stop choices are implicated; resulting scope/basis verified in new authorization | Material control-economic assumption change triggers Project reassessment; changed business basis or attractiveness decision belongs to Organization and then returns to Project before new production authorization |
| `SR-M17` | Evidence routes by the decision basis it invalidates rather than the team that observed it, and each horizon selects only outcomes inside its authority | Nested lifecycle authority + active authorization/ownership semantics | N/A — reassessment-routing relationship | Misrouting can normalize broken architecture, over-escalate local defects, or let a lower level take a higher-level business/authority decision | Incident/change workflow + explicit Delivery/Project/Organization ownership references | Delivery, Project, and Organization each own only their respective decision bases | Local realization/config/evidence defect → **Delivery** may repair, contain, rollback, narrow or re-run its bounded decisions inside delegation. Technical necessity/viability/control-architecture/capacity/economics evidence → **Project** may redesign, technically reauthorize within standing Organizational coverage, issue Architectural Veto, or return a changed-basis finding. Organizational premise/authority/business-attractiveness change → **Organization** may change/renew/suspend the basis or reshape/defer/stop the initiative; Project then reassesses the changed basis | Evidence package identifies invalidated basis, active authority/baseline, consequence and requested decision; the owning Controller records its scoped outcome and subsequent state/authorization is verified | Routing semantics change only with legitimate ownership changes; repeated misrouting or lower-horizon attempt to take a higher-horizon outcome is itself process/control evidence |
| `SR-M18` | Reserved-boundary research remains distinct from assessment eligibility and production permission **as a conditionally material lifecycle-transition relationship; it is inactive in the current production baseline** | Standing Organizational basis + Project-defined experiment + specific Bounded Research Authorization when reserved exposure is crossed. It is part of substitution equivalence only when the tested claim includes the lifecycle capability to open/carry a reserved-boundary research branch, not for a current-production-state-only equivalence claim | N/A — conditional research-authorization relationship | Collapsing these states can expose reserved data/authority without authorization or let research silently become production, but requiring an inactive branch in every current-state substitution would overstate the tested scope | Project research proposal/decision record + Organizational Bounded Research Authorization + research-only Project Authorization + Delivery experiment/release record | Project defines experiment and evidence need; Organization authorizes reserved exposure; Project scopes technical research authorization; Delivery releases only that experiment | Project-local evidence inside standing envelope needs no new Organizational ceremony; reserved-boundary experiment requires specific authorization; research evidence returns to Project; production requires a separate viable basis and Business Authorization | Experiment scope/exposure/stopping/evidence records consumed by Project; Actuators bound/stop experiment; post-experiment evidence verifies scope and answers/reopens question | Inactive in the present production state. If the substitution claim includes lifecycle transition capability, new/expanded reserved exposure requires new Organization decision; research-only and production scopes may coexist only with explicit scope/precedence semantics; no automatic promotion. A current-production-state-only substitution verdict must state that `SR-M18` was outside the tested claim rather than treating the inactive branch as failed equivalence |
| `SR-M19` | Exogenous change to an Organizationally owned authoritative/business basis can activate Organization directly and advances the affected effective Runtime authorization epoch | Law, contract, policy, explicit Organizational vendor/deployment condition, shared capability, price/segment/funding/service/portfolio basis outside local Runtime/Delivery evidence path + affected authoritative state / `AUTHZSTATE-SUP-*` | N/A — reference-basis/authority-state relationship | Treating external change as runtime-generated evidence creates false causality; treating suspension as documentation-only can leave old effects running; pretending cached leases revoke instantaneously hides propagation latency | Organizational source/change/business decision system + authoritative signed/versioned authorization record + authenticated monotonic projection/epoch + bounded-lease or commit-time enforcement + protective disable notification to Delivery/Runtime | Legitimate Organizational owner changes its basis; Project owns technical consequences; Delivery owns faithful projection/enforcement; Runtime cannot redefine either | Organization may update/suspend/replace basis directly. Change advances authorization epoch. Ordinary gates cease new consequential commits according to the declared enforcement model/`L_rev`; where faster cessation is required, bounded protective disable/contain is invoked and verified. Purely technical model/vendor changes remain Project-first unless Organizational premise is implicated | Source/version/signature, epoch transition, projection/partition health, lease expiry/commit checks, gate denials, protective containment and Project reassessment evidence are consumed by owning Controllers | Exogenous Organizational change creates new basis/effectivity state; failure to authenticate/propagate/enforce within declared latency is a control failure; old ordinary permission cannot be recreated locally |

**Considered-but-excluded relationship inventory for `v12`.** The materiality rule applies to exclusions as well as inclusions. These candidates were considered explicitly and are not separate material relationships under this baseline; they can become material if scope or authority changes, and reverse mapping may reopen them.

| Candidate ID | Candidate relationship considered | Decision under current baseline | Exclusion rationale / where the necessary semantics remain carried |
|---|---|---|---|
| `SR-X01` | One standalone UA document for each material relationship | Excluded | Artifact identity is not a control relationship. Ownership/lifecycle/access may justify a new record, but the semantics can be carried by existing compositions; carrier sufficiency, not framework-specific file count, is the test. |
| `SR-X02` | One independent relationship for every cell in the four-horizon × four-capability reasoning matrix | Excluded | The matrix is diagnostic, not a requirement for sixteen components or records. Only end-to-end relationships that satisfy the materiality rule are retained. |
| `SR-X03` | Model, prompt, routing topology, tool, or provider identity as a separate Organizational authorization relationship | Excluded by default | These are Project/Delivery technical choices unless `OBA-SUP-01 v1` explicitly makes one an Organizational condition. Their active identity remains material for reconstructability under `SR-M15`, and technical changes may trigger Project reassessment. |
| `SR-X04` | Raw telemetry, evaluator score, or dashboard metric as an independent decision relationship | Excluded | Evidence is material only relative to a decision consumer, validity basis, action, and reassessment path; those semantics are carried by `SR-M10`, `SR-M11`, `SR-M12`, `SR-M15`, and `SR-M17`. |
| `SR-X05` | Existence of a human-approval UI as a separate assurance relationship | Excluded | A button or queue is not substantive Human Authority. Authority, information, competence, capacity, latency, independence, power, fallback, and evidence are carried by `SR-M09`; deterministic transaction gating is separately carried by `SR-M07`. |
| `SR-X06` | A universal scalar AI risk, control, or maturity score as the reference that determines authorization | Excluded | A score may support analysis, but it cannot replace scoped authority, guarantee, evidence, viability, or reassessment semantics. The material decisions remain distributed across the **retained `SR-M*` relationship set through `SR-M26`**; IDs are intentionally not renumbered when a prior row is subsumed. |
| `SR-X07` | A standing governance committee as the mandatory Organizational Controller | Excluded | Organizational structure is not the relationship. Legitimate authority may be one person, several functions, or an existing process; the relevant authority/basis semantics are carried by `SR-M01`, `SR-M17`, and `SR-M19`. |
| `SR-X08` | Every implementation component/version as its own material relationship | Excluded | Individual implementation detail is not independently material unless changing it alters authority, guarantee, decision, evidence fitness, viability, or another retained relationship. Material active-baseline identity and correlation are carried by `SR-M15`. |
| `SR-X09` | Project Constraint inheritance as a separate artifact-level relationship | Excluded as a separate row | Inheritance is material, but its semantics are already preserved through the Project Authorization/operating-contract handoff in `SR-M03`–`SR-M04` and through the retained specific realized/semantic boundary rows `SR-M05`–`SR-M07`, `SR-M22`, and `SR-M23`. If a material inherited Constraint cannot be traced through those relationships, the map is incomplete and must be versioned rather than creating a ceremonial inheritance row. |
| `SR-X10` | A universal standalone exception-authority relationship | Excluded under the current baseline | This baseline assumes no free-standing override that can bypass the retained authority boundaries. Reserved Human Authority and delegated autonomous eligibility are carried by `SR-M07`/`SR-M09`/`SR-M22`; any exception that changes `OBA-SUP-01 v1` coverage or Project scope routes through `SR-M01`, `SR-M17`, and `SR-M19`. A future explicit exception regime would require a new materiality decision. |
| `SR-X11` | Negative-case learning/stabilization as an independent control relationship | Excluded as an independent row for this carrier/substitution baseline | The paper treats cross-level learning/stabilization as a proposed operating discipline under validation. The material semantics needed to act on a negative case—evidence fitness, bounded correction, verification, fallback, viability/economics, and routing by invalidated decision basis—are already carried by `SR-M10`–`SR-M17`. If a later validation program tests stabilization as its own causal mechanism, that research object must be assessed separately rather than assumed from this map. |

| `SR-X12` | A separate customer-communication/refund-recommendation policy-consistency relationship formerly carried as `SR-M08` | **Subsumed in v6; no longer a separate material row** | The narrower policy-consistency semantics are already contained by `SR-M23`, which covers policy mapping, remedy selection/parameters, factual grounding, and consequential communication with the broader and correct validity/reassessment basis. Keeping both rows would duplicate one semantic obligation with divergent lifecycle rules. `SR-M08` remains a historical v1–v5 identifier and is not renumbered/reused. |
The exclusion inventory is not a closed universe. Article §6 must still reverse-map native concepts from serious substitutes. A newly observed candidate is first added to the considered set with provenance and an independent materiality decision; material or unresolved discoveries require a new mapping version and full substitution rerun.

**v12 completeness note.** The rows above are the current material set for the canonical editorial case under the stated baseline and assumptions. Their inclusion is justified by the pre-carrier materiality rule, not by whether they fit neatly into one capability-family or decision-horizon cell. Article §6 reverse mapping may reveal an absent native relationship, a different but equivalent authority structure, or a loss introduced by this partition. Such evidence must be handled by the reverse-mapping closure rule below rather than forced into `v12` after a verdict has already been chosen.
...
**Accepted drafting decisions from the 2026-08-16 Article §5 first-draft iteration:**

- Open from the practical risk of reifying the complete map into bureaucracy; do not reteach the four horizons or capability families.
...
- Keep the lower-authority sibling explicitly separate from the canonical trace and category-retest it positively because the system's recommendation/draft is a Consequential Runtime Responsibility that materially influences the downstream human decision and itself depends partly on Model Judgment; human execution reduces reachable authority but is not the category criterion.
- Hold lifecycle maturity constant for the authority/scope comparison at the Project / Architecture candidate-design point; treat the canonical system's later Organizational Business Authorization, production-capable Project Authorization, Delivery-approved Requirement/Operating Envelope, realizations, Release decision, and Runtime carriers only as a later maturity callback.
- Describe carriers as a many-to-many composition. Distinguish records from operating mechanisms and preserve owner/authority, operating contract, source/scope/assumptions/version, evidence/consumer, decision/action, post-action verification, and reassessment semantics end to end.
- Establish `support-resolution-canonical-material-map / v12` as the blueprint-owned canonical working map for the Article §6 handoff. Keep it explicitly unfrozen until a substitution run records an immutable mapping-content digest or intentionally preserved repository snapshot.
- The v12 baseline additionally closes the remaining authority/freshness edges: one shared authoritative financial ledger enforces both the €50 automated sublimit and the €500 total in-object per-order ceiling, protective readiness is an explicit material relationship consumed by ordinary execution gates, reconstructability includes protective state, financial reservations, and Human read entitlement, and the controlled object stops at a neutral Finance-review-required state rather than emitting a Finance instruction.
- Keep the manuscript mapping excerpt representative only; it must not be used as the completeness universe for a whole-controlled-object substitution verdict.
- Preserve reverse mapping as a real falsification path: a serious substitute may reveal a material relationship missing from the derived map, forcing a mapping-version increment and full retest rather than a convenient force-fit.
...
Use current paper-level terms: Thinking System, Explicitly Authored Software, Model Judgment, Judgment Node, Constraint, Constraint Realization, Hard Constraint, Soft Constraint, Sensor, Controller, Actuator, Human Authority, Project Constraint Architecture, Constraint Realization Map, Nested Control Lifecycle, Project Authorization, Project Reauthorization, DoR, DoD, and Release Gate. **Project viability conclusion**, **initial assessment eligibility**, **specific Bounded Research Authorization**, **Organizational Business Authorization**, **research-only Project Authorization**, **production-capable Project Authorization**, and **Architectural Veto** are paper-level lifecycle-refinement terms/distinctions until separately accepted into status-bearing UA sources.

**Explicitly Authored Software** is the publication-facing research label introduced through the 2026-08-24 external-review reconciliation because `Linear Software` repeatedly invited the wrong topology reading. The draft-normative glossary continues to use **Linear Software**. No repository-wide normative rename is implied; any canonical change requires separate deliberate framework review and coupled glossary/doctrine updates.

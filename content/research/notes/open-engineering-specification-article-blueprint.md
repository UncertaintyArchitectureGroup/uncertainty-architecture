...
- **Section 5.3** — take one stable refund-authority boundary and expose the four capability families as parallel control functions around that boundary: Constraint + Constraint Realization, Sensors, Controller / Human Authority, and Actuators. Show why a policy sentence is not yet a complete control path without implying an execution pipeline.

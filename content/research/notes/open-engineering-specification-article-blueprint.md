...
→ Organization first supplies admissibility, authoritative boundaries, shared capabilities, reserved authority, and business intent for bounded Project / Architecture research
→ Project / Architecture owns the AI-necessity analysis, concrete bounded control architecture, technical feasibility, Human Authority/fallback/capacity analysis, and complete control economics, and returns a Project viability conclusion rather than silently converting that analysis into the organization's final business launch decision
...
→ when a credible control architecture exists but economics, price, capacity, latency, Human Authority burden, or investment assumptions make the business case unattractive, the result is an organizational business decision rather than an architectural impossibility
→ Organization owns the final launch / continuation / reshape / bounded-research / no-launch decision after receiving the Project viability conclusion; it may change price, target segment, service promise, scope, business outcome, funding, or investment assumptions and return the changed basis to Project / Architecture
→ after a positive organizational launch, continuation, or bounded-research decision on a technically viable basis, Project / Architecture issues or reissues the versioned Project Authorization technical baseline consumed by Delivery
...
- **Article §4** — establish the four horizons and make the Organization ↔ Project / Architecture handshake explicit. Organization first establishes admissibility, business intent, reserved authority, shared capabilities, and evidence obligations. Project / Architecture then performs AI-necessity, control-architecture, technical-feasibility, Human-Authority/fallback/capacity, and control-economics analysis and returns a viability conclusion. An Architectural Veto blocks launch of the unchanged proposal because no credible bounded control architecture exists; economic non-viability instead returns to Organization because business levers may change. Organization owns the launch / continuation / reshape / bounded-research / no-launch decision. After a positive Organizational decision on a viable basis, Project / Architecture issues the versioned Project Authorization technical baseline consumed by Delivery. Runtime and Delivery evidence later route by the decision basis invalidated.
- **Article §4 figure sequence** — first show the four decision-ownership horizons as a standalone model, including the Organization ↔ Project / Architecture viability/business-authorization handshake, downward inheritance into Delivery after Project Authorization, and direct reassessment from realization or operation evidence to the horizon whose decision basis is invalidated. Do not imply that reassessment originates only at Runtime. Only then combine that model with the **Article §3** capability anatomy to produce the orthogonal horizons × capability-families map. Preserve the pedagogical order: capability model → decision-ownership model → combined model.
...
Project / Architecture owns the analysis and the resulting **viability conclusion**, not the organization's final launch decision. Representative conclusions include: Model Judgment is unnecessary for the stated outcome; technically viable as proposed; technically viable only with narrower scope or different authority; further bounded research is needed; technically controllable but economically unattractive under current assumptions; or **Architectural Veto** because no credible bounded control architecture exists for the current proposal. The first five conclusions become inputs to an Organizational business decision. Architectural Veto blocks launch of the unchanged proposal; Organization can change the outcome, scope, authority, service model, price, investment basis, or other assumptions and return the revised proposal for Project reassessment.

Do not predetermine that variant C wins. A legitimate Project conclusion may prefer variant A or B, narrower variant C, require bounded research/redesign, identify economic non-viability, or issue Architectural Veto. Organization then decides whether to launch, reshape, fund further research, defer, or stop the initiative. After a positive Organizational launch/continuation/bounded-research decision on a viable basis, Project / Architecture issues the technical Project Authorization baseline for Delivery.
...
→ Project Reauthorization if the resulting organizational decision changes the authorized technical basis
```
...
- Organization owns initial admissibility/business intent and the final launch/continuation/reshape/no-launch decision after receiving the Project viability conclusion; it also owns reserved authority, shared-capability/evidence obligations, and business levers such as price, service promise, target scope, and investment assumptions.
- Project / Architecture compares deterministic/manual/model-assisted/autonomous alternatives, decides whether Model Judgment is needed for the stated outcome, places Judgment, designs the complete control architecture, characterizes technical/control feasibility and complete economics, and returns a viability conclusion. Architectural Veto blocks the unchanged proposal; economic non-viability routes to Organization rather than being treated as technical impossibility.
- After a positive Organizational launch/continuation/bounded-research decision on a technically viable basis, Project / Architecture issues the versioned Project Authorization technical baseline consumed by Delivery.
...
- Use Case A for Delivery reassessment, Case B for Project viability reassessment → Organizational business review → Project Reauthorization where required, and Case C for direct Organizational review after an authoritative-basis change.
...
- [ ] Organization does not absorb the Project-owned AI-necessity, control-architecture, or control-economics analysis; Project / Architecture returns a viability conclusion, while Organization owns the final launch/continuation/reshape/bounded-research/no-launch business decision and may change business inputs before sending the proposal back for reassessment.
- [ ] Architectural Veto is a Project / Architecture conclusion that blocks launch of the unchanged proposal when no credible bounded control architecture exists; Organization may change the proposal and request reassessment but may not waive the veto on the same decision basis.
- [ ] A technically credible architecture with unattractive economics routes to Organization for a business decision rather than being mislabeled as architectural impossibility; a positive Organizational business decision on a viable basis precedes issuance/reissuance of the technical Project Authorization baseline consumed by Delivery.
...
- [ ] Case B routes through Project viability reassessment to Organizational business/continuation review because economics or the business basis is invalidated; Project Reauthorization follows only for the resulting technically viable authorized basis.
...
10. why Project viability conclusion, Organizational launch/business authorization, Project Authorization, DoR, DoD, Release Gate, runtime correction, and Project Reauthorization are different decisions;
...
**Core claim:** Different control decisions require different evidence, authority, time horizons, automation, and corrective actions. Together, the four levels describe how one Thinking System moves from organizational admissibility and business intent through Project / Architecture viability research, an Organizational launch/business authorization decision, the technical Project Authorization baseline, Delivery realization/release, Runtime operation, correction, and reassessment. They are not four documents or four meetings; they are connected decision horizons in one socio-technical control system, and Organization and Project / Architecture may interact more than once before Delivery begins.

**Lifecycle-refinement status.** Current authority-bearing UA sources already separate Organization, Project / Architecture, Delivery, and Runtime and already place AI necessity, control-architecture feasibility, Human Authority/capacity, and control economics at Project level. This blueprint makes an additional decision handshake explicit for the paper: **Project / Architecture owns the viability conclusion; Organization owns the final business launch/continuation decision; Project Authorization is the versioned technical baseline issued or reissued after a positive Organizational launch/continuation/bounded-research decision on a viable basis.** Architectural Veto is a Project conclusion that cannot be overridden for the unchanged proposal; economic non-viability is an input to an Organizational business decision because Organization may change business levers and return a changed basis for reassessment. Until this exact sequencing is separately reconciled into the authority-bearing lifecycle/pattern sources through framework review, describe it as a paper-level lifecycle refinement under validation rather than silently claiming that the current normative wording already says the same thing.
...
- Make explicit that four horizons do **not** imply one pass through four boxes. The Organization ↔ Project / Architecture handshake may iterate before launch and during continuation/reassessment: Organization supplies admissibility/business basis; Project returns viability evidence; Organization makes the business authorization decision; Project then issues the technical Project Authorization baseline for Delivery.
...
Show two adjacent views. The **decision-ownership side must present the canonical four-horizon model defined in this section**. Preserve the four horizons while showing the Organization ↔ Project / Architecture handshake explicitly instead of implying a single one-way `Organization → Project → Delivery` gate sequence. Keep the same decision-ownership model consistent between the standalone four-horizon Figure 8 and Figure 9 rather than creating a second competing representation.

```text
Decision ownership — canonical four-horizon model

Organization — What is admissible, and should the organization launch, continue, reshape, fund, or stop the initiative?
   │ initial admissibility · authoritative sources · reserved authority · business intent
   ↓
Project / Architecture — Is Model Judgment needed, and does a credible, operable control architecture exist? What is the viability conclusion?
   │ viability conclusion: AI necessity · technical/control feasibility · control economics · alternatives · Architectural Veto if applicable
   ↑
Organization — launch / continue / reshape / bounded research / no-launch
   │ positive decision on a technically viable basis
   ↓
Project / Architecture — issue or reissue the versioned Project Authorization technical baseline
   ↓
...
Reassessment evidence → Project / Architecture: architecture / feasibility / evidence / Human Authority / fallback / capacity / economics / AI-necessity basis challenged
Project viability reassessment → Organization: business case / launch-continuation basis challenged, Architectural Veto, or changed business/authority input required
Reassessment evidence → Organization directly: authoritative source / reserved decision right / shared capability changed
...
The repeated Organization and Project / Architecture labels above are repeated **decisions at the same two horizons**, not additional lifecycle levels. The diagram must make the handshake legible without turning it into a mandatory sequence of separate meetings or implying that every local Project Reauthorization requires a new Organizational decision. A Project change that remains technically viable and inside the standing Organizational business/authority basis may be reauthorized at Project level; Organization is reactivated when the business launch/continuation basis, authoritative boundary, reserved authority, or another organizationally owned decision is implicated.
...
**Primary question owned:** Within which authoritative boundaries, shared capabilities, and decision rights may Project / Architecture research proceed—and, after the Project viability conclusion, should the organization launch, continue, reshape, fund, or stop the initiative?
...
2. **business launch / continuation authorization** — after Project / Architecture returns the AI-necessity, technical/control-viability, control-economics, and alternative-design conclusion, whether the organization should launch, continue, reshape, fund further research, defer, or stop the initiative.
...
- a Project viability conclusion requiring a launch, continuation, reshape, further-research, or no-launch decision;
...
- the Project / Architecture viability conclusion, including AI-necessity analysis, alternative designs, complete bounded control-architecture feasibility, control economics/capacity, residual exposure/uncertainty, required business assumptions, and any Architectural Veto;
...
- After receiving the Project viability conclusion, should the organization launch or continue the initiative on that basis, authorize only bounded research, reshape the business case, or stop/defer it?
...
- If Project issues Architectural Veto, what part of the business or authorization basis must change before reassessment? Organization may not approve launch of the unchanged proposal by simply accepting the technical impossibility as residual risk.
...
- Human authority will usually dominate consequential organizational decisions, but sensing and decision support may be automated where useful and credible: source/version tracking, policy checks, evidence aggregation, threshold detection, notification, routing, dependency-health signals, and preparation of exception, viability, continuation, or reauthorization context.
- Automation may help apply explicit delegated rules, but it must not invent authority, change a prohibited-use boundary, waive an Architectural Veto for an unchanged proposal, or silently accept residual exposure outside delegation.
...
- authorize bounded research rather than launch;
...
- after Project viability analysis, an explicit business decision: launch/continue on the viable basis, bounded research, reshape/change assumptions and return to Project, defer, or no-launch;
...
- where launch/continuation/bounded research is approved on a technically viable basis, the Organizational business authorization that allows Project / Architecture to issue or reissue the versioned technical Project Authorization baseline for Delivery.
...
- Project viability conclusion covering AI necessity, alternatives, technical/control feasibility, Human Authority/fallback/capacity, residual exposure, and complete control economics;
...
- launch/business decisions repeatedly ignored or obscured Project viability evidence;
...
authoritative sources + shared capabilities + reserved authority + business intent ───────────┼→ Organizational Controller / legitimate decision owners
                                                                                               │
Project viability conclusion / Runtime evidence / authority-change request ────────────────────┘
                                                                                               ↓
                                                                                  Organizational decision
                                             admissibility · launch/continue · bounded research · reshape · no-launch
                                                                                               ↓
                                                                                  Organizational Actuators:
...
**Primary question owned:** Is Model Judgment needed for the stated outcome, does a credible complete bounded control architecture exist, and what is the resulting technical/operational/economic **Project viability conclusion** to return to Organization?

This is the analytical/architectural horizon in which organizational admissibility becomes a concrete viability conclusion for the proposed controlled system. A successful prototype is not evidence of final launch authorization. **Project / Architecture owns the AI-necessity analysis, project-level outcome formulation, control architecture, technical/control feasibility, Human Authority/fallback/capacity analysis, complete control economics, and the viability conclusion. Organization owns the business launch/continuation decision. Project Authorization is the technical baseline Project issues only after that Organizational decision is positive for a technically viable basis.**
...
- a positive Organizational launch/continuation/bounded-research decision requiring Project to formalize the technical Project Authorization baseline;
...
- delivery or runtime evidence that invalidates project risk, authority, feasibility, evidence, Human Authority capacity, control latency, shared-capability assumptions, AI necessity, or economics;
...
- where this is a reassessment, the current Organizational business authorization and Project Authorization baseline.
...
- Does a credible complete bounded control architecture exist at all for the proposed scope, authority, consequence profile, and assumptions—or must the Project issue **Architectural Veto**?
- If the architecture is technically credible, what are the complete solution + control-perimeter economics, capacity, latency, Human Authority, and operational burdens under the stated business assumptions?
...
- After a positive Organizational launch/continuation/bounded-research decision on a viable basis, what exact technical scope and baseline may be formalized as Project Authorization for Delivery?
- Which assumptions or evidence changes require Project viability reassessment, Project Reauthorization, or Organizational business review?
...
→ Organizational launch / continuation / reshape / bounded-research / no-launch decision
```
...
The distinction between **Architectural Veto** and **economic non-viability** is mandatory:

- **Architectural Veto:** no credible complete bounded control architecture exists for the current scope/authority/consequence/assumption set. Project cannot issue Project Authorization for that unchanged proposal. Organization may change the proposal and request a new assessment, but may not simply accept the missing control path as a business trade-off.
- **Economic non-viability:** a credible control architecture exists, but the current business assumptions do not justify its total solution/control burden. This conclusion must go to Organization because Organization may change price, service promise, target segment, scope, outcome, funding, investment horizon, Human Authority capacity, or decide not to launch. Any changed technical basis returns to Project for reassessment before Project Authorization.
...
- viability status: viable / viable with conditions or narrower scope / further research / technically viable but economically unattractive / Model Judgment unnecessary or simpler alternative preferred / Architectural Veto;
- the assumptions whose change would require reassessment.

**B. Project Authorization technical baseline flowing to Delivery after a positive Organizational decision on a viable basis:**

- Project Authorization status and exact authorized technical scope;
- the current Project Constraint Architecture and assumptions;
...
The repository's **Project Control Architecture and Viability Review** is one canonical pattern that can carry much of this baseline. The article should not imply that every implementation must use that exact form; existing architecture, product, risk, business-case, or decision records may carry the same decisions when ownership, versioning, and evidence remain explicit.
...
- evidence that the stated business/value assumptions or AI-necessity rationale are no longer true.

**Local action versus escalation:**

Project / Architecture may analyze, redesign, narrow, request more research, compare alternatives, and issue the viability conclusion. It may issue or reissue Project Authorization only when the proposal is technically viable **and** a standing positive Organizational launch/continuation/bounded-research decision covers that basis.

- If a changed technical design remains viable and inside the standing Organizational business/authority basis, Project may perform Project Reauthorization without a new Organizational business decision.
- If evidence shows that no credible bounded control architecture exists for the current proposal, Project issues Architectural Veto and routes the result to Organization. No Project Authorization exists for that unchanged proposal.
- If the architecture remains credible but economics, price/value assumptions, Human Authority/capacity burden, AI-necessity rationale, or business outcome no longer close, Project returns the viability conclusion to Organization for a launch/continuation/reshape/no-launch decision.
...
- If Organization changes the basis, Project reassesses before issuing a new Project Authorization.

**Learning and stabilization:**

Negative cases should test whether the project model of the controlled object was wrong: missing scenario, incorrect assumption, insufficient Constraint, non-credible Hard claim, poor Sensor coverage or validity, wrong Controller authority, ineffective Actuator, unrealistic Human Authority capacity, fallback/common-mode failure, automation failure, non-reconstructable active baseline, invalid AI-necessity rationale, or invalid economics. Repeated delivery/runtime workarounds are evidence that the viability conclusion or Project Authorization may need revision rather than more local tuning. This systematic negative-case learning loop is a publication-facing proposal under validation; the article must distinguish it from the separate Project-viability / Organizational-business-authorization lifecycle refinement.

**Supporting figure — Figure 11: Project control architecture, viability conclusion, and authorization handshake:** Show:
...
→ separate required operating-contract properties + evidence obligations
...
+ residual exposure / uncertainty
→ Project viability conclusion:
   viable | narrower design | research | economic non-viability | simpler/non-LLM alternative | Architectural Veto
→ Organization:
   launch / continue | reshape business basis | bounded research | defer / no-launch
   ├→ changed basis → Project / Architecture reassessment
   └→ positive decision on viable basis → Project Authorization technical baseline → Delivery

Delivery / Runtime invalidating evidence
→ Project viability reassessment
   ├→ still viable inside standing Organizational basis → Project Reauthorization
   └→ business basis challenged / Architectural Veto / wider authority required → Organizational review
```

The figure must not place the required operating-contract properties inside the Project Constraint Architecture. They are a separate Project output that Delivery later translates into its approved Requirement and Operating Envelope. The figure must also keep **Project viability conclusion**, **Organizational business authorization**, and **Project Authorization** visually distinct; “Architectural Veto” is a Project conclusion, while “no-launch” is an Organizational business decision.
...
- Project Authorization of a bounded implementation or experiment following the applicable Organizational business decision;
...
- Can material release, incident, and correction evidence be correlated to the active authoritative-source, Organizational-business, Project, Delivery, realization, model/configuration, evaluator, deployment, and fallback baseline rather than to an ambiguous list of independent versions?
...
- Does any new evidence invalidate Project Authorization, the Project viability conclusion, or the Organizational business basis rather than only a local realization?
...
- active model, prompt/instruction, context/retrieval, tool/routing, policy, evaluator, realization, deployment-scope, and fallback versions where material, correlated with relevant source/Organizational-business/Project/Delivery baselines enough to reconstruct the material active behavioral/control state;
...
Delivery may repair, reconfigure, roll back, narrow exposure, disable, or re-release within delegated authority. It may not silently expand project authority, weaken an inherited Hard Constraint, change an organizational prohibition, or normalize evidence that project viability has failed. Delivery evidence that challenges the technical Project Authorization routes to Project viability reassessment / Project Reauthorization. If Project concludes that the architecture is no longer viable, that economics/AI necessity/business assumptions are invalid, that Architectural Veto now applies, or that a changed Organizational boundary/exception/business decision is required, Project routes the conclusion to Organization before a new technical baseline can be issued. An exogenous change to the organizational basis may activate Organization directly; the affected Project Authorization must then be reassessed before Delivery proceeds under a new baseline.
...
Definition of Done
 ...
 deployment-specific Release Gate
   ...
   → organizational-boundary / reserved-authority issue discovered through Delivery evidence: Project viability reassessment
runtime evidence
   → local implementation / realization / evidence issue: contain / roll back / narrow / disable → delivery implementation / realization
   → Project Authorization / viability basis invalidated: Project viability reassessment
Project viability reassessment
   → viable inside standing Organizational business/authority basis: Project Reauthorization
   → economics / AI necessity / business basis invalidated, Architectural Veto, or Organizational change required: Organizational review
Organizational review
   → changed business/authority basis or positive continuation decision
   → Project Reauthorization before Delivery proceeds
...
- active Organizational business/authority basis plus authorized Project and Delivery baselines;
...
- enough correlated baseline identity to reconstruct relevant authoritative-source, Organizational-business, Project, Delivery, realization, model/prompt/context/retrieval/tool/routing/evaluator/policy/deployment/fallback state for material evidence;
...
- Has operation returned to a known authorized state, or has the basis of Project viability, Project Authorization, or Organizational launch/continuation authorization been invalidated?
...
- correlated active source/Organizational-business/project/delivery/configuration/version traceability sufficient to reconstruct the material behavioral/control baseline;
...
→ Project viability reassessment
   → if still viable inside standing Organizational business/authority basis: Project Reauthorization

project economics, AI-necessity rationale, business-value assumptions, or viable-scope conclusion changed materially
→ Project viability reassessment
→ Organizational launch / continuation / reshape / no-launch review
→ Project Reauthorization only for the resulting authorized technically viable basis

no credible bounded control architecture remains for the current proposal
→ Project Architectural Veto
→ Organizational review may change the proposal, but may not override the veto on the same basis

authoritative source, reserved decision right, prohibited-use boundary, approved vendor/deployment mode, shared capability, or other Organizational basis changed
→ Organizational review
→ Project Reauthorization if the project remains viable and the technical baseline changes

proposed authority expansion
→ Project viability reassessment
→ Organizational review where the Organizational boundary must change
→ Project Reauthorization for the resulting authorized basis
```

**Local action versus escalation:**

Runtime may reject, contain, compensate, route to fallback, narrow exposure, roll back, disable, or stop within delegated authority. These actions may restore a previously authorized state. They do not authorize redesign, new authority, a new business boundary, or a new launch decision. A fallback is not presumed safe merely because it is configured; failed/common-mode fallback, saturation, or inability to restore an authorized state is evidence for reassessment. Persistent or repeated local recovery is evidence for reassessment when the underlying basis remains invalid.
...
- a project scenario, AI-necessity rationale, or assumption that was wrong;
...
Keep Figure 13's escalation exit generic (`authorization or viability basis invalidated → route by decision ownership`) and let Figure 14 own the concrete Delivery / Project / Organization destinations. Ensure Figure 12 and Figure 14 agree: a Delivery-discovered issue that challenges Project Authorization routes first to Project viability reassessment; if Project can reauthorize inside the standing Organizational basis, it does so. If economics/business basis/AI necessity is invalidated, Architectural Veto applies, wider authority is needed, or an Organizational boundary changed, Organization owns the next business/authority decision before Project issues the resulting technical baseline.
...
**Epistemic-status rule:** Existing authority-bearing UA sources already establish downward inheritance, runtime evidence, local reassessment, Project Reauthorization, organizational review, and capability relationships. This paper proposes two additional pieces of research framing that must remain visibly under validation until separately reconciled: (1) the explicit **Project viability conclusion → Organizational business authorization → Project Authorization** handshake described above; and (2) systematic negative-case analysis and control-improvement feedback across the levels. Do not present either as settled normative UA behavior before framework review accepts the corresponding change.
...
Was the project scenario, AI-necessity rationale, assumption, or economics wrong?
Was the Organizational source, decision right, business assumption, launch basis, or shared capability wrong or changed?
```
...
- reflected in revised Organizational business authorization or Project Authorization where required.
...
- **Organization / Project boundary and handshake:** Organization owns initial admissibility, authoritative boundaries, shared capabilities, reserved decisions, evidence obligations, exceptions, business intent, and the final launch/continuation/reshape/no-launch decision. Project / Architecture owns AI necessity, concrete control architecture, technical/control feasibility, Human Authority/fallback/capacity analysis, complete control economics, Architectural Veto, and the Project viability conclusion. After a positive Organizational business decision on a viable basis, Project issues Project Authorization as the technical baseline for Delivery. Do not collapse the Project viability conclusion into the Organizational business decision or vice versa.
- **Architectural Veto versus economics:** Architectural Veto means the current proposal has no credible complete bounded control architecture and cannot be launched unchanged; Organization may change the proposal and request reassessment but cannot waive the technical impossibility. Economic non-viability means a credible control architecture exists but the current business assumptions do not justify it; that result belongs at the Organizational business-decision surface because price, scope, service level, funding, investment horizon, and other business levers may change.
- **Normative reconciliation:** The current authority-bearing lifecycle/pattern sources do not yet express this exact two-step `Project viability conclusion → Organizational business authorization → Project Authorization` semantics. Until a separate framework decision reconciles them, the article must label this sequencing as a paper-level lifecycle refinement rather than silently presenting it as current normative UA.
...
- **Behavioral/control baseline correlation:** Material evidence, incidents, release decisions, and Actuator actions need enough correlation across authoritative-source, Organizational-business, Project, Delivery, realization, model/prompt/context/retrieval/tool/routing/evaluator/policy/deployment/fallback state to reconstruct what was actually active. The paper must not turn this need into a mandatory universal registry or invent a canonical “Behavioral Configuration Baseline” artifact before application evidence shows one is warranted.
...
- **Organizational Actuator taxonomy:** The paper may show functional examples such as changing permission, exception, vendor approval, shared capability, project eligibility, launch/continuation status, or business assumptions. Do not create a mandatory exhaustive taxonomy unless future doctrine requires one.
...
> Project / Architecture owns the viability conclusion; Organization owns the business decision to launch, continue, reshape, fund further research, or stop; Project Authorization is the technical baseline that follows a positive Organizational decision on a viable basis.

> Architectural Veto blocks the unchanged proposal because the control architecture is not credible; economic non-viability is different because Organization may change business assumptions before Project reassesses.
...
- Make the Organization ↔ Project / Architecture handshake explicit: Organization owns initial admissibility/business intent and final launch/continuation/reshape/no-launch authority; Project / Architecture owns AI necessity, concrete control architecture, complete control economics, Architectural Veto, and the viability conclusion; after a positive Organizational business decision on a viable basis, Project issues the technical Project Authorization baseline for Delivery.
...
- Do not let Organization “override” Project AI-necessity analysis for an unchanged outcome by treating a desire to use AI as evidence; Organization may instead change the desired outcome or business problem and request reassessment.
...
- Preserve enough correlated source/Organizational-business/Project/Delivery/realization/model-config/evaluator/deployment/fallback identity to reconstruct the material active behavioral/control baseline for release, incident, and corrective evidence; do not create a mandatory universal registry by implication.
...
- Treat the exact Project-viability / Organizational-business-authorization / Project-Authorization handshake as a paper-level lifecycle refinement until separately reconciled into the status-bearing framework sources.
...
- the paper-level Project viability conclusion → Organizational launch/continuation/business authorization → Project Authorization handshake, including the distinction between Architectural Veto and economic non-viability and the requirement to reconcile this exact sequencing with status-bearing framework sources before treating it as normative UA;
- authority, decision rights, and Constraints flow downward by reference while scoped realizations become concrete; changed authoritative/business sources and Delivery realization or Runtime operation evidence return to the horizon whose decision basis they affect or invalidate;
...
**Article §5** applies these results. It must not reproduce the four-horizon pressure test, the viability/business-authorization handshake, baseline-correlation argument, fallback-credibility argument, or implementation-depth argument from **Article §4** unless a newly discovered contradiction requires revising the earlier manuscript.
...
```text
Organizational business authorization on a viable basis
...
→ fit-for-decision evidence
→ continue, narrow, redesign, stop, reshape business basis, or reauthorize
...
After recording that positive category result, inspect the counterfactual against all four decision horizons and all four capability families as a distinct candidate controlled object and Project scope before calling it lighter; do not imply that the comparison itself grants Project Authorization or Organizational launch authorization. Publication prose should show the material differences while **holding lifecycle maturity constant for the authority/scope comparison**. Compare proposed operating-contract properties, reachable authority, boundaries, evidence obligations, Human Authority, candidate Actuation, reassessment, dependencies, and economics for both Project / Architecture candidates. Only afterward may the article recall the canonical system's active Organizational launch basis, Project Authorization, Delivery-approved Requirement/Operating Envelope, and Runtime realization as a separate maturity callback.
...
|---|---|---|
| Lifecycle point used for the authority/scope comparison | candidate Project / Architecture design before final Organizational launch/business authorization and Delivery approval | candidate Project / Architecture design before final Organizational launch/business authorization and Delivery approval |
| Intended outcome and proposed operating-contract basis | proposed required properties that a later Project Authorization and Delivery-approved Requirement/Operating Envelope would need to carry if Organization chose to launch this viable candidate | proposed required properties that a later Project Authorization and Delivery-approved Requirement/Operating Envelope would need to carry if Organization chose to launch this viable action-capable candidate |
...
**Lifecycle-maturity callback — not part of the authority-delta test:** After the table, briefly reconnect the canonical action-capable design to the already drafted post-viability state from **Article §4**: Organizational launch authorization, Project Authorization, active Delivery-approved Requirement and Operating Envelope, transaction guard, active evidence instruments, release decision, and runtime carriers are consequences of having matured a viable candidate into authorized operation. They must be presented as the canonical case's **maturity delta**, not as proof that broader authority alone requires “more documents.” A viable lower-authority candidate that Organization chose to launch would likewise acquire Project/Delivery/Runtime carriers appropriate to its own scope.
...
- **controlled object and lifecycle baseline** — exactly which canonical system state is being mapped, including the applicable Organizational business authorization and Project Authorization when the mapping represents an authorized delivered system;
- **source/authorization baseline** — Organizational business/authority decision, Project Authorization, authoritative source references, and other standing decisions used by the mapping;
...
For a standing viability/economic relationship, **Decision / reference semantics** must preserve the accountable viability owner, relevant input categories, legitimate decision right, available outcomes such as Project viability conclusion → Organizational launch/reshape/no-launch decision → Project Authorization where applicable, and the decision basis whose change triggers reassessment. The actual cost, latency, capacity, staffing, Human Authority burden, maintenance burden, operational friction, and failure surface remain separate observations for the later practical-preference comparison.

The manuscript may render only three or four representative rows—including the Organizational launch/business authorization or Project Authorization, the active Requirement/Operating Envelope, the deterministic transaction boundary, the Soft semantic communication/policy expectation, and Human Authority—provided that it labels them illustrative and does not imply that the visible sample is the complete analysis. Render the publication-facing sample as a table, a clearly separated block, or a replacement figure if that is clearer; do not require both a mapping table and a mapping figure.
...
11. **Viability-decision semantics and control economics** — does it preserve who owns the technical/control viability conclusion, which cost/capacity/latency/burden categories inform it, which organization-level business decision consumes that conclusion, which outcomes can follow, and what triggers reassessment? Record actual lifecycle cost, Human Authority burden, fallback cost, false blocks, latency, incidents, maintenance, and operational friction separately for the later practical-preference comparison; numerical burden is not itself the semantic-equivalence test.
...
- applicable law and binding contractual obligations may directly create authoritative source obligations within scope; Organization identifies applicable authoritative sources and legitimately adopts internal or voluntary sources where appropriate, Project interprets them into scoped project Constraints, viability conclusions, required operating-contract properties, assumptions, authority, and evidence obligations, Organization makes the business launch/continuation decision on the viable basis, Project issues the technical Project Authorization, and Delivery translates the inherited baseline into an approved Requirement and Operating Envelope for the bounded scope;
...
+ reassessment route
+ viability-decision semantics: Project viability owner/conclusion, Organizational business-decision consumer/options, Project Authorization handoff where applicable, and reassessment trigger
```

The final viability-decision item applies only where economics/viability is material to the relationship. It preserves the meaning of the viability/business-authorization decision chain, not the numerical burden of one implementation. Actual lifecycle cost, latency, Human Authority load, capacity demand, maintenance burden, operational friction, and failure surface are recorded separately and compared only during **practical preference**.
...
For a material **economics/viability relationship**, semantic equivalence concerns the decision semantics: Project / Architecture must own the technical/control viability analysis and conclusion; the relevant cost/capacity/latency/burden input categories must be available; Organization or an equivalent legitimate business authority must consume that conclusion and own launch/continuation/reshape/no-launch choices; and the technical authorization handoff plus reassessment trigger must remain explicit where applicable. A substitute may therefore be semantically equivalent even when its actual costs or operating burden are higher. Those measured or estimated values belong to the practical-preference comparison below.
...
- **Viability/launch-authority nuance:** the paper's explicit Project-viability → Organizational-business-authorization → Project-Authorization handshake is a research refinement until framework reconciliation. Landscape comparison must allow an equivalent existing method/composition to distribute these decisions differently if legitimate analytical and business authority, veto semantics, technical authorization, and reassessment are preserved.
- **Guarantee nuance:** map the complete realized path; never label an entire product Hard or Soft.
- **Economics semantics:** preserve the semantics of a material viability/business decision separately from the numerical control burden. Different cost/latency/staffing values affect practical preference and Organizational launch decisions; they do not by themselves create or close a semantic gap.
...
- **Duplication risk:** do not reteach the capability anatomy, horizons, Human Authority, Hard/Soft, the Organization↔Project viability/business-authorization handshake, baseline reconstructability, fallback credibility, or reassessment semantics already established in **Article §§3–4**.
...
- the four decision horizons, the paper-level Project-viability / Organizational-business-authorization / Project-Authorization handshake, and invalidated-decision-basis routing;
...
- an explicit paper-level handshake in which Project / Architecture owns AI necessity and the technical/control/economic viability conclusion, Organization owns the launch/continuation/business decision and cannot override Architectural Veto for the unchanged proposal, and Project Authorization becomes the technical baseline issued after a positive Organizational decision on a viable basis;
...
- whether the explicit Project viability conclusion → Organizational launch/business authorization → Project Authorization handshake improves ownership clarity and should be accepted into status-bearing lifecycle/pattern sources;
...
- the Project-viability / Organizational-business-authorization / Project-Authorization handshake and Architectural-Veto/economic-non-viability distinction are paper-level lifecycle refinements not yet reconciled into the current authority-bearing lifecycle/pattern sources;
...
1. **Specification boundary and indexed engineering surfaces** — `SPECIFICATION.md` defines/indexes the specification boundary and conformance/document-status model; the definitions, controlled object, capability anatomy, nested lifecycle, Requirements/Constraints/Correctness, Human Authority, patterns, capabilities, Control Plane guidance, and failure-mode taxonomy that it indexes are interpreted only according to each document's declared status and scope. The exact Project-viability / Organizational-business-authorization / Project-Authorization handshake remains paper-level research until framework reconciliation changes those status-bearing surfaces.
...
+ Project viability conclusion ↔ Organizational business authorization ↔ Project Authorization handshake
...
- the Project-viability / Organizational-business-authorization / Project-Authorization handshake is one such paper-only hypothesis until accepted;
...
> `SPECIFICATION.md` defines and indexes the current UA specification boundary; this paper's broader composition—including its explicit Project-viability / Organizational-business-authorization lifecycle refinement—remains a testable research synthesis, not proof that every boundary, partition, artifact, operating hypothesis, or claimed landscape gap is already right.
...
- validation of the Project-viability / Organizational-business-authorization lifecycle refinement;
...
| **3. Decision ownership, guarantee, evidence, authority, and reassessment semantics** | the Project viability conclusion → Organizational launch/business authorization → Project Authorization handshake, Architectural-Veto/economic-non-viability distinction, complete-path Hard/Soft claims, evidence-instrument validity, active-baseline reconstructability, fallback/recovery credibility, substantive Human Authority, Controller/Actuator separation, and invalidated-decision-basis routing improve clarity and operability | teams cannot apply the viability/business-authorization ownership split consistently; Organizational decision makers routinely need to own analysis that the model assigns to Project, or Project can credibly own final launch authority without loss; Architectural Veto versus economic non-viability proves non-actionable or systematically ambiguous; material incidents cannot be reconstructed against the actual active source/Organizational-business/Project/configuration baseline; fallback shares primary/common dependencies, lacks capacity, or cannot restore an authorized state; evidence cannot be routed to a credible owner; automation/Human Authority boundaries or reassessment ownership create systematic ambiguity or excessive escalation | route each failed subclaim to its owning research/doctrine/pattern decision; revise the lifecycle refinement, semantics, evidence lifecycle, routing, authority, or ownership only through the applicable explicit framework decision and status-bearing change |
| **4. Proportionality, carrier sufficiency, usability, and economics** | independent teams can inspect the full map, apply the pre-substitution materiality rule, select the lightest credible carriers, preserve the approved operating contract and other material relationships, and estimate enough control-perimeter cost to make earlier Project viability and Organizational launch decisions | low-consequence systems are routinely overbuilt, materiality cannot be judged consistently before seeing a carrier/substitute, material relationships disappear into “existing tools,” authority-delta comparisons cannot be separated from lifecycle maturity, or cost/Human Authority/capacity cannot be assessed early enough to affect Project viability or Organizational business decisions | revise the paper's application method and evidence record; propose pattern/specification changes separately if the result warrants them |
...
- traces of pre-launch Project viability analysis including AI-necessity, alternative-design, control-architecture, Human Authority/fallback/capacity, and full control-economics conclusions;
- Organizational launch/continuation decisions showing how Project viability evidence affected price, scope, business model, funding, service promise, bounded research, defer, or no-launch decisions;
...
- cases where a technically credible design was economically unattractive and Organization changed a business lever successfully or decided not to launch;
...
- release/runtime/incident records showing whether material source, Organizational-business, Project, Delivery, realization, model/prompt-instruction/context-retrieval/tool-routing/evaluator-policy/deployment/fallback state could be correlated well enough to reconstruct the active behavioral/control baseline without relying on a new mandatory registry;
...
- independent duplicate substitution assessments against the same mapping ID/version/immutable snapshot, including disagreements about scope alignment, equivalence, stronger semantics, viability/business-authorization semantics, or non-comparability;
...
> The support-resolution example is editorially constructed. It demonstrates continuity of explanation; it cannot validate the category, operating map, Project-viability / Organizational-business-authorization lifecycle refinement, proportionality method, landscape gap, or Uncertainty Architecture.

Use the case only to formulate transfer questions:

- Can another team distinguish Project-owned AI-necessity/control-viability/economics analysis from Organization-owned launch/business authority without creating duplicated bureaucracy?
...
2. **Architectural anchor B — Figure 9: Two orthogonal models** — already established in **Article §4**: the decision side reproduces the four-horizon decision model, including the Organization ↔ Project / Architecture viability/business-authorization handshake, reassessment evidence from Delivery realization or Runtime operation, and the same direct/conditional return routes, while a visually distinct capability-family side shows Actuators, Constraints and realizations, Sensors and evidence, and Controllers and decision authority as functions that may appear at every horizon. Ordering is a reading aid, not an execution pipeline or one-to-one mapping.
...
- Figure 8 — one controlled object across four decision horizons, including the Organization ↔ Project viability/business-authorization handshake and Delivery-realization / Runtime-operation reassessment evidence;
- Figure 9 — the two orthogonal models;
- Figure 10 — organizational control process;
- Figure 11 — project control architecture, viability conclusion, and authorization handshake;
...
- distinguish external authority sources, business authorization, technical viability/authorization, implementation surfaces, evidence, decision ownership, action, and revision where material;
...
- conflate **Project viability conclusion**, **Organizational launch/business authorization**, and **Project Authorization**; the paper intentionally separates technical/control/economic analysis, business authority to pursue the initiative, and the technical baseline consumed by Delivery;
...
- present the new viability/business-authorization handshake as current normative UA before the relevant status-bearing lifecycle/pattern sources are explicitly reconciled;
...
- [ ] The **Article §5 authority/scope comparison holds lifecycle maturity constant** across both candidates and explicitly separates scope/authority delta from later Organizational-launch/Project-Authorization/Delivery/Runtime maturity delta.
...
- [ ] Figure 8 keeps Organization, Project / Architecture, Delivery, and Runtime as four decision horizons while showing the **Organization ↔ Project / Architecture handshake**: initial Organizational admissibility/business context → Project viability conclusion → Organizational launch/continuation/reshape/no-launch decision → Project Authorization technical baseline → Delivery. Repeated Organization/Project decisions do not create extra horizons.
- [ ] Figure 9 reproduces the Figure 8 four-horizon decision model verbatim—including the Project-viability/Organizational-business-authorization handshake, reassessment evidence from Delivery realization or Runtime operation, and the same return routes—and adds the capability-family dimension as a visually distinct green group with undirected structural grouping and no implied one-to-one mapping or execution pipeline.
...
- [ ] **Article §4** treats all four levels as operating processes through time, not only static ownership descriptions, and explicitly permits Organization ↔ Project iteration before launch and during reassessment.
...
- [ ] Organizational control owns initial admissibility, authoritative boundaries, shared capabilities, reserved/delegated decision rights, evidence obligations, exceptions, business assumptions, and the final launch/continuation/reshape/no-launch business decision after Project viability analysis; it does **not** absorb the Project-owned AI-necessity, control-architecture, technical-feasibility, or complete control-economics analysis.
- [ ] Project / Architecture owns AI necessity, alternative comparison, concrete bounded control architecture, complete control economics, Architectural Veto, and the viability conclusion. It issues Project Authorization only after a positive Organizational launch/continuation/bounded-research decision on a technically viable basis.
...
- [ ] Organizational evidence includes both Project viability/Runtime evidence and exogenous/cross-project evidence such as legal, contractual, audit, vendor, business-assumption, and shared-capability change.
- [ ] Figure 10 shows external/organizational evidence, authoritative/business reference sources, and Project/Runtime evidence/authority requests as converging inputs to the Organizational Controller rather than a sequential pipeline.
- [ ] Project viability conclusions, Architectural Veto, economics findings, and Runtime invalidation are represented as examples inside the lower-level evidence lane in Figure 10, not duplicated as additional parallel arrows.
- [ ] Figure 10 shows a real control relationship including Organizational Actuators such as launch/continue/reshape/no-launch and business-assumption changes, not only a list of participating departments or an approval box.
...
- [ ] Project / Architecture explicitly owns project-level outcome formulation, AI necessity, project boundary, intended Judgment landscape, Project Constraint Architecture, control viability, complete control economics, and the Project viability conclusion for the specific system; it receives a hypothesis rather than presupposed proof of Model Judgment value and allows viable/narrower/research/economic-non-viability/AI-unnecessary/Architectural-Veto conclusions without claiming final business launch authority or Delivery's implementation-level Judgment Nodes / approved Requirement and Operating Envelope.
...
- [ ] Material evidence can be correlated to the active source/Organizational-business/Project/Delivery/realization/model-config/evaluator/deployment/fallback baseline sufficiently to reconstruct what governed a material release, incident, or Actuator decision; no universal registry is implied.
...
- [ ] Delivery clearly owns implementation-level Judgment Nodes and the approved Requirement and Operating Envelope for the bounded scope and distinguishes them from the inherited Organizational business basis, project boundary, intended Judgment landscape, Project Authorization, project Constraints, concrete realization, DoR, DoD, Release Gate, local repair, Project viability reassessment, and Project Reauthorization.
- [ ] Figure 12 distinguishes Release Gate **rework**, **terminal stop/defer/reject**, and **Project viability reassessment** for delivery-discovered project/organizational-business issues; it does not bypass Project / Architecture to Organization. Project may reauthorize locally when the basis remains viable inside standing Organizational authorization; Organization is reached when business/authority basis, Architectural Veto, or wider Organizational change is implicated.
...
- [ ] Runtime routes economics/AI-necessity/business-basis invalidation through Project viability reassessment to Organizational business review before a new Project Authorization is issued; direct Organization review remains appropriate for exogenous Organizational-basis change.
...
- [ ] The article clearly distinguishes current status-bearing lifecycle semantics from both the **paper-level Project-viability / Organizational-business-authorization refinement** and the **proposed cross-level negative-case learning/stabilization discipline under validation**.
...
- [ ] **Article §5** revisits implementation depth and carrier choice when authoritative sources, Organizational business authorization, Project Authorization, scope, Project assumptions, Delivery realization evidence, Runtime operation evidence, or evidence-instrument validity change; such evidence may strengthen, narrow, redistribute, consolidate, simplify, recalibrate, or replace parts of the implementation.
...
- [ ] **Article §7** states the proposed contribution as a paper-level synthesis/recomposition, separates the existing specification boundary from the Project-viability/Organizational-business-authorization refinement and other paper-only carrier/substitution/reverse-mapping/integration-gap hypotheses, defines what UA is and is not, and requires explicit framework review plus a corresponding status-bearing change before research can alter the specification.
...
- [ ] **Validation Program 3** explicitly tests the Project-viability/Organizational-business-authorization/Project-Authorization ownership split and Architectural-Veto/economic-non-viability distinction as paper-level lifecycle hypotheses, alongside evidence/reassessment semantics.
...
- [ ] Figure 10 is process-oriented rather than a department influence map and shows exogenous evidence, initial authoritative/business context, Project viability evidence, Organizational Actuators, and return-to-Project routes.
...
- [ ] Evidence maturity is explicit: repository rigor is not empirical validation, and the next validation threshold includes independent tests of the Project-viability/Organizational-business-authorization handshake, Architectural Veto/economic-non-viability distinction, practical application, failure/correction traces, baseline-reconstruction evidence, fallback/common-mode/capacity/restoration evidence, evidence-instrument validity traces, negative-case learning traces, proportionality tests, landscape corrections, reproducible semantic-equivalence/substitution cases, practical-overhead comparisons, immutable mapping-snapshot reruns, cross-project comparison, and revision under contradictory evidence.
- [ ] Decision levels and capability families remain orthogonal.
- [ ] Constraint and Constraint Realization remain distinct.
- [ ] Project viability conclusion, Organizational launch/business authorization, Project Authorization, DoR, DoD, Release Gate, runtime correction, and Project Reauthorization remain separate.

...
→ Organization first supplies admissibility, authoritative boundaries, shared capabilities, reserved authority, and business intent for bounded Project / Architecture assessment or research
...
→ if Project / Architecture concludes that a deterministic/manual alternative leaves no Consequential Runtime Responsibility materially dependent on Model Judgment, the category is retested and the design exits the Thinking-System lifecycle rather than being forced through it; a narrower model-assisted design remains inside the category when the responsibility dependency remains
...
→ when production viability remains unresolved but a bounded experiment itself has a credible control envelope, Project may conclude further research is required; Organization may authorize that bounded research and Project issues a research-only technical authorization that cannot silently become production permission
...
→ Organization owns the proceed / continue / reshape / bounded-research / defer / do-not-proceed decision after receiving the Project viability conclusion; it may change price, target segment, service promise, scope, business outcome, funding, or investment assumptions and return the changed basis to Project / Architecture
→ Project Authorization is a scoped technical baseline: research-only after a legitimate bounded-research decision when production viability is still open, or production-capable after a positive Organizational Business Authorization on a technically viable production basis
...
→ an exogenous change to an Organizational authoritative or business basis may reactivate Organization directly; Project / Architecture then reassesses the technical basis before Delivery proceeds under a changed authorization
...
- **Article §4** — establish the four horizons and make the Organization ↔ Project / Architecture handshake explicit. Organization first establishes admissibility, business intent, reserved authority, shared capabilities, and evidence obligations. Project / Architecture then performs Model-Judgment-necessity, control-architecture, technical-feasibility, Human-Authority/fallback/capacity, and control-economics analysis and returns a viability conclusion. If a deterministic/manual alternative eliminates Model Judgment from every Consequential Runtime Responsibility, retest the category and exit the Thinking-System lifecycle when the test is negative; a narrower model-assisted alternative remains inside the category when the dependency persists. If production viability is unresolved but a bounded experiment has a credible control envelope, Organization may authorize bounded research and Project issues a research-only technical authorization whose evidence returns to viability analysis. An Architectural Veto blocks the unchanged proposal because no credible bounded control architecture exists; economic non-viability instead returns to Organization because business levers may change. Organization owns the proceed / continue / reshape / bounded-research / defer / do-not-proceed business decision. A production-capable Project Authorization is issued only for a technically viable basis covered by a positive Organizational Business Authorization. Runtime and Delivery evidence later route by the decision basis invalidated, with exogenous Organizational business-basis change able to reactivate Organization directly.
- **Article §4 figure sequence** — first show the four decision-ownership horizons as a standalone model, including the Organization ↔ Project / Architecture viability/business-authorization handshake, research-only versus production-capable Project Authorization, downward inheritance into Delivery, and direct reassessment from realization or operation evidence to the horizon whose decision basis is invalidated. Figure 11 must additionally show category retest and explicit exit when no Consequential Runtime Responsibility remains Model-Judgment-dependent. Do not imply that reassessment originates only at Runtime. Only then combine that model with the **Article §3** capability anatomy to produce the orthogonal horizons × capability-families map. Preserve the pedagogical order: capability model → decision-ownership model → combined model.
...
The Organizational horizon also owns the **business authorization decision around the Project viability conclusion**. Project / Architecture may determine that a credible control architecture exists but that the resulting economics do not support the current price, service promise, target segment, or investment assumptions. Organization may then change those business inputs, simplify or narrow the use case, authorize bounded research, defer, or decide not to proceed. If Project / Architecture issues an Architectural Veto because no credible bounded control architecture exists for the unchanged proposal, Organization may alter the proposal and request reassessment but may not simply waive the veto and treat the unchanged system as viable. If Project concludes that production viability is still open but a bounded experiment is credibly controllable, Organization may authorize that research without pretending production viability has already been established.
...
Project / Architecture owns the analysis and the resulting **viability conclusion**, not the organization's business decision. Representative conclusions include: Model Judgment is unnecessary for the stated outcome; technically viable as proposed; technically viable only with narrower scope or different authority; further bounded research is needed; technically controllable but economically unattractive under current assumptions; or **Architectural Veto** because no credible bounded control architecture exists for the current proposal.

If Model Judgment is unnecessary and the selected deterministic/manual alternative leaves no Consequential Runtime Responsibility materially dependent on Model Judgment, explicitly **retest the category and exit this Thinking-System lifecycle**. If the selected narrower model-assisted design still materially informs a consequential responsibility, it remains a Thinking System and the map is reapplied to that narrower candidate.

If `further bounded research is needed`, Project / Architecture must specify the experiment's technical control envelope and evidence question. Organization then decides whether the research is worth pursuing. A positive research decision permits Project to issue a **research-only Project Authorization**; it does not assert production viability and cannot silently mature into production permission.
...
Do not predetermine that variant C wins. A legitimate Project conclusion may prefer variant A or B, narrower variant C, require bounded research/redesign, identify economic non-viability, or issue Architectural Veto.
...
- correlated source/Organizational-business/Project/Delivery/configuration/version records sufficient to reconstruct the active behavioral and control baseline for material evidence;
...
- correlated authoritative-source / Organizational-business / Project-Authorization-scope / Delivery / realization / model / prompt-instruction / context-retrieval / tool-routing / evaluator-policy / deployment-scope / fallback identity sufficient to reconstruct the material active baseline;
...
→ change price / target segment / service promise / scope / investment assumptions / bounded research / stop
→ Project Reauthorization if the resulting organizational decision supports a technically viable Thinking-System basis
→ category exit instead if the selected alternative leaves no Consequential Runtime Responsibility dependent on Model Judgment
```

The point: the system may be respecting every Hard Constraint and still become non-viable under the current business assumptions. Project / Architecture diagnoses and characterizes that viability change; Organization owns the business continuation decision because it can change business levers the project cannot. If Project instead concludes that no credible bounded control architecture exists for the current proposal, that is an Architectural Veto: Organization may change the proposal and send it back for reassessment, but it may not override the veto for the unchanged basis.

**Case C — Organizational review: authoritative or business basis changes exogenously.**

An external contractual, legal, customer, organizational-policy, product-strategy, price/service-model, funding, or other Organizationally owned change alters the basis under which the project was authorized.

```text
external / organizational evidence or business decision
...
The point: this evidence need not originate as a model failure. Organization changes an Organizationally owned context; Project / Architecture reassesses the technical consequences before Delivery proceeds under a changed baseline.
...
→ Organization decides whether bounded research is worth pursuing
→ research approved
...
The point: research authorization closes the evidence loop without pretending that the production case has already passed technical or business authorization.
...
Then ask what remains: who authorizes the refund boundary, whether Model Judgment is justified, whether the `Hard` claim is actually deterministic across the complete path, which evidence invalidates which decision, whether Human Authority capacity is viable, whether the current Project Authorization is research-only or production-capable, and whether the resulting control perimeter makes economic sense.
...
- Organization owns initial admissibility/business intent and the proceed/continue/reshape/bounded-research/defer/do-not-proceed business decision after receiving the Project viability conclusion; it also owns reserved authority, shared-capability/evidence obligations, and business levers such as price, service promise, target scope, and investment assumptions.
- Project / Architecture compares deterministic/manual/model-assisted/autonomous alternatives, decides whether Model Judgment is needed for the stated outcome, category-retests any simpler alternative, places Judgment where retained, designs the complete control architecture, characterizes technical/control feasibility and complete economics, and returns a viability conclusion. Architectural Veto blocks the unchanged proposal; economic non-viability routes to Organization rather than being treated as technical impossibility.
- If production viability remains open but Project can define a credibly bounded experiment, Organization may authorize bounded research and Project issues a research-only Project Authorization. Its Delivery/Runtime evidence returns to Project and cannot self-promote into production.
...
- Use Case A for Delivery reassessment, Case B for Project viability reassessment → Organizational business review → Project Reauthorization where required, Case C for direct Organizational review after an authoritative/business-basis change, and Case D for bounded research without production promotion.
...
- assume the system should maximize autonomous resolution. The Project viability analysis may conclude that Model Judgment is unnecessary, a narrower model-assisted or deterministic design is preferable, more research is required, or Architectural Veto applies; Organization then owns the business decision on whether to reshape, research, defer, or stop the initiative;
- force a deterministic/manual alternative through the Thinking-System lifecycle after category retest shows that no Consequential Runtime Responsibility remains materially dependent on Model Judgment;
...
- [ ] Organization does not absorb the Project-owned Model-Judgment-necessity, control-architecture, or control-economics analysis; Project / Architecture returns a viability conclusion, while Organization owns the proceed/continue/reshape/bounded-research/defer/do-not-proceed business decision and may change business inputs before sending the proposal back for reassessment.
- [ ] If a simpler selected alternative leaves no Consequential Runtime Responsibility materially dependent on Model Judgment, the category is retested and the design exits the Thinking-System lifecycle; a narrower model-assisted alternative remains inside the category when the dependency persists.
- [ ] `Further research required` is not a dead end: Organization may authorize a bounded experiment, Project issues a **research-only Project Authorization** with explicit exposure/evidence limits, Delivery releases only that experiment, and evidence returns to Project viability analysis without automatic production promotion.
...
- [ ] Material release/runtime evidence can be correlated to the active source/Organizational-business/Project-Authorization-type-and-scope/Delivery/realization/model-config/evaluator/deployment/fallback baseline without requiring one universal registry.
...
- [ ] Case B routes through Project viability reassessment to Organizational business/continuation review because economics or the business basis is invalidated; Project Reauthorization follows only for the resulting technically viable authorized basis, and category exit remains possible if Model Judgment is removed from all Consequential Runtime Responsibilities.
- [ ] Case C routes to Organization when an authoritative or Organizationally owned business basis changes exogenously, followed by Project viability reassessment where the technical baseline is affected.
- [ ] Case D uses research-only Project Authorization to close an evidence gap without asserting production viability or production permission.
...
- when redesign removes Model Judgment from every Consequential Runtime Responsibility, the category must be retested; a negative result means the redesigned system is no longer a Thinking System even if an earlier version was.
...
10. why Project viability conclusion, Organizational Business Authorization, research-only versus production-capable Project Authorization, DoR, DoD, Release Gate, runtime correction, and Project Reauthorization are different decisions;
...
12. why bounded research can be technically authorized without pretending production viability exists, and why evidence from research must return to Project viability analysis before any production-capable authorization;
13. why a selected deterministic/manual alternative exits the Thinking-System lifecycle when category retest shows that no Consequential Runtime Responsibility remains Model-Judgment-dependent;
...
**Core claim:** Different control decisions require different evidence, authority, time horizons, automation, and corrective actions. Together, the four levels describe how one Thinking System moves from Organizational admissibility and business intent through Project / Architecture viability analysis, optional bounded research, Organizational Business Authorization for a production basis, scoped technical Project Authorization, Delivery realization/release, Runtime operation, correction, and reassessment. They are not four documents or four meetings; they are connected decision horizons in one socio-technical control system, and Organization and Project / Architecture may interact repeatedly before and after Delivery begins.

**Lifecycle-refinement status.** Current authority-bearing UA sources already separate Organization, Project / Architecture, Delivery, and Runtime and already place AI necessity, control-architecture feasibility, Human Authority/capacity, and control economics at Project level. This blueprint makes an additional decision handshake explicit for the paper: **Project / Architecture owns the viability conclusion; Organization owns the business decision to proceed, reshape, research, defer, or stop; Project Authorization is the scoped technical baseline that connects that decision to Delivery.** Two scopes are distinguished: **research-only Project Authorization** may follow an Organizational bounded-research decision even while production viability remains open, provided the experiment itself has a credible bounded control envelope; **production-capable Project Authorization** follows only a positive Organizational Business Authorization on a technically viable production basis. Research evidence must return to Project viability reassessment and cannot promote itself into production. Architectural Veto is a Project conclusion that cannot be overridden for the unchanged proposal; economic non-viability is an input to an Organizational business decision because Organization may change business levers and return a changed basis for reassessment. If a selected simpler alternative leaves no Consequential Runtime Responsibility materially dependent on Model Judgment, category retest exits the Thinking-System lifecycle. Until this exact sequencing is separately reconciled into authority-bearing lifecycle/pattern sources through framework review, describe it as a paper-level lifecycle refinement under validation rather than silently claiming that current normative wording already says the same thing.
...
- Make explicit that four horizons do **not** imply one pass through four boxes. The Organization ↔ Project / Architecture handshake may iterate before production and during continuation/reassessment: Organization supplies admissibility/business basis; Project returns viability evidence; Organization may authorize bounded research or make a business decision on a viable production basis; Project then issues the correspondingly scoped technical Project Authorization for Delivery.
- State explicitly that a simpler alternative can leave this lifecycle after category retest; the paper does not keep a deterministic/manual design inside the Thinking-System operating model merely because the project started as an AI proposal.
...
Show two adjacent views. The **decision-ownership side must present the canonical four-horizon model defined in this section**. Preserve the four horizons while showing the Organization ↔ Project / Architecture handshake, the research branch, and the two scopes of Project Authorization explicitly instead of implying a single one-way `Organization → Project → Delivery` gate sequence. Keep the same decision-ownership model consistent between standalone Figure 8 and Figure 9 rather than creating a second competing representation.

```text
Decision ownership — canonical four-horizon model

Organization — What is admissible, and what may the organization pursue, research, continue, reshape, or stop?
   │ initial admissibility · authoritative sources · reserved authority · business intent
   ↓
Project / Architecture — Is Model Judgment needed? Is production viable, or what evidence remains missing?
   │ viability conclusion: necessity · technical/control feasibility · control economics · alternatives · Architectural Veto if applicable
   ↑
Organization — bounded research | proceed / continue | reshape | defer | do not proceed
   ├─ bounded research authorized
   │       ↓
...
simpler alternative selected
→ category retest
→ if no Consequential Runtime Responsibility remains Model-Judgment-dependent: exit Thinking-System lifecycle

Delivery realization evidence ─┐
Runtime / research evidence ───┴→ Reassessment evidence — behavior · outcomes · control state · changed assumptions

Reassessment evidence → Delivery: implementation / realization / evidence issue
Reassessment evidence → Project / Architecture: architecture / feasibility / evidence / Human Authority / fallback / capacity / economics / Model-Judgment-necessity basis challenged or research question answered
Project viability reassessment → Organization: business case challenged, Architectural Veto, or changed business/authority input required
Reassessment evidence → Organization directly: authoritative source / reserved decision right / shared capability / Organizational business basis changed exogenously
...
**Primary question owned:** Within which authoritative boundaries, shared capabilities, and decision rights may Project / Architecture assessment or research proceed—and, after a Project viability conclusion, should the organization proceed, continue, reshape, fund bounded research, defer, or stop the initiative?
...
1. **initial admissibility / research basis** — whether the use category may be explored at all and under which authoritative boundaries, reserved rights, shared capabilities, and bounded-research conditions;
2. **business proceed / continuation authorization** — after Project / Architecture returns the Model-Judgment-necessity, technical/control-viability, control-economics, and alternative-design conclusion, whether the organization should proceed, continue, reshape, fund further research, defer, or stop the initiative.

Organization does **not** perform the Project-level analysis that determines whether Model Judgment is necessary, whether a credible complete bounded control architecture exists, or what the complete control economics are. Those analyses and the resulting viability conclusion belong to Project / Architecture. Organization owns business authority over the initiative and the business levers that may change the decision basis.

**Activation triggers:**

- a new business capability, process problem, customer need, or product opportunity for which a Thinking-System or bounded research path is proposed;
- a Project viability conclusion requiring a proceed, continuation, reshape, further-research, or do-not-proceed decision;
...
- a Project conclusion that Model Judgment is unnecessary for the stated outcome or that a deterministic/manual/narrower design is preferable;
...
- a Project Reauthorization request that cannot be resolved inside the standing Organizational business/authority basis;
...
- the Project / Architecture viability conclusion, including Model-Judgment-necessity analysis, alternative designs, complete bounded control-architecture feasibility, control economics/capacity, residual exposure/uncertainty, required business assumptions, any bounded-research proposal, and any Architectural Veto;
...
- Is the proposed use category admissible for the organization at all, including as bounded research?
...
- After receiving the Project viability conclusion, should the organization proceed/continue on that basis, authorize only bounded research, reshape the business case, defer, or stop?
- If production viability remains unresolved but Project proposes a credibly bounded experiment, is the evidence worth acquiring and which Organizational limits must bind that research?
...
- If Project concludes that Model Judgment is unnecessary for the stated outcome, should the organization pursue the simpler design, change the intended outcome, or stop the AI path? Organization may change the desired business problem, but it must not treat an executive preference for “using AI” as proof that Model Judgment is technically necessary for an unchanged outcome.
...
Organization may express that a class of activity must remain deterministic where an authoritative source genuinely requires that property, but the engineering choice between deterministic, manual, narrower model-assisted, and broader Thinking-System designs—and the conclusion about whether Model Judgment is needed for the stated outcome—remains a Project / Architecture analysis.
...
- Automation may help apply explicit delegated rules, but it must not invent authority, change a prohibited-use boundary, waive an Architectural Veto for an unchanged proposal, promote research exposure into production, or silently accept residual exposure outside delegation.
...
- approve, deny, defer, continue, narrow, suspend, or revoke the **business permission to pursue the initiative**;
- authorize bounded research rather than production-capable pursuit;
...
The article must distinguish these actions from Project or Runtime Actuators. Organization changes the authoritative/business context or permission boundary; it does not design the concrete control architecture and does not directly perform a runtime rollback unless the same person or mechanism separately holds runtime authority.

**Outputs and artifacts flowing downward:**

The level should produce or make explicit, by reference where possible:

- initial Organizational status: prohibited, eligible only for bounded research, or eligible for Project / Architecture assessment inside stated conditions;
...
- after Project viability analysis, an explicit business decision: proceed/continue on the viable production basis, bounded research, reshape/change assumptions and return to Project, defer, or do not proceed;
- where bounded research is approved, the Organizational research basis and limits that allow Project to issue a **research-only Project Authorization** without asserting production viability;
...
- Project viability conclusion covering Model-Judgment necessity, alternatives, technical/control feasibility, Human Authority/fallback/capacity, residual exposure, and complete control economics;
- bounded-research proposal or research evidence that changes the viability basis;
...
- business decisions repeatedly ignored or obscured Project viability evidence;
...
The resulting change may improve source clarity, delegated authority, evidence obligations, shared capabilities, exception handling, project-entry criteria, research boundaries, investment/business assumptions, or Organizational admissibility rules. The manuscript must present systematic negative-case learning as a **proposed operating discipline under validation**, while the Project-viability / Organizational-business-authorization / scoped-Project-Authorization handshake itself is a separate paper-level lifecycle refinement requiring framework reconciliation.

**Supporting figure — Figure 10: Organizational control process across the lifecycle:** Replace the current department-centric influence map with a process-oriented Organizational control loop showing both initial context and post-Project viability evidence as **converging inputs**, not as a sequential evidence pipeline:
...
authoritative sources + shared capabilities + reserved authority + business intent ───────────┼→ Organizational Controller / legitimate decision owners
                                                                                               │
Project viability conclusion / research or Runtime evidence / authority-change request ───────┘
                                                                                               ↓
                                                                                  Organizational decision
                              admissibility · bounded research · proceed/continue · reshape · defer · do-not-proceed
                                                                                               ↓
                                                                                  Organizational Actuators:
                          change permission · business assumptions · price/scope/outcome/funding · shared capability ·
                          vendor/deployment approval · exception · narrow · suspend
                                                                                               ↓
                                               updated authoritative/business context + evidence obligations
                                                                                               ↓
                                                                                  Project / Architecture
```

Examples within the lower-level evidence lane include a Project viability conclusion, bounded-research proposal/result, Architectural Veto, economics/business-case finding, authority request, and Runtime evidence that invalidates an Organizational basis. They are examples of that lane, not additional independent inputs that should be drawn a second time.

The three upper inputs must remain visually parallel or convergent in the manuscript figure. Authoritative sources/business intent are the reference basis, while external/Organizational evidence and Project/Runtime evidence are evidence streams; none should be drawn as if it is generated by the preceding input. The caption must state that the figure represents a decision horizon and control relationship, not a required department structure or a claim that Organization directly performs downstream technical actions. The same Organizational horizon may act before Project research and again after the Project viability conclusion without creating a fifth level.

#### Project / architecture control and viability

**Primary question owned:** Is Model Judgment needed for the stated outcome, does a credible complete bounded control architecture exist for production, or what evidence remains missing—and what is the resulting technical/operational/economic **Project viability conclusion** to return to Organization?

This is the analytical/architectural horizon in which Organizational admissibility becomes a concrete viability conclusion for the proposed controlled system. A successful prototype is not evidence of final production viability. **Project / Architecture owns the Model-Judgment-necessity analysis, project-level outcome formulation, control architecture, technical/control feasibility, Human Authority/fallback/capacity analysis, complete control economics, and the viability conclusion. Organization owns the business proceed/continuation/research decision. Project Authorization is the scoped technical baseline Project issues after the applicable Organizational decision: research-only for an authorized bounded experiment, production-capable for a viable production basis covered by Organizational Business Authorization.**
...
- a positive Organizational bounded-research decision requiring Project to formalize a research-only technical baseline;
...
- Organizational admissibility, business/value assumptions, authoritative source references, prohibited states, reserved decisions, shared-capability dependencies, evidence obligations, and reassessment triggers;
...
- where this is a reassessment, the current Organizational business/research authorization and Project Authorization type/scope.
...
- If a simpler alternative is selected, does any Consequential Runtime Responsibility remain materially dependent on Model Judgment? If not, the design exits this Thinking-System lifecycle; if yes, reapply the map to the narrower candidate.
...
- After an Organizational bounded-research decision, what exact research-only technical scope may be formalized for Delivery?
...
- Which assumptions or evidence changes require Project viability reassessment, Project Reauthorization, Organizational business review, or category exit?
...
- enough cost/capacity/latency information to make the viability conclusion usable by Organization as a business decision input;
- for research-only authorization, an explicit prohibition on exposure beyond the research envelope and an evidence-return path to Project viability reassessment.
...
→ Organizational decision:
   bounded research | proceed / continue | reshape | defer | do not proceed
```
...
The distinction between **Architectural Veto**, **economic non-viability**, and **research uncertainty** is mandatory:
...
- **Further bounded research required:** production viability remains unresolved, but Project can identify a credibly bounded experiment and the evidence it needs. Organization decides whether the evidence is worth acquiring; a positive decision allows a research-only Project Authorization, not production permission.
...
**B1. Research-only Project Authorization flowing to Delivery after Organizational bounded-research authorization:**
...
The repository's **Project Control Architecture and Viability Review** is one canonical current pattern that can carry much of the current framework baseline. The exact research-only/production-capable Project Authorization split is a paper-level research refinement until framework reconciliation; the article should not imply that current status-bearing templates already encode it. Existing architecture, product, risk, business-case, or decision records may carry equivalent decisions when ownership, versioning, and evidence remain explicit.
...
Project / Architecture may analyze, redesign, narrow, request more research, compare alternatives, category-retest, and issue the viability conclusion. It may issue or reissue Project Authorization only when a standing Organizational decision covers that **specific scope**.

- If bounded research is approved and the experiment itself is credibly controllable, Project may issue a research-only Project Authorization even though production viability remains unresolved.
- Research-only authorization may be revised within the standing research basis but may not be promoted to production without a new viability conclusion and positive Organizational Business Authorization on a production-viable basis.
...
- If the architecture remains credible but economics, price/value assumptions, Human Authority/capacity burden, Model-Judgment-necessity rationale, or business outcome no longer close, Project returns the viability conclusion to Organization for a proceed/continuation/reshape/defer/do-not-proceed decision.
- If the selected deterministic/manual design leaves no Consequential Runtime Responsibility Model-Judgment-dependent, record the negative category retest and exit the Thinking-System lifecycle rather than issue another Thinking-System Project Authorization.
...
→ Model Judgment necessity / deterministic or narrower alternative check
   → simpler alternative selected → category retest
      ...
      → dependency remains
         → continue with narrower Thinking-System candidate
...
+ residual exposure / uncertainty
→ Project viability conclusion:
   viable | narrower design | further research | economic non-viability | simpler alternative | Architectural Veto
→ Organization:
   bounded research | proceed / continue | reshape | defer / do-not-proceed
   ├→ bounded research → research-only Project Authorization → Delivery experiment → evidence → Project viability reassessment
...
→ Project viability reassessment
   ├→ research remains bounded / authorized → research-only Project Reauthorization
   ├→ still production-viable inside standing Organizational basis → production Project Reauthorization
   └→ business basis challenged / Architectural Veto / wider authority required → Organizational review
```

The figure must not place required operating-contract properties inside the Project Constraint Architecture. They are a separate Project output that Delivery later translates into its approved Requirement and Operating Envelope. It must keep **Project viability conclusion**, **Organizational business/research decision**, **research-only Project Authorization**, **production-capable Project Authorization**, and Delivery release visually distinct. Architectural Veto is a Project conclusion; `do not proceed` is an Organizational business decision. The category-exit branch must be real rather than decorative: a non-Thinking-System alternative leaves this lifecycle.
...
- research-only Project Authorization of a bounded experiment following Organizational bounded-research authorization;
...
- Can material release, experiment, incident, and correction evidence be correlated to the active authoritative-source, Organizational-business/research, Project-Authorization-type/scope, Delivery, realization, model/configuration, evaluator, deployment, and fallback baseline rather than to an ambiguous list of independent versions?
...
- active model, prompt/instruction, context/retrieval, tool/routing, policy, evaluator, realization, deployment-scope, and fallback versions where material, correlated with relevant source/Organizational-business-or-research/Project-Authorization/Delivery baselines enough to reconstruct the material active behavioral/control state;
...
Project viability reassessment
   → research remains bounded and authorized: research-only Project Reauthorization
   → production remains viable inside standing Organizational business/authority basis: production Project Reauthorization
   → economics / Model-Judgment necessity / business basis invalidated, Architectural Veto, or Organizational change required: Organizational review
Organizational review
   → bounded research / changed business or authority basis / renewed Business Authorization
   → Project reassessment before Delivery proceeds under a new scope
...
- enough correlated baseline identity to reconstruct relevant authoritative-source, Organizational-business/research, Project-Authorization-type/scope, Delivery, realization, model/prompt/context/retrieval/tool/routing/evaluator/policy/deployment/fallback state for material evidence;
...
- correlated active source/Organizational-business-or-research/Project-Authorization-type/project/delivery/configuration/version traceability sufficient to reconstruct the material behavioral/control baseline;
...
→ Organizational bounded-research / business review as required
...
→ Project viability reassessment
→ Organizational proceed / continuation / reshape / defer / do-not-proceed review
→ production Project Reauthorization only for the resulting authorized technically viable basis
→ category exit if selected redesign leaves no Consequential Runtime Responsibility Model-Judgment-dependent
...
authoritative source, reserved decision right, prohibited-use boundary, approved vendor/deployment mode, shared capability, or Organizational business basis changed exogenously
→ Organizational review
...
proposed authority / exposure expansion
→ Project viability reassessment
...
Keep Figure 13's escalation exit generic (`authorization or viability basis invalidated → route by decision ownership`) and let Figure 14 own the concrete Delivery / Project / Organization destinations. Ensure Figure 12 and Figure 14 agree: a Delivery-discovered issue that challenges Project Authorization routes first to Project viability reassessment; research results never self-promote to production; if Project can reauthorize inside the standing scope/basis, it does so. If economics/business basis/Model-Judgment necessity is invalidated, Architectural Veto applies, wider authority is needed, or an Organizational authoritative/business basis changed exogenously, Organization owns the next business/authority decision before Project issues the resulting scoped technical baseline.
...
**Epistemic-status rule:** Existing authority-bearing UA sources already establish downward inheritance, runtime evidence, local reassessment, Project Reauthorization, organizational review, and capability relationships. This paper proposes two additional pieces of research framing that must remain visibly under validation until separately reconciled: (1) the explicit **Project viability conclusion → Organizational business/research decision → scoped Project Authorization** handshake, including research-only versus production-capable technical authorization and category exit; and (2) systematic negative-case analysis and control-improvement feedback across the levels. Do not present either as settled normative UA behavior before framework review accepts the corresponding change.
...
**Supporting figure — Figure 15: Cross-level learning and stabilization loop:** Show negative case/research evidence → triage to owning decision level → diagnose Sensor/Constraint/Realization/Controller/Actuator/Human Authority/fallback/automation/baseline/assumption weakness → change within authority or reassess/reauthorize at the owning horizon → improved control architecture or decision basis → runtime/lifecycle verification. The figure must not imply that every case escalates to Organization or that every negative case is a Bug.
...
- **Organization / Project boundary and handshake:** Organization owns initial admissibility, authoritative boundaries, shared capabilities, reserved decisions, evidence obligations, exceptions, business intent, and the business decision to proceed/continue/reshape/research/defer/stop. Project / Architecture owns Model-Judgment necessity, concrete control architecture, technical/control feasibility, Human Authority/fallback/capacity analysis, complete control economics, Architectural Veto, and the Project viability conclusion. Project Authorization is a scoped technical baseline, not the Organizational business decision.
- **Research authorization maturity:** `Further research required` must not dead-end and must not pretend production viability exists. Organization may authorize bounded research when the experiment itself is credibly controllable; Project then issues a research-only Project Authorization with explicit exposure/evidence limits. Delivery/Runtime evidence returns to Project viability assessment. No Release Gate or local Project action may promote that research authorization into production-capable permission.
...
- **Category-exit discipline:** when the selected deterministic/manual redesign leaves no Consequential Runtime Responsibility materially dependent on Model Judgment, category retest exits the Thinking-System lifecycle. A narrower model-assisted candidate remains a Thinking System if the material dependency remains. Do not turn the lifecycle into an AI-retention mechanism.
...
- **Behavioral/control baseline correlation:** Material evidence, incidents, release decisions, experiment results, and Actuator actions need enough correlation across authoritative-source, Organizational-business/research, Project-Authorization-type/scope, Delivery, realization, model/prompt/context/retrieval/tool/routing/evaluator/policy/deployment/fallback state to reconstruct what was actually active. The paper must not turn this need into a mandatory universal registry or invent a canonical “Behavioral Configuration Baseline” artifact before application evidence shows one is warranted.
...
- **Organizational Actuator taxonomy:** The paper may show functional examples such as changing permission, exception, vendor approval, shared capability, project eligibility, business authorization, bounded research, or business assumptions. Do not create a mandatory exhaustive taxonomy unless future doctrine requires one.
...
> Project / Architecture owns the viability conclusion; Organization owns the business decision to proceed, research, reshape, defer, or stop; Project Authorization is the scoped technical baseline that connects the applicable Organizational decision to Delivery.

> Research-only Project Authorization can generate missing viability evidence; it is not provisional production permission. Production-capable Project Authorization follows only a technically viable basis covered by Organizational Business Authorization.
...
> If category retest shows that no Consequential Runtime Responsibility remains materially dependent on Model Judgment, the redesigned system exits this Thinking-System lifecycle rather than being forced through it.
...
- Replace the department-centric Organization figure with a process-oriented Organizational control relationship in which authoritative sources/business intent, external/Organizational evidence, and Project/Runtime evidence converge on legitimate decision owners.
- Make the Organization ↔ Project / Architecture handshake explicit: Organization owns initial admissibility/business intent and the business decision to proceed/continue/reshape/research/defer/stop; Project / Architecture owns Model-Judgment necessity, concrete control architecture, complete control economics, Architectural Veto, and the viability conclusion.
- Distinguish **research-only** Project Authorization from **production-capable** Project Authorization. The former follows bounded-research authority when production viability is still open; the latter follows positive Organizational Business Authorization on a viable production basis. Neither Delivery nor Runtime may widen the active scope.
- Require category retest when a simpler alternative is selected; exit the Thinking-System lifecycle if no Consequential Runtime Responsibility remains materially Model-Judgment-dependent.
...
- Preserve enough correlated source/Organizational-business-or-research/Project-Authorization-type-and-scope/Delivery/realization/model-config/evaluator/deployment/fallback identity to reconstruct the material active behavioral/control baseline for release, incident, experiment, and corrective evidence; do not create a mandatory universal registry by implication.
...
- Treat the exact Project-viability / Organizational-business-or-research-authorization / scoped-Project-Authorization handshake as a paper-level lifecycle refinement until separately reconciled into status-bearing framework sources; `framework-traceability.md` records this as **Needs Resolution**.
...
- the paper-level Project viability conclusion → Organizational business/research decision → scoped Project Authorization handshake, including research-only versus production-capable authorization, Architectural Veto versus economic non-viability, and the requirement to reconcile this exact sequencing with status-bearing framework sources before treating it as normative UA;
- category retest and lifecycle exit when the selected redesign leaves no Consequential Runtime Responsibility materially dependent on Model Judgment;
- authority, decision rights, and Constraints flow downward by reference while scoped realizations become concrete; changed authoritative/business sources and Delivery realization, research, or Runtime operation evidence return to the horizon whose decision basis they affect or invalidate;
...
→ 8. revisit depth, scope, carrier choice, or evidence-instrument validity when an authoritative source, Organizational business/research authorization, Project Authorization type/scope, Project assumption, model/population/policy/operating condition, Delivery realization, or Runtime operation changes the materiality judgment
```
...
```text
Project viability conclusion: further research required
→ Organizational bounded-research authorization
→ research-only Project Authorization with explicit experiment purpose / exposure / evidence obligations / stopping conditions
...
→ active Organizational / Project decision basis and version
→ downstream obligations and accountable owner
→ fit-for-decision evidence
→ continue, narrow, redesign, research, stop, reshape business basis, reauthorize, or exit category
...
For a standing viability/economic relationship, **Decision / reference semantics** must preserve the accountable viability owner, relevant input categories, legitimate decision right, available outcomes such as Project viability conclusion → Organizational research/business decision → scoped Project Authorization where applicable, and the decision basis whose change triggers reassessment. For a research relationship, the semantics must preserve the non-production scope and evidence-return path. The actual cost, latency, capacity, staffing, Human Authority burden, maintenance burden, operational friction, and failure surface remain separate observations for the later practical-preference comparison.
...
5. **Decision-horizon contribution** — where does it materially contribute across Organization, Project / Architecture, Delivery, and Runtime, including the distinction among Project viability conclusion, Organizational business/research authorization, research-only versus production-capable Project Authorization, and category exit where those distinctions are part of the tested paper model?
...
11. **Viability-decision semantics and control economics** — does it preserve who owns the technical/control viability conclusion, which cost/capacity/latency/burden categories inform it, which Organization-level business/research decision consumes that conclusion, which outcomes can follow, what technical authorization scope results, and what triggers reassessment? Record actual lifecycle cost, Human Authority burden, fallback cost, false blocks, latency, incidents, maintenance, and operational friction separately for the later practical-preference comparison; numerical burden is not itself the semantic-equivalence test.
...
- applicable law and binding contractual obligations may directly create authoritative source obligations within scope; Organization identifies applicable authoritative sources and legitimately adopts internal or voluntary sources where appropriate, Project interprets them into scoped Project Constraints, viability conclusions, required operating-contract properties, assumptions, authority, and evidence obligations, Organization makes the business/research decision on the Project viability basis, Project issues the correspondingly scoped technical Project Authorization, and Delivery translates the inherited baseline into an approved Requirement and Operating Envelope for the bounded scope;
...
+ reassessment route
+ viability-decision semantics: Project viability owner/conclusion, Organizational business/research-decision consumer/options, scoped Project Authorization handoff where applicable, and reassessment trigger
```

The final viability-decision item applies only where economics/viability is material to the relationship. It preserves the meaning of the viability/business-authorization decision chain, not the numerical burden of one implementation. If research authorization is material in a tested composition, preserve the distinction between bounded evidence generation and production permission. Actual lifecycle cost, latency, Human Authority load, capacity demand, maintenance burden, operational friction, and failure surface are recorded separately and compared only during **practical preference**.
...
For a material **economics/viability relationship**, semantic equivalence concerns the decision semantics: Project / Architecture or an equivalent legitimate engineering authority must own the technical/control viability analysis and conclusion; the relevant cost/capacity/latency/burden input categories must be available; Organization or an equivalent legitimate business authority must consume that conclusion and own proceed/continuation/reshape/research/defer/do-not-proceed choices; the technical authorization handoff and reassessment trigger must remain explicit where applicable. If bounded research is part of the tested lifecycle, an equivalent composition must preserve the non-production exposure boundary rather than merely call an experiment “approved.” A substitute may therefore be semantically equivalent even when its actual costs or operating burden are higher. Those measured or estimated values belong to the practical-preference comparison below.
...
- **Viability/business/research-authority nuance:** the paper's explicit Project-viability → Organizational-business/research-decision → scoped-Project-Authorization handshake is a research refinement until framework reconciliation. Landscape comparison must allow an equivalent existing method/composition to distribute these decisions differently if legitimate analytical and business authority, bounded-research versus production semantics, veto semantics, technical authorization, category exit, and reassessment are preserved.
...
- **Duplication risk:** do not reteach the capability anatomy, horizons, Human Authority, Hard/Soft, the Organization↔Project viability/business/research-authorization handshake, baseline reconstructability, fallback credibility, or reassessment semantics already established in **Article §§3–4**.
...
- the four decision horizons, the paper-level Project-viability / Organizational-business-or-research-authorization / scoped-Project-Authorization handshake, category exit, and invalidated-decision-basis routing;
...
- an explicit paper-level handshake in which Project / Architecture owns Model-Judgment necessity and the technical/control/economic viability conclusion, Organization owns the business/research decision and cannot override Architectural Veto for the unchanged proposal, and Project Authorization becomes a scoped technical baseline—research-only for bounded evidence generation or production-capable for a viable production basis covered by Organizational Business Authorization;
- explicit category exit when a selected redesign leaves no Consequential Runtime Responsibility materially dependent on Model Judgment;
- authority, decision rights, and Constraints flowing downward by reference while scoped realizations become concrete, with realization/operation/research evidence routed upward to the owning decision basis;
...
- whether the explicit Project viability conclusion → Organizational business/research decision → research-only or production-capable Project Authorization handshake improves ownership clarity and should be accepted into status-bearing lifecycle/pattern sources;
- whether research-only authorization provides a useful non-production evidence path without creating bureaucracy or authorization ambiguity;
- whether category exit after Model-Judgment removal is operationally clear and consistently applied;
...
- the Project-viability / Organizational-business-or-research-decision / scoped-Project-Authorization handshake, research-only versus production-capable distinction, category-exit rule, and Architectural-Veto/economic-non-viability distinction are paper-level lifecycle refinements not yet reconciled into current authority-bearing lifecycle/pattern sources; `framework-traceability.md` records the question as **Needs Resolution**;
...
1. **Specification boundary and indexed engineering surfaces** — `SPECIFICATION.md` defines/indexes the specification boundary and conformance/document-status model; the definitions, controlled object, capability anatomy, nested lifecycle, Requirements/Constraints/Correctness, Human Authority, patterns, capabilities, Control Plane guidance, and failure-mode taxonomy that it indexes are interpreted only according to each document's declared status and scope. The exact Project-viability / Organizational-business-or-research-decision / scoped-Project-Authorization handshake remains paper-level research until framework reconciliation changes those status-bearing surfaces.
...
+ Project viability conclusion ↔ Organizational business/research decision ↔ scoped Project Authorization handshake
+ explicit research-only / production-capable distinction and category exit
...
- the Project-viability / Organizational-business-or-research-decision / scoped-Project-Authorization handshake is one such paper-only hypothesis until accepted;
...
> `SPECIFICATION.md` defines and indexes the current UA specification boundary; this paper's broader composition—including its explicit Project-viability / Organizational-business-or-research / scoped-Project-Authorization lifecycle refinement—remains a testable research synthesis, not proof that every boundary, partition, artifact, operating hypothesis, or claimed landscape gap is already right.
...
- validation of the Project-viability / Organizational-business-or-research-decision / scoped-Project-Authorization lifecycle refinement, including category exit;
...
|---|---|---|---|
| **1. Category and controlled-object boundary** | Consequential Runtime Responsibility + probabilistic Model Judgment identifies a useful engineering class, the whole software Thinking System is the useful controlled object, and the surrounding control perimeter is correctly treated as socio-technical; category retest can also identify when redesign removes the system from the class | systematic over/under-classification, inability to distinguish material from trivial model use, domains where a different object yields clearer decisions, or evidence that the controlled-process/control-architecture boundary or lifecycle-exit rule is placed incorrectly | record the result in research/traceability; propose a narrower/replaced test or controlled-object rule; change doctrine only through explicit framework acceptance |
...
- traces of pre-production Project viability analysis including Model-Judgment necessity, alternative-design, control-architecture, Human Authority/fallback/capacity, and full control-economics conclusions;
- cases where a simpler selected alternative triggered category retest and either exited the Thinking-System lifecycle or remained a narrower Thinking System;
- bounded-research cases showing how an unresolved production-viability question became a research-only Project Authorization, whether the research scope remained bounded, what evidence returned, and whether any team attempted or avoided silent promotion into production;
...
- cases where Project concluded Model Judgment was unnecessary and Organization either accepted the simpler design or changed the intended outcome before reassessment;
...
- release/runtime/incident/research records showing whether material source, Organizational-business-or-research, Project-Authorization-type/scope, Delivery, realization, model/prompt-instruction/context-retrieval/tool-routing/evaluator-policy/deployment/fallback state could be correlated well enough to reconstruct the active behavioral/control baseline without relying on a new mandatory registry;
...
- independent duplicate substitution assessments against the same mapping ID/version/immutable snapshot, including disagreements about scope alignment, equivalence, stronger semantics, viability/business/research-authorization semantics, or non-comparability;
...
> The support-resolution example is editorially constructed. It demonstrates continuity of explanation; it cannot validate the category, operating map, Project-viability / Organizational-business-or-research / scoped-Project-Authorization lifecycle refinement, proportionality method, landscape gap, or Uncertainty Architecture.

Use the case only to formulate transfer questions:

- Can another team distinguish Project-owned Model-Judgment-necessity/control-viability/economics analysis from Organization-owned business/research authority without creating duplicated bureaucracy?
- Can it use research-only authorization to gather missing evidence without either dead-ending or leaking into production?
- Can it identify when category retest should exit the Thinking-System lifecycle after a simpler redesign?
...
2. **Architectural anchor B — Figure 9: Two orthogonal models** — already established in **Article §4**: the decision side reproduces the four-horizon decision model, including the Organization ↔ Project / Architecture viability/business/research-authorization handshake, research-only versus production-capable Project Authorization, reassessment evidence from Delivery realization or Runtime/research operation, and the same direct/conditional return routes, while a visually distinct capability-family side shows Actuators, Constraints and realizations, Sensors and evidence, and Controllers and decision authority as functions that may appear at every horizon. Ordering is a reading aid, not an execution pipeline or one-to-one mapping.
...
- Figure 8 — one controlled object across four decision horizons, including the Organization ↔ Project viability/business/research-authorization handshake and Delivery-realization / Runtime/research reassessment evidence;
- Figure 9 — the two orthogonal models;
- Figure 10 — Organizational control process;
- Figure 11 — Project control architecture, viability conclusion, category exit, and research-only/production-capable authorization handshake;
- Figure 12 — Delivery translation, bounded exposure, and release loop;
- Figure 13 — Runtime control and reassessment;
- Figure 14 — evidence and change routing, including exogenous Organizational business-basis change;
...
- distinguish external authority sources, business/research authorization, technical viability/authorization, implementation surfaces, evidence, decision ownership, action, and revision where material;
...
- conflate **Project viability conclusion**, **Organizational business/research authorization**, **research-only Project Authorization**, **production-capable Project Authorization**, and **Delivery Release Gate**;
- let bounded research dead-end because production viability is not yet established; use a research-only technical authorization when the experiment itself can be bounded credibly;
...
- keep a deterministic/manual redesign inside the Thinking-System lifecycle after category retest shows that no Consequential Runtime Responsibility remains Model-Judgment-dependent;
...
- present the new viability/business/research-authorization handshake as current normative UA before relevant status-bearing lifecycle/pattern sources are explicitly reconciled;
...
- imply that every negative case must escalate to Organization; route by invalidated decision basis, and reactivate Organization only when an Organizational business/authority decision is implicated;
- omit exogenous Organizational business-basis change from the direct Organizational review path merely because the signal did not originate in runtime;
...
- [ ] Figure 8 keeps Organization, Project / Architecture, Delivery, and Runtime as four decision horizons while showing the **Organization ↔ Project / Architecture handshake**: initial Organizational admissibility/business context → Project viability conclusion → Organizational bounded-research or business decision → correspondingly scoped Project Authorization → Delivery. Repeated Organization/Project decisions do not create extra horizons; bounded research is not forced through a production-viability claim.
- [ ] Figure 9 reproduces the Figure 8 four-horizon decision model—including bounded research, research-only versus production-capable Project Authorization, reassessment evidence from Delivery realization or Runtime/research operation, and the same return routes—and adds the capability-family dimension as a visually distinct green group with undirected structural grouping and no implied one-to-one mapping or execution pipeline.
...
- [ ] Organizational control owns initial admissibility, authoritative boundaries, shared capabilities, reserved/delegated decision rights, evidence obligations, exceptions, business assumptions, and the proceed/continue/reshape/bounded-research/defer/do-not-proceed decision after Project viability analysis; it does **not** absorb the Project-owned Model-Judgment-necessity, control-architecture, technical-feasibility, or complete control-economics analysis.
- [ ] Project / Architecture owns Model-Judgment necessity, alternative comparison, concrete bounded control architecture, complete control economics, Architectural Veto, and the viability conclusion. It issues a **research-only Project Authorization** after a legitimate Organizational bounded-research decision when the experiment itself is credibly bounded, or a **production-capable Project Authorization** only after positive Organizational Business Authorization on a technically viable production basis.
...
- [ ] When Project selects a deterministic/manual or other simpler alternative, the category is retested. If no Consequential Runtime Responsibility remains materially dependent on Model Judgment, the design exits the Thinking-System lifecycle; a narrower model-assisted design stays inside it when the dependency persists.
...
- [ ] Organizational evidence includes both Project viability/research/Runtime evidence and exogenous/cross-project evidence such as legal, contractual, audit, vendor, **business-assumption**, and shared-capability change.
- [ ] Figure 10 shows external/Organizational evidence, authoritative/business reference sources, and Project/research/Runtime evidence/authority requests as converging inputs to the Organizational Controller rather than a sequential pipeline.
...
- [ ] Figure 10 shows a real control relationship including Organizational Actuators such as bounded research, proceed/continue/reshape/do-not-proceed and business-assumption changes, not only a list of participating departments or an approval box.
...
- [ ] Project / Architecture explicitly owns project-level outcome formulation, Model-Judgment necessity, project boundary, intended Judgment landscape, Project Constraint Architecture, control viability, complete control economics, and the Project viability conclusion for the specific system; it receives a hypothesis rather than presupposed proof of Model Judgment value and allows viable/narrower/research/economic-non-viability/Model-Judgment-unnecessary/Architectural-Veto conclusions without claiming the Organizational business decision or Delivery's implementation-level Judgment Nodes / approved Requirement and Operating Envelope.
...
- [ ] Figure 11 keeps the Project Constraint Architecture limited to scoped Project Constraints/assumptions, renders required operating-contract properties/evidence obligations as a **separate Project output**, includes category retest/exit, and distinguishes Project viability conclusion → Organizational bounded-research/business decision → research-only or production-capable Project Authorization → Delivery.
...
- [ ] Material evidence can be correlated to the active source/Organizational-business-or-research/Project-Authorization-type-and-scope/Delivery/realization/model-config/evaluator/deployment/fallback baseline sufficiently to reconstruct what governed a material release, experiment, incident, or Actuator decision; no universal registry is implied.
...
- [ ] Figure 12 distinguishes Release Gate **rework**, **terminal stop/defer/reject**, and **Project viability reassessment**; it releases only inside the current Project Authorization scope, never promotes research-only authorization into production, and does not bypass Project / Architecture to Organization. Project may reauthorize locally when the basis remains viable inside standing Organizational authorization; Organization is reached when business/authority basis, Architectural Veto, or wider Organizational change is implicated.
...
- [ ] Runtime routes research results, economics/Model-Judgment-necessity/business-basis invalidation through Project viability reassessment before any new Project Authorization; direct Organization review remains appropriate for exogenous Organizational authoritative **or business-basis** change.
...
- [ ] The article clearly distinguishes current status-bearing lifecycle semantics from both the **paper-level Project-viability / Organizational-business-or-research / scoped-Project-Authorization refinement** and the **proposed cross-level negative-case learning/stabilization discipline under validation**; framework traceability records the lifecycle refinement as **Needs Resolution**.
...
- [ ] **Article §5** produces both the authority-variant comparison and a complete canonical material-relationship-to-carrier working mapping that **Article §6** can consume without rerunning the carrier analysis; the mapping has a stable ID/version, controlled-object/lifecycle baseline, source/Organizational-business/Project-authorization baseline, scope, assumptions, materiality decision rule, guarantee semantics where applicable, and materiality rationales for inclusion/exclusion. The mapping header does not contain a self-referential repository SHA; the substitution run/verdict records an immutable snapshot reference after freeze.
...
- [ ] **Article §5** revisits implementation depth and carrier choice when authoritative sources, Organizational business/research authorization, Project Authorization type/scope, Project assumptions, Delivery realization evidence, Runtime operation evidence, or evidence-instrument validity change; such evidence may strengthen, narrow, redistribute, consolidate, simplify, recalibrate, or replace parts of the implementation.
...
- [ ] **Article §7** states the proposed contribution as a paper-level synthesis/recomposition, separates the existing specification boundary from the Project-viability/Organizational-business-or-research/scoped-Project-Authorization refinement and other paper-only carrier/substitution/reverse-mapping/integration-gap hypotheses, defines what UA is and is not, and requires explicit framework review plus a corresponding status-bearing change before research can alter the specification.
...
- [ ] **Validation Program 3** explicitly tests the Project-viability/Organizational-business-or-research/scoped-Project-Authorization ownership split, research-only versus production-capable distinction, category exit, and Architectural-Veto/economic-non-viability distinction as paper-level lifecycle hypotheses, alongside evidence/reassessment semantics.
...
- [ ] Figure 10 is process-oriented rather than a department influence map and shows exogenous evidence, initial authoritative/business context, Project viability/research evidence, Organizational Actuators, and return-to-Project routes.
...
- [ ] Evidence maturity is explicit: repository rigor is not empirical validation, and the next validation threshold includes independent tests of the Project-viability/Organizational-business-or-research/scoped-Project-Authorization handshake, research-only versus production-capable distinction, category exit, Architectural Veto/economic-non-viability distinction, practical application, failure/correction traces, baseline-reconstruction evidence, fallback/common-mode/capacity/restoration evidence, evidence-instrument validity traces, negative-case learning traces, proportionality tests, landscape corrections, reproducible semantic-equivalence/substitution cases, practical-overhead comparisons, immutable mapping-snapshot reruns, cross-project comparison, and revision under contradictory evidence.
- [ ] Decision levels and capability families remain orthogonal.
- [ ] Constraint and Constraint Realization remain distinct.
- [ ] Project viability conclusion, Organizational business/research authorization, research-only/production-capable Project Authorization, DoR, DoD, Release Gate, runtime correction, and Project Reauthorization remain separate.
...
- [ ] Runtime evidence and proposed authority/business changes are not conflated.

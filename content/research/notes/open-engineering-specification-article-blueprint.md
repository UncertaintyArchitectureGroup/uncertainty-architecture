...
- **Section 5.3** — take one stable refund-authority boundary and expose the four capability families as parallel control functions around that boundary: Constraint + Constraint Realization, Sensors, Controller / decision functions operating within delegated authority, and Actuators. Keep decision-authority ownership outside the Controller definition: where substantive judgment is reserved, show Human Authority or the applicable horizon owner as the holder of that authority or escalation destination. Show why a policy sentence is not yet a complete control path without implying an execution pipeline.

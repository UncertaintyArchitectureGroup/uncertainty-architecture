...
**External-review reconciliation (2026-08-24).** Substantive pre-publication review by **Maximiliano Armesto** changed the editorial contract in four bounded ways without promoting the review to framework authority or endorsement:

- the publication-facing binary no longer uses **Linear Software**; it describes the opposite side of the category boundary through explicitly authored consequential responsibilities, while the draft-normative glossary remains unchanged pending a separate framework-level terminology decision;
- the systems covered by the category definition are stated as predating LLMs, while LLMs remain the practical trigger and primary contemporary focus because they made model-mediated judgment more general-purpose, accessible, and widespread;
- category membership, consequence severity, and required control depth are separated early through one intentionally low-consequence case;
- Article §4 carries one direct STAMP/STPA positioning paragraph after the four-horizon model is introduced, while the full bidirectional comparison remains later research across STAMP/STPA, Simplex/runtime assurance, control-theoretic work, production ML/software engineering, risk and management systems, and implementation approaches and platforms.

The detailed review provenance and decision record lives in [`thinking-systems-external-review-maximiliano-armesto.md`](thinking-systems-external-review-maximiliano-armesto.md).
...
→ systems covered by the category predate LLMs; earlier probabilistic systems may qualify when at least one Consequential Runtime Responsibility depends partly on their Model Judgment
→ LLMs are the practical trigger and expansion point because they made model-mediated interpretation, generation, routing, planning, and action selection far more general-purpose, accessible, and widespread
→ category membership, consequence severity, and required control depth remain separate; one intentionally low-consequence case makes proportionality visible before the complete map appears
...
**Historical-scope and early-proportionality rule.** The category definition must state early that model-mediated consequential responsibility predates LLMs. LLMs are the practical expansion point and the paper's primary contemporary focus, not the historical origin of the category. Include one intentionally low-consequence case—such as an internal summarizer materially influencing a reversible human review—and keep three questions separate: category membership, consequence severity, and required implementation depth. This clarification must not expand the article into a general survey of every probabilistic system.
...
**Antecedent-positioning rule.** After Article §4 introduces the four-horizon model, include one direct paragraph acknowledging that STAMP/STPA already supports hierarchical socio-technical control structures extending into management and regulatory authority. The paper must not imply novelty for that generic control perimeter. The narrower candidate contribution is whether, for model-judgment-dependent software, the four-horizon model adds practitioner value by explicitly separating ownership of technical viability, business and authority authorization, release scope, runtime correction, and reassessment when the basis of operation becomes invalid. Treat that as a hypothesis to demonstrate, narrow, or reject. The complete comparison remains later, broad, and bidirectional; the publication adaptation must not compress that substantial analysis into a superficial page of claims.
...
- fixed, linear, iterative, adaptive, or dynamically selected orchestration does not determine whether a consequential responsibility remains explicitly authored or depends partly on Model Judgment;
...
- Show a simple classification boundary: ask whether any **Consequential Runtime Responsibility** depends partly on probabilistic Model Judgment. Route **No** to the relevant consequential responsibility remaining explicitly authored and **Yes** to Thinking System. Show orchestration topology, autonomy, and delegated authority as independent dimensions that affect architecture, risk, and control demand but do not decide category membership.
...
- Describe the explicitly authored side of the responsibility boundary using the canonical responsibility boundary: no Consequential Runtime Responsibility depends partly on probabilistic Model Judgment; consequential responsibilities, if any, are fulfilled entirely through explicitly encoded logic.

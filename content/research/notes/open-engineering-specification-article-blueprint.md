...
| Relationship ID | Material relationship | Source / scope / assumptions | Guarantee strength where applicable | Materiality rationale | Active carrier or carrier composition | Owner / authority | Evidence, decision, action, and verification | Reassessment trigger |
|---|---|---|---|---|---|---|---|---|
...
The retained full working mapping need not enumerate immaterial implementation detail, but it must cover every material operating-contract, boundary, authority, evidence, action, verification, and reassessment relationship and preserve its active version, source/scope/assumptions, guarantee semantics where applicable, and materiality decisions. Representative publication rows support only relationship-level illustration; they cannot by themselves support a whole-controlled-object substitution conclusion.
...
- [ ] **Article §5** produces both the authority-variant comparison and a complete canonical material-relationship-to-carrier working mapping that **Article §6** can consume without rerunning the carrier analysis; the mapping has a stable ID/version, controlled-object/lifecycle baseline, source/authorization baseline, scope, assumptions, materiality decision rule, guarantee semantics where applicable, and materiality rationales for inclusion/exclusion. The mapping header does not contain a self-referential repository SHA; the substitution run/verdict records an immutable snapshot reference after freeze.

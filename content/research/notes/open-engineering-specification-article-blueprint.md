...
For a simpler candidate, Project determines whether it can satisfy the same Organizationally authorized outcome and remain inside the standing authority/business basis. If yes, Project / Architecture may select the deterministic/manual/narrower technical design as an architecture decision and immediately confirm the selected design's category. If no Consequential Runtime Responsibility remains materially dependent on Model Judgment, the selected design **exits this Thinking-System lifecycle**; if a narrower model-assisted design still materially informs a consequential responsibility, it remains a Thinking System and the map is reapplied to that narrower candidate. Organization is reactivated only when the engineering recommendation requires changing an Organizationally owned premise—such as the intended outcome, authority, price, segment, service promise, funding, or investment basis—or when the business decision is to defer or stop.
...
- State explicitly that Project may identify a candidate simpler design/category result, but Project / Architecture selects the technical/design path within the standing Organizational business/authority basis; Organization changes the business/design basis when needed; a selected non-Thinking-System design exits this lifecycle only after Project confirms that the selected design no longer meets the category test.
...
- [ ] `Simpler alternative preferred` is a Project viability conclusion and reaches Organization before lifecycle exit. Project / Architecture selects the technical/design path within the standing Organizational business/authority basis; Organization changes the business/design basis when needed. If it selects a deterministic/manual or other simpler path, Project confirms the **selected design's** category; exit occurs only when no Consequential Runtime Responsibility remains materially dependent on Model Judgment, while a narrower model-assisted design stays inside the category when the dependency persists.

...
A publication adaptation draft also remains a research note until it is actually published. It may compress, omit, or reorder a bounded argument from a larger living paper, but must identify its derivation and must not silently become a second canonical source for framework meaning. Claim-bearing definitions, scope boundaries, ownership semantics, and maturity caveats should remain as close as practical to the owning manuscript wording rather than being freely paraphrased for style. After external publication, immediately preserve the resulting exact **content edition** under [`../publications/`](../publications/) before feedback-driven reconciliation, with publication URLs and immutable source identity. Equivalent Medium, LinkedIn, website, or other platform copies may be recorded as renditions of that same content edition; platform-only formatting or image substitutions do not require separate repository editions unless the substantive text materially diverges.
...
- [`thinking-systems-publication-draft.md`](thinking-systems-publication-draft.md) — publication-facing adaptation titled *Thinking Systems: When the Controlled Object Changes*. It is intentionally aligned closely to the merged long-form §§1–4: it preserves the dominant-uncertainty/category/controlled-object deduction and major explanatory figures, compresses bounded-control detail, and reduces the later lifecycle treatment to the question of how far the socio-technical control perimeter can reach. It is intended to expose that argument to external criticism before the larger paper is completed.

...
- Pull-request declarations, notable-change companion updates, research-state traceability, maintained-path deletion or rename decisions, explicit `agent_assistance` applicability, and high-impact classification have a diff-aware validation foundation. Change coupling and agent checkpoint share one PR-owned diff definition based on the merge-base of the current target tip and PR head; target-only changes synchronized into a branch are not reclassified as PR-owned changes. The protected repository-policy surface includes `.github/`, `AGENTS.md`, `CONTRIBUTING.md`, and `DOCUMENT-METADATA.md`; those paths force `change_class: repository-policy`. Maintained Markdown remains high-impact when either the current-target or candidate version has `status: draft-normative` or `status: normative`, so a same-PR status downgrade cannot weaken the control. Existing open human-authored PRs created before the applicability field migrate by adding it on their next maintained iteration rather than being silently grandfathered.
- AI-assisted pull-request iterations have a deterministic **checked-state checkpoint** bound to the PR diff-base identity, current target tip, head, tested merge result, PR-description digest, trusted PR-review/inline-review feedback digest, and effective scoped `AGENTS.md` blobs. `agent_assistance: none` is a target-branch CODEOWNER-controlled **head-bound** opt-out and the marker author must remain a current `write` / `maintain` / `admin` collaborator with trusted GitHub association. High-impact AI-assisted work remains Draft while repository-changing work is active. Leaving Draft requires a fresh checkpoint and a current-authority CODEOWNER approval bound to exact head, tested merge SHA, PR-body digest, and checkpoint fingerprint; the approval must exist and remain unedited before Ready. Durable readiness is verified from the expected job and success step of the protected `.github/workflows/change-coupling.yml` pull-request run attached to the PR head, with paginated check-run discovery rather than a same-named check alone. Failed or premature Ready transitions cannot be legalized retroactively by later checkpoint/comment edits; same-head review feedback may stale the checkpoint without revoking readiness, while a substantive PR-body edit, later head, changed target tip/tested-merge state, or new Ready cycle requires fresh authorization. `ua-exception/pr-contract` bypasses only the malformed/missing declaration and leaves agent controls active with safe assisted defaults.
...
- Repository code contributions now have scoped human and AI guidance: `CONTRIBUTING.md` owns the general code contract, `.github/AGENTS.md` and `quartz/AGENTS.md` carry directory-specific agent rules, and `quartz/README.md` maps the maintained fork, upstream baseline, ownership, execution, and test architecture. Changed repository-owned code receives diff-bounded formatting or Python syntax validation; TypeScript static analysis, split Quartz/publication regressions, and publication-impact routing are enforced in Build Integrity.
...
- [x] Establish scoped code-contribution guidance, a reproducible Quartz/publishing architecture map, protected formatter configuration, alias-safe changed-code validation, TypeScript static analysis, split regression suites, and test-backed publication-impact routing.
...
The [bounded initial comparison](.github/tests/repository_intelligence/AB-EVALUATION.md) uses 12 independently selected real tasks / 24 fresh sessions, ordinary live-GitHub Control and normal optional compact-context Treatment. Protocol v11 keeps same-state/isolation checks, blind scoring and measured costs while reducing evidence to study, runs and scores; the evaluator generates source-pinned bootstrap messages, self-contained blind scoring packets and four-outcome reports. It replaces the unexecuted v10 two-wave design. PR #119 has merged. The [separate descriptive study](.github/tests/repository_intelligence/studies/2026-09-17-initial/README.md) uses 12 maintainer-approved, assistant-prepared tasks with frozen prompts and expectations. Collection is interrupted at 20/24 original attempts plus 2 supplemental attempts, with refusals, assisted recovery and all missing slots preserved. Its [unblinded organizer analysis](.github/tests/repository_intelligence/studies/2026-09-17-initial/ANALYSIS.md) finds useful repository explanations in both conditions, with incremental RI benefit inconclusive. Actual RI exposure, model parity, isolation and costs are not established; this is not valid v11 benefit/cost evidence or proof of deep conceptual transfer. Retain optional RI and direct-owner fallback without expansion on this evidence. Independent blind scoring and full RI-EVAL scenario acceptance remain open. No more sessions are requested under the same collection limitations; any decision-specific follow-up requires a separate frozen study.

A separately frozen [forced RI-inventory pilot](.github/tests/repository_intelligence/studies/2026-09-18-inventory-pilot/RESULTS.md) now records four new tasks / eight fresh native-agent sessions with guaranteed inventory supply in B, independently authored expectations and arm-hidden scoring. All four quality comparisons tie (2/2, 1/1, 2/2, 2/2), with no serious errors and no provisional benefit signal. Recorded reader responses plus initial messages are about 7.4% larger in B; actual tokens, latency and monetary costs remain unmeasured. Production guidance is unchanged: the tested 7,126-byte inventory has not justified mandatory loading. This is a narrow inventory result with configured model equality and instructional isolation, not full-projection/graph acceptance, statistical equivalence or a completion of the earlier study.
...
- diff-aware changelog, glossary, roadmap, research-traceability, deletion, rename, compatibility, pull-request declaration, derived repository-policy/document-status classification, and `agent_assistance` validation over a shared PR-owned current-target merge-base diff;
- a checked-state AI-agent iteration checkpoint that rejects stale attestations, resolves effective root/nested `AGENTS.md` from the tested merge state, incorporates trusted review/inline-review feedback on the PR lifecycle, requires current-authority head-bound CODEOWNER evidence before a human-only `none` opt-out, and enforces Draft/readiness through CODEOWNER approval bound to head + tested merge + PR-body digest + checkpoint fingerprint followed by paginated verified readiness workflow/job/step provenance on the PR head; retroactive edits to pre-Ready comments do not authorize Ready, and repository-contract registration is mutation-protected rather than self-tested only by direct extension loading;
- a target-owned trusted-base agent guard that uses `pull_request_target` without candidate checkout/execution, posts a head status, and on `main` pushes revalidates open PRs for target-tip, high-impact classification, current CODEOWNER authority, and checkpoint freshness; its first bootstrap PR is explicitly reviewed before target-owned enforcement becomes available;
...
- diff-bounded code formatting, changed Python syntax validation, TypeScript static analysis, separate Quartz/publication test commands, scoped agent instructions for `.github/` and `quartz/`, and protected publication-impact routing that excludes non-executable Quartz guidance from expensive render jobs;
...
- independent mutation fixtures proving that protected structural, metadata, research-register, change-coupling, agent-checkpoint, trusted-base-guard, and supply-chain regressions fail predictably;
...
- make `Agent protocol / trusted-base guard` and `Agent protocol / checked-state checkpoint`, and where appropriate `Agent protocol / readiness authorization`, required merge checks through a repository ruleset or equivalent branch protection; require current-base integration before merge so a stale or missing target-owned guard/checkpoint cannot be bypassed by ordinary merge flow;

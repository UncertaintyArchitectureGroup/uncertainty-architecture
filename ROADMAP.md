...
- Repository code contributions now have an explicit clarity, comment, structure, failure, and regression-test contract; the Quartz-derived core, exact upstream baseline, reproducible root/local delta, UA publishing boundary, execution paths, and test layers are mapped; changed repository-owned code receives upstream-compatible, alias-safe bounded formatting or Python-syntax validation; renderer and publication-impact paths are detected through a NUL-safe tested contract; and TypeScript static analysis plus Quartz and UA publication regressions share stable package commands and Build Integrity coverage.
...
- [x] Establish repository code-contribution guidance, a reproducible Quartz/publishing architecture and test map, protected upstream-compatible formatting, alias-safe incremental changed-code quality validation, complete test-backed publication-impact routing, and combined Quartz/publication regression commands in Build Integrity.

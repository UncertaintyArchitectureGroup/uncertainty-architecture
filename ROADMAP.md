...
- Pull-request declarations, notable-change companion updates, research-state traceability, maintained-path deletion or rename decisions, and explicit `agent_assistance` applicability have a diff-aware validation foundation; open human-authored PRs created before the applicability field migrate by adding it on their next maintained iteration rather than being silently grandfathered.
- AI-assisted pull-request iterations have a deterministic **checked-state checkpoint**: it binds the attestation to the PR diff-base, current target tip, head, tested merge result, PR-description digest, trusted PR-review/inline-review feedback digest, and effective scoped `AGENTS.md` blobs for PR-owned changed paths. Target-only changes already synchronized into the branch do not expand agent scope; high-impact AI-assisted change classes are Draft-gated while repository-changing work is active; and a GitHub `ready_for_review` transition binds readiness to the then-current head so later review feedback may stale the checkpoint without revoking readiness while a later repository commit requires Draft again. Top-level PR conversation comments remain semantic feedback but are not represented as immediate PR-head status signals because GitHub emits `issue_comment` workflows on the default-branch ref/SHA.
...
- [x] Establish a deterministic AI-agent checked-state checkpoint with explicit human-versus-agent applicability, PR-owned diff scoping from current-target merge-base, tested-merge instruction resolution, PR-description and trusted PR-review/inline-review feedback watermarks, current-head readiness/Draft enforcement for high-impact AI-assisted changes, trusted GitHub review/inline-review triggers, coherent PR-contract exception handling, explicit migration for pre-existing open PRs, and mutation/regression protection of the stable control surface.
...
- a checked-state AI-agent iteration checkpoint that rejects stale attestations, derives PR-owned scope from current-target merge-base, resolves effective root/nested `AGENTS.md` from the tested merge state, incorporates trusted review/inline-review feedback on the PR lifecycle, derives current-head readiness from GitHub timeline events, and enforces Draft working state for AI-assisted repository-policy and normative-class changes without treating top-level `issue_comment` events as PR-head status checks;

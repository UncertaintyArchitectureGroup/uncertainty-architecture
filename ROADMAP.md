...
The [bounded initial comparison](.github/tests/repository_intelligence/AB-EVALUATION.md) uses 12 independently selected real tasks / 24 fresh sessions, ordinary live-GitHub Control and normal optional compact-context Treatment. Protocol v11 keeps same-state/isolation checks, blind scoring and measured costs while reducing evidence to study, runs and scores; the evaluator generates source-pinned bootstrap messages, self-contained blind scoring packets and four-outcome reports. It replaces the unexecuted v10 two-wave design. PR #119 has merged. The [separate experiment preparation record](.github/tests/repository_intelligence/studies/2026-09-17-initial/README.md) retains two unscored client rehearsals: the first reports unavailable GitHub access and web-search substitution; the second reports GitHub reads with unverified RI and source fallback. The shared ChatGPT record identifies the submitted prompt and page-reported model, but redacts connector outputs. Connector setup is now maintainer-confirmed and the next handoff is selection of concrete tasks; source/delivery/cost capture, model/settings parity and Memory-disabled isolation remain unresolved. Any descriptive comparison using incomplete capture needs an explicit scope decision and cannot silently become a valid v11 benefit verdict; no primary sessions or scores are recorded yet. Full RI-EVAL scenario acceptance remains open. Additional evidence follows a frozen decision-specific rule rather than an obligatory second wave.

...
- Repository code contributions now have an explicit clarity, comment, structure, failure, and regression-test contract; the Quartz-derived core, exact upstream baseline, local delta, UA publishing boundary, execution paths, and test layers are mapped; changed repository-owned code receives bounded formatting or Python-syntax validation; and TypeScript static analysis plus Quartz and UA publication regressions share stable package commands and Build Integrity coverage.

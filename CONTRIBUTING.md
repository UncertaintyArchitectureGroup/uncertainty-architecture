...
Every human-authored pull request declares `agent_assistance` as `used` or `none` in `ua-change-contract`. This is an applicability declaration, not a quality judgment. `none` is a maintainer-controlled, **head-bound** opt-out, not a self-attested bypass: except for the Dependabot compatibility path, it disables the checkpoint only after a target-branch global CODEOWNER who still has current `write`, `maintain`, or `admin` repository permission and trusted GitHub association adds the structured `ua-agent-assistance-none` approval for the exact current head using the marker defined in [`.github/pull_request_template.md`](.github/pull_request_template.md). Its controlled fields and values are owned by [`.github/policy/agent-checkpoint-contract.json`](.github/policy/agent-checkpoint-contract.json). A later repository head invalidates the prior opt-out. Existing open human-authored PRs created before the field was introduced must add it on their next maintained iteration; this is an explicit compatibility migration.
...
4. Declare whether `agent_assistance` was materially `used` or `none`; `none` becomes effective only after the maintainer-controlled, head-bound, current-authority opt-out evidence using the marker defined in [`.github/pull_request_template.md`](.github/pull_request_template.md) and controlled by [`.github/policy/agent-checkpoint-contract.json`](.github/policy/agent-checkpoint-contract.json).

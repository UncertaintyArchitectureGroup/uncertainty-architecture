...
- Added a research publication-adaptation cycle for exposing a bounded standalone argument before completion of a larger living paper when early external criticism can improve the remaining research. The workflow keeps the blueprint/long-form manuscript as the owning pair, develops the shorter adaptation under `content/research/notes/`, requires claim-bearing adaptation prose to prefer direct compression/reordering of the owning wording over fresh semantic paraphrase, routes classified material feedback back into the owning research, and immediately freezes the exact externally published **content edition** under `content/research/publications/` before feedback-driven reconciliation. The preserved edition records publication date, one principal canonical URL, equivalent platform-rendition URLs, and immutable source identity; substantive later changes require a new edition or explicit material-transformation record. The first adaptation, *Thinking Systems: When the Controlled Object Changes*, is aligned closely to the merged long-form Sections 1–4, preserves the category/controlled-object deduction and major explanatory figures, compresses bounded-control and lifecycle detail around the socio-technical control-perimeter thesis, and records explicit provenance for the `Thinking Systems` formulation through Vitalii Oborskyi's exchange with Arkadiy Dobkin while separating that provenance from the UA-specific engineering definition and from endorsement or co-authorship.

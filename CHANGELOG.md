...
- Reopened the project/organization lifecycle-ownership question as explicit research rather than silently changing status-bearing UA semantics: the article now tests **Project viability conclusion → Organizational Business Authorization → scoped Project Authorization**, including **research-only** authorization for bounded experiments before production viability is established, **production-capable** authorization for viable production paths, explicit category exit when a simpler design leaves no Consequential Runtime Responsibility dependent on Model Judgment, and direct Organizational reassessment when the business basis changes. `content/research/framework-traceability.md` records the refinement as **Needs Resolution** pending deliberate framework review.
- Consolidated the public-article research workflow around the living blueprint and target manuscript after rechecking the former Phase 1 completion and operational-extension planning notes. The operational-extension review transferred two concerns that were only partially represented: reconstructability of the material active authorization/behavioral-control baseline across source, project, delivery, realization, model/configuration, evaluator, deployment, and fallback state; and explicit evidence that fallback/recovery paths are sufficiently independent from relevant primary/common-mode failures, available at required capacity/latency, transition correctly, and can restore an authorized state. Evidence-instrument validity/calibration was already present and remains explicit. The redundant intermediary note paths were then removed without changing the underlying research-claim state represented by those two retired notes; Git history preserves both records, and the research-notes index points to the two active article artifacts.

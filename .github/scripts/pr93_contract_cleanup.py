...
def main_validator_from_current_main() -> str:
...
def update_validator() -> None:
    ...
    text = main_validator_from_current_main()
    ...
    text = replace_once(text, old, new, "validator critical-file hook")
...
def update_contract() -> None:
    ...
    package_rule["required_text"] = [
        marker
        for marker in package_rule.get("required_text", [])
        if not marker.startswith('"pdf') and not marker.startswith('"publication:')
    ]
    package_rule["required_json"] = [
        {"pointer": f"/scripts/{name}", "equals": value}
    ...
    critical["quartz/scripts/render-copy-ready.test.mjs"]["required_text"] = [
        "copy-ready HTML embeds local images as data URIs",
        "copy-ready document exposes one article-only copy surface",
        "Medium upload plan is hero plus nine figures in deterministic order",
    ]
    critical["quartz/scripts/verify-publication-package.mjs"]["required_text"] = [
        ...
        "medium/upload/00-medium-hero.png",
        "copy-ready HTML contains obsolete copy controls",
    ]
    critical["quartz/scripts/verify-publication-package.test.mjs"]["required_text"] = [
        ...
        "Medium upload manifest requires ten ordered images plus instructions",
        "Medium upload manifest rejects any claim that clipboard images are supported",
    ]
...
def update_cases() -> None:
    ...
    by_name["explicit generic PDF entrypoint deletion is rejected"].update(
        {
            "mutation": {
                "type": "delete_json_pointer",
                "path": "package.json",
                "pointer": "/scripts/pdf",
            },
            "expected_error": "package.json: JSON pointer '/scripts/pdf'",
        }
    )
    by_name["curated working-paper command deletion is rejected"].update(
        {
            "mutation": {
                "type": "delete_json_pointer",
                "path": "package.json",
                "pointer": "/scripts/pdf:working-paper",
            },
            "expected_error": "package.json: JSON pointer '/scripts/pdf:working-paper'",
        }
    )
    cases.extend(
        [
            {
                "name": "package script JSON formatting is ignored",
                "mutation": {
                    "type": "format_json_compact",
                    "path": "package.json",
                },
                "expected_valid": True,
            },
            {
                "name": "package script moved outside scripts is rejected",
                "mutation": {
                    "type": "move_json_pointer",
                    "path": "package.json",
                    "pointer": "/scripts/publication:bundle",
                    "destination": "/publication:bundle",
                },
                "expected_error": "package.json: JSON pointer '/scripts/publication:bundle'",
            },
        ]
    )
...
def update_contract_tests() -> None:
    ...
    text = replace_once(text, old_tail, new_tail, "contract test mutations")
    path.write_text(text, encoding="utf-8")


def remove_failed_transport_assets() -> None:
    import shutil

    shutil.rmtree(
        ROOT / "content/research/notes/thinking-systems-platform-assets",
        ignore_errors=True,
    )
...
if __name__ == "__main__":
    ...
    remove_failed_transport_assets()

...
def eco_select(items, seed):
    ...
    req(
        len(exact) >= 2 and len(new) >= 2,
        "ecological pool needs at least two anchors of each required type",
    )
    exact = sorted(
        exact,
        key=lambda item: rank(seed, "exact-owner", item["ecological_source_id"]),
    )[:2]
    new = sorted(
        new,
        key=lambda item: rank(
            seed, "legitimate-new-artifact", item["ecological_source_id"]
        ),
    )[:2]
...
def envelope(p, wave, task_id, arm, prompt_sha):
    return {
        ...
        "task_id": task_id,
        "arm": arm,
        "treatment_delivery_mode": (
            p["treatment_delivery"]["mode"] if arm == TREATMENT else "not-applicable"
        ),
...
def validate_randomization_provenance(p):
    ...
    req(
        provenance.get("method") == "independent_assessor_after_pool_commitment",
        "randomization provenance method invalid",
    )
    req(
        isinstance(provenance.get("pool_commitment_reference"), str)
        and provenance["pool_commitment_reference"],
        "pool_commitment_reference required",
    )
    req(
        provenance.get("pool_commitment_sha256")
        == p["ecological_source_pool"]["normalized_eligible_pool_sha256"],
    ...
    for name in ("ecological_selection", "pilot_arm_order", "confirmatory_arm_order"):
        ...
        for field in ("independent_assessor", "generation_reference"):
            req(
                ...
                f"randomization provenance {name}.{field} required",
            )
        req(
            entry.get("generated_after_pool_commitment") is True,
            f"randomization provenance {name} must postdate pool commitment",
        )
        req(
            entry.get("single_generation_attested") is True,
            f"randomization provenance {name} must attest one generation",
        )
...
def validate_prereg(p):
    ...
    req(
        p.get("execution_policy") == "always_run_pilot_and_confirmatory",
        "both primary waves must run",
    )
    lock = p.get("source_state_lock")
    req(
        isinstance(lock, dict)
        and lock.get("mode") == "stable_default_branch_window"
        and lock.get("expected_default_branch_tip_sha") == p["repository_ref"],
        "invalid source_state_lock",
    )
    for field in (
        "repository_mutations_prohibited_during_primary",
        "pre_run_tip_check_required",
        "post_run_tip_check_required",
    ...
    req(
        isinstance(preflight.get("smoke_evidence_reference"), str)
        and preflight["smoke_evidence_reference"],
        "smoke_evidence_reference required",
    )
    ...
    req(
        ...
        and delivery.get("mode")
        in {"connector_compact_surface", "local_cli", "dedicated_adapter"},
        "invalid Treatment Delivery Mode",
    )
    identity = delivery.get("aid_identity")
    req(
        isinstance(identity, dict) and identity.get("mode") == delivery["mode"],
        "Treatment aid identity required",
    )
    for field in ("identifier", "git_blob_sha", "content_sha256", "source_identity"):
        req(
            isinstance(identity.get(field), str) and identity[field],
            f"Treatment aid identity {field} required",
        )
    req(
        set(p.get("mandatory_scenario_families", [])) == MANDATORY,
        "mandatory scenario family set mismatch",
    )
    req(
        set(p.get("confirmatory_required_scenario_families", [])) >= CRITICAL,
        "confirmatory critical coverage set incomplete",
    )
    for key in ("pilot", "confirmatory"):
        wave = p.get(key)
        req(
            isinstance(wave, dict)
            and wave.get("case_count") == 12
            and wave.get("stress_cases") == 6
            and wave.get("ecological_cases") == 6,
            f"{key} must be 6+6",
        )
    ...
    req(
        isinstance(eco, dict) and eco.get("mode") == "combined_pool",
        "v9 requires one combined ecological pool",
    )
    ...
    req(
        p.get("prompt_pack_schema_version") == 3
        and p.get("scoring_key_schema_version") == 3,
        "v9 requires schema version 3",
    )
    req(
        p.get("orientation_cost_boundary") == "task_orientation_events_only",
        "orientation cost boundary invalid",
    )
    req(
        p.get("claim_boundary")
        == "engineering acceptance; not statistical significance or universal productivity proof",
...
def validate_treatment_surface(path, p):
    ...
    req(
        isinstance(source, dict) and source.get("digest") == identity["source_identity"],
        "Treatment surface source_identity mismatch",
    )
    return {
        "content_sha256": s256b(payload),
...
def validate_pool(path, p, seed):
    req(
        s256f(path) == p["ecological_source_pool"]["normalized_eligible_pool_sha256"],
        "ecological pool hash mismatch",
    )
    req(
        s256t(seed) == p["ecological_source_pool"]["selection_seed_sha256"],
        "ecological selection seed commitment mismatch",
    )
    pool = load(path, "ecological pool")
    items = pool.get("items")
    req(
        pool.get("schema_version") == 1 and isinstance(items, list) and len(items) >= 24,
        "invalid ecological pool",
    )
    source_ids, family_ids, prompts = set(), set(), set()
    for item in items:
        req(
            isinstance(item, dict)
            and item.get("anchor_type")
            in {"exact-owner", "legitimate-new-artifact", "none"},
            "invalid ecological pool item",
        )
        for field in ("ecological_source_id", "task_family_id", "normalized_prompt"):
            req(
                isinstance(item.get(field), str) and item[field],
                f"ecological pool {field} required",
            )
        scenarios = item.get("scenario_families")
        req(
            isinstance(scenarios, list) and all(value in MANDATORY for value in scenarios),
            "ecological pool scenario_families invalid",
        )
        if item["anchor_type"] == "exact-owner":
            req(
                "exact-owner-recovery" in scenarios,
                "exact-owner pool anchor must exercise exact-owner-recovery",
            )
        if item["anchor_type"] == "legitimate-new-artifact":
            req(
                "legitimate-new-artifact" in scenarios,
                "new-artifact pool anchor must exercise legitimate-new-artifact",
            )
...
def validate_pack(path, expected_hash, wave_name, seed):
    ...
    req(
        ...
        and pack.get("wave") == wave_name
        and isinstance(cases, list)
        and len(cases) == 12,
        f"{wave_name} prompt pack invalid",
    )
    out = {
        "ids": [],
        "classes": {},
    ...
    for case in cases:
        ...
        req(
            isinstance(task_id, str) and task_id and task_id not in out["ids"],
            f"{wave_name} task_id invalid",
        )
        req(
            corpus in {"stress", "ecological"}
            and isinstance(prompt, str)
            and prompt.strip()
            and isinstance(family, str)
            and family,
            f"{wave_name} case invalid",
        )
        req(
            isinstance(scenarios, list) and all(value in MANDATORY for value in scenarios),
            f"{wave_name} scenarios invalid",
        )
        req(
            (corpus == "ecological" and isinstance(source, str) and source)
            or (corpus == "stress" and source is None and scenarios),
            f"{wave_name} source/scenario contract invalid",
        )
    ...
    req(
        list(out["classes"].values()).count("stress") == 6
        and list(out["classes"].values()).count("ecological") == 6,
        f"{wave_name} must be 6+6",
    )
    return out
...
def validate_key(path, expected_hash, wave_name, ids):
    ...
    cases = key.get("cases")
    req(
        key.get("schema_version") == 3
        and key.get("wave") == wave_name
        and isinstance(cases, list),
        f"{wave_name} scoring key invalid",
    )
    out = {}
    for case in cases:
        task_id = case.get("task_id")
        ...
        req(
            type(case.get("companion_validation_applicable")) is bool,
            f"{task_id} companion applicability missing",
        )
        for field in (
            "expected_owner_or_route",
            "required_authoritative_evidence",
            "serious_error_conditions",
            "decision_acceptance_conditions",
        ):
            req(
                isinstance(case.get(field), list) and case[field],
                f"{task_id} scoring key {field} must be non-empty",
            )
        req(
            isinstance(case.get("acceptable_alternatives"), list),
            f"{task_id} acceptable_alternatives must be list",
        )
        anchors = case.get("dimension_anchors")
        req(isinstance(anchors, dict), f"{task_id} dimension_anchors missing")
        dimensions = set(CORE_SCORE_FIELDS) | (
            {OPTIONAL_SCORE_FIELD} if case["companion_validation_applicable"] else set()
        )
        ...
        for dimension, levels in anchors.items():
            req(
                ...
                and set(levels) == {"0", "1", "2"}
                and all(isinstance(value, str) and value for value in levels.values()),
                f"{task_id} {dimension} anchors invalid",
            )
        out[task_id] = case
...
def verify_evidence(p, paths, seeds):
    pool_items, selected = validate_pool(
        paths["ecological_pool"], p, seeds["ecological_selection"]
    )
    ...
    for key, wave_name in (("pilot", "PILOT"), ("confirmatory", "CONFIRMATORY")):
        req(
            s256t(seeds[key + "_arm"]) == p[key]["arm_order_seed_sha256"],
            f"{wave_name} arm seed commitment mismatch",
        )
        packs[key] = validate_pack(
            paths[key + "_prompts"],
            p[key]["prompts_sha256"],
            wave_name,
            seeds[key + "_arm"],
        )
        keys[key] = validate_key(
            paths[key + "_key"], p[key]["scoring_key_sha256"], wave_name, packs[key]["ids"]
        )
        ...
        for task_id in packs[key]["ids"]:
            ...
            req(
                packs[key]["families"][task_id] == item["task_family_id"],
                f"{wave_name} selected task family differs from pool",
            )
            req(
                packs[key]["prompt_text"][task_id] == item["normalized_prompt"],
                f"{wave_name} selected prompt differs from frozen normalized pool",
            )
            req(
                packs[key]["scenarios"][task_id] == item["scenario_families"],
                f"{wave_name} selected scenarios differ from frozen pool",
            )
        eco_scenarios = {
            scenario
            for task_id, scenarios in packs[key]["scenarios"].items()
            if packs[key]["classes"][task_id] == "ecological"
            for scenario in scenarios
        }
        req(ANCHORS <= eco_scenarios, f"{wave_name} ecological half misses required anchors")
    req(
        not set(packs["pilot"]["families"].values())
        & set(packs["confirmatory"]["families"].values()),
        "task_family_id overlaps across waves",
    )
    pilot_sources = {source for source in packs["pilot"]["sources"].values() if source}
    confirm_sources = {
        source for source in packs["confirmatory"]["sources"].values() if source
    }
    ...
    all_scenarios = {
        ...
        for scenarios in pack["scenarios"].values()
        for scenario in scenarios
    }
    req(MANDATORY <= all_scenarios, "held-out corpora miss mandatory scenarios")
    confirm_scenarios = {
        scenario
        for scenarios in packs["confirmatory"]["scenarios"].values()
        for scenario in scenarios
    }
    req(
        set(p["confirmatory_required_scenario_families"]) <= confirm_scenarios,
        "Confirmatory misses critical scenarios",
    )
    return {
        "packs": packs,
        "keys": keys,
...
def score_total(scores, key):
    req(isinstance(scores, dict), "scores required")
    values = {}
    for field in CORE_SCORE_FIELDS:
        req(
            type(scores.get(field)) is int and 0 <= scores[field] <= 2,
            f"{field} score invalid",
        )
        values[field] = scores[field]
    companion = scores.get(OPTIONAL_SCORE_FIELD)
    if key["companion_validation_applicable"]:
        req(
            type(companion) is int and 0 <= companion <= 2,
            "companion_validation must be scored",
        )
    ...
    req(
        scores.get("total_applicable_correctness") == total,
        "total_applicable_correctness mismatch",
    )
    req(
        type(scores.get("serious_routing_error")) is bool,
        "serious_routing_error must be boolean",
    )
...
def validate_event(event, index, p):
    req(
        ...
        and event.get("phase") in {"study_infrastructure", "task_orientation"}
        and event.get("resource_class") in RESOURCE_CLASSES
        and event.get("repository") == p["repository"],
        "invalid tool event",
    )
    ...
    req(
        event.get("response_bytes") is None
        or type(event["response_bytes"]) is int
        and event["response_bytes"] >= 0,
        "invalid response_bytes",
    )


def compact_surface_coverage(events, p, treatment):
    if p["treatment_delivery"]["mode"] != "connector_compact_surface":
        return bool(events)
    ...
    for event in events:
        req(
            event.get("content_identity") == p["treatment_delivery"]["aid_identity"],
            "Treatment compact-surface event has wrong identity",
        )
        start = event.get("payload_byte_start")
        end = event.get("payload_byte_end")
        chunk_sha = event.get("payload_chunk_sha256")
        req(
            type(start) is int and type(end) is int and 0 <= start < end <= size,
            "Treatment compact-surface byte range invalid",
        )
        req(valid_sha256(chunk_sha), "Treatment compact-surface chunk SHA-256 required")
        req(
            event.get("response_bytes") == end - start,
            "Treatment compact-surface response_bytes must equal byte range length",
        )
        req(
            s256b(payload[start:end]) == chunk_sha,
            "Treatment compact-surface chunk bytes do not match exact surface",
        )
...
def derive(run, p, treatment):
    ...
    req(isinstance(events, list), "tool_events must be list")
    req(
        run.get("instrumentation_source") in INSTRUMENTATION_SOURCES,
        "instrumentation_source must be machine_capture or exported_transcript",
    )
    req(
        isinstance(run.get("raw_evidence_reference"), str) and run["raw_evidence_reference"],
        "raw_evidence_reference required",
    )
    req(valid_sha256(run.get("raw_evidence_sha256")), "raw_evidence_sha256 required")
    req(
        isinstance(run.get("event_extractor_version"), str) and run["event_extractor_version"],
        "event_extractor_version required",
    )
    req(
        run.get("event_log_sha256") == csha(events),
        "event_log_sha256 does not match structured events",
    )
    ...
    repo_bytes = (
        None
        if any(event.get("response_bytes") is None for event in task)
        else sum(event["response_bytes"] for event in task)
    )
    ri_bytes = (
        None
        if any(event.get("response_bytes") is None for event in ri)
        else sum(event["response_bytes"] for event in ri)
    )
    exact_identity = [
        event
        for event in ri
        if event.get("content_identity") == p["treatment_delivery"]["aid_identity"]
    ]
    complete = (
        compact_surface_coverage(compact, p, treatment)
        if p["treatment_delivery"]["mode"] == "connector_compact_surface"
        else bool(exact_identity)
    )
    return {
        "calls": len(task),
        ...
        "bad_control": any(event["resource_class"] in RI_CLASSES for event in task),
...
def run_valid(run, arm, wave, task_id, prompt_text, prompt_hash, order, p, treatment):
    ...
    checks = {
        ...
        "submitted full message hash mismatch": run.get("submitted_full_message_sha256")
        == s256t(expected_message),
        ...
        "corrective feedback exposed": run.get("corrective_scoring_feedback_before_pair_complete")
        is False,
        ...
        "source pre mismatch": run.get("source_state_pre_sha") == p["repository_ref"],
        "source post mismatch": run.get("source_state_post_sha") == p["repository_ref"],
    ...
    if arm == CONTROL:
        ...
        if not derived["ri"] or not derived["identity"]:
            reasons.append("Treatment events do not prove exact aid access")
...
def eval_case(case, pack, key, wave, p, treatment):
    task_id = case.get("task_id")
    req(
        task_id in pack["ids"] and case.get("corpus_class") == pack["classes"][task_id],
        "run case mismatch",
    )
    ...
    req(
        ...
        and len(runs) == 2
        and [run.get("arm") for run in runs] == order,
    ...
    for arm in (CONTROL, TREATMENT):
        invalid[arm], derived[arm] = run_valid(
            by_arm[arm],
            arm,
            wave,
            task_id,
            pack["prompt_text"][task_id],
            pack["prompts"][task_id],
            order,
            p,
            treatment,
        )
    ...
    a_total, a_serious, _ = score_total(by_arm[CONTROL]["scores"], key)
    b_total, b_serious, b_values = score_total(by_arm[TREATMENT]["scores"], key)
    state = (
        "A-serious/B-no-serious"
        if a_serious and not b_serious
        else "A-no-serious/B-serious"
        if b_serious and not a_serious
        else "both-serious"
        if a_serious
        else "neither-serious"
    )
    quality = all(
        value >= p["positive_reversal_quality_gate"]["minimum_each_applicable_dimension"]
        for value in b_values.values()
    )
    metric = p["context_volume"]["metric"]
    volume_a = (
        derived[CONTROL]["repo_bytes"]
        if metric == "repository_response_utf8_bytes"
        else by_arm[CONTROL].get("measured_input_tokens")
        if metric == "input_tokens"
        else None
    )
    volume_b = (
        derived[TREATMENT]["repo_bytes"]
        if metric == "repository_response_utf8_bytes"
        else by_arm[TREATMENT].get("measured_input_tokens")
        if metric == "input_tokens"
        else None
    )
    return {
        ...
        "context_volume_ratio_b_over_a": (
            None if volume_a is None or volume_b is None else rat(volume_b, volume_a)
        ),
...
def eval_wave(name, cases, pack, keys, p, treatment):
    ...
    evaluated = [
        eval_case(by_id[task_id], pack, keys[task_id], name, p, treatment)
    ...
    ecological_positive = sum(
        case.get("qualifying_positive_reversal") is True for case in ecological
    )
    reverse = sum(
        case.get("serious_error_outcome") == "A-no-serious/B-serious" for case in valid
    )
    ecological_reverse = sum(
        case.get("serious_error_outcome") == "A-no-serious/B-serious"
        for case in ecological
    )
    deltas = [case["correctness_delta_b_minus_a"] for case in ecological]
    nonreg = (
        all_valid
        and med(deltas) >= p["ecological_correctness_non_regression"]["minimum_per_wave"]
        and ecological_reverse == 0
    )
    connector_ratios = [case["connector_ratio_b_over_a"] for case in ecological]
    connector_median = med(connector_ratios)
    connector_gate = (
        all_valid
        and connector_median <= p["connector_cost_gate"]["acceptable_median_ratio_b_over_a"]
        and sum(
            value > p["connector_cost_gate"]["high_overhead_ratio_threshold"]
            for value in connector_ratios
        )
        <= p["connector_cost_gate"]["acceptable_high_overhead_case_count"]
    )
    connector_gain = (
        all_valid
        and nonreg
        and reverse == 0
        and connector_median
        <= p["connector_interaction_gain_rule"]["maximum_median_ratio_b_over_a_per_wave"]
    )
    volume_ratios = [case["context_volume_ratio_b_over_a"] for case in ecological]
    complete_volume = (
        p["context_volume"]["metric"] != "unavailable"
        and all(value is not None for value in volume_ratios)
    )
    volume_gate = (
        complete_volume
        and med(volume_ratios) <= p["context_volume"]["maximum_median_ratio_b_over_a_per_wave"]
        and sum(
            value > p["context_volume"]["high_overhead_ratio_threshold"]
            for value in volume_ratios
        )
        <= p["context_volume"]["acceptable_high_overhead_case_count"]
    )
    return {
        "name": name,
        "cases": evaluated,
...
def final(pilot, confirmatory, p):
    ...
    thresholds = (
        confirmatory["qualifying_positive_reversals"]
        >= rule["minimum_confirmatory_positive_reversals"]
        and confirmatory["qualifying_ecological_positive_reversals"]
        >= rule["minimum_confirmatory_ecological_positive_reversals"]
        and positive >= rule["minimum_combined_positive_reversals"]
        and reverse <= rule["maximum_combined_reverse_serious_reversals"]
...
def validate_run_record(record, p):
    req(
        ...
        and record.get("study_id") == p["study_id"],
        "run record protocol/study mismatch",
    )
    preflight = record.get("execution_preflight")
    req(
        isinstance(preflight, dict) and preflight.get("eligible_to_start_primary") is True,
        "run record preflight/eligibility missing",
    )
    for field in PREFLIGHT:
        req(preflight.get(field) is True, f"run record preflight {field} must be true")
    prereg_preflight = p["execution_preflight"]
    req(
        preflight.get("smoke_evidence_reference")
        == prereg_preflight["smoke_evidence_reference"],
        "run record smoke evidence reference mismatch",
    )
    req(
        preflight.get("smoke_evidence_sha256") == prereg_preflight["smoke_evidence_sha256"],
        "run record smoke evidence SHA-256 mismatch",
    )
...
def evaluate(p, record, evidence):
    ...
    pilot = eval_wave(
        "PILOT",
        record.get("pilot_cases"),
        evidence["packs"]["pilot"],
        evidence["keys"]["pilot"],
        p,
        treatment,
    )
    confirmatory = eval_wave(
        "CONFIRMATORY",
        record.get("confirmatory_cases"),
        evidence["packs"]["confirmatory"],
        evidence["keys"]["confirmatory"],
        p,
        treatment,
    )
    return {
        ...
        "evidence_verification": {
            ...
            "treatment_delivery_completeness_derived_from_byte_ranges": (
                p["treatment_delivery"]["mode"] == "connector_compact_surface"
            ),
...
def main(argv=None):
    ...
    try:
        ...
        output = evaluate(prereg, record, evidence)

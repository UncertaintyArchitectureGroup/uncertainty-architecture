...
Every generated message explicitly asks the fresh session to read root `AGENTS.md` in full through the GitHub connector at the study SHA, then apply its benchmark-arm clause, the referenced protocol and applicable scoped instructions. This bootstrap is identical in both arms and leaves the original task prompt unchanged. Record those reads and their costs like any other interaction; if the session skips required instructions, record the deviation rather than silently repairing or rerunning it.

Before freezing `runs.json`, remove only untouched `init` placeholders for sessions that were never started. Keep every started session, including failures and interruptions, with its actual session ID, available evidence and deviations. Leave the frozen task list unchanged: omitted runs remain visible as missing in the report and produce INCONCLUSIVE; retained records with an empty session ID are rejected by `prepare`.

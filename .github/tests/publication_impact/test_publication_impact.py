...
def load_validator():
    spec = importlib.util.spec_from_file_location(
        "detect_publication_impact", VALIDATOR_PATH
    )
...
class PublicationImpactTests(unittest.TestCase):
    ...
    def test_renderer_core_configuration_and_publication_sources_are_impacting(self) -> None:
        impacting = [
        ...
        self.assertFalse(
            self.validator.is_publication_impact_path("CONTRIBUTING.md")
        )
    ...
    def test_invalid_revision_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "git merge-base"):
                self.validator.publication_render_required(
                    root, "missing-revision", "HEAD"
                )
    ...
    def test_changed_paths_preserve_unicode_newlines_and_both_rename_sides(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(
                self.validator.publication_render_required(root, base, head)
            )

...
Прямий запит «Ок зведи це на слайд 4 з необхідними поясненнями» від 23 вересня 2026 авторизує v17 лише на 4: GitClear 2021/2026 snapshots, фактичний churn 2021–2024, прогноз 2025 та індекс 2023–2026. Інші слайди, усі назви/порядок і попередні notes збережено. Оновлення freeze обмежене переглянутими source/slide/notes 4.

Прямий запит maintainer щодо слайда 9 від 23 вересня 2026 авторизує v18 лише на 9: повернути порівняння визначеного сценарію та простору варіантів, scope входів/виходів, semantic boundary, роль стандартів і складність coverage. Усі інші слайди, назви/порядок та попередні notes збережено; freeze оновлюється лише для перевірених source/slide/notes 9.
...
```pptx-slide
{
  ...
  "gitclearCaveat": "*Different editions / samples; no causal estimate of AI.",
  "otherCards": [
    [
      "METR · MAY 2026",
      "1.4–2× work value",
  ...
  "gitclearSource": "GITCLEAR · 2021 → 2026 SNAPSHOTS*",
  "gitclearSecondary": "2023→26: calls 343 → 223 / 1k changed lines; duplicates ≈+81%",
  "comparisonCaveat": "Different studies and outcomes. These signals do not form one causal estimate.",
  "gitclearChurnTitle": "Two-week churn · lines revised / reverted within 14 days",
  "gitclearChurn": [
    "2021 → 2024 actual: 3.27% → 5.67% (+73%)",
    "2025 forecast: 6.87% (+110% vs 2021); not observed",
    "2026: +15% vs 2023 index; absolute rate unavailable"
  ]
}
```
...
Дві колонки з чіткою вертикальною межею. Зліва — NBER та marketplace outcomes. Справа — одночасний позитивний зв’язок AI із throughput і негативний зі stability у DORA; приблизний standardized coefficient є підписом. Нижче — GitClear snapshots 2021/2026, окремий churn-блок із трьома різними статусами даних та додаткові calls/duplication. Унизу спільний ряд METR та Xu / Agarwal. Жодних стрілок між несумірними дослідженнями або outcomes; усі числа мають власні підписи. Докладні таблиці залишаються у нотатках.
...
**GitClear — зрізи 2021 → 2026:** moved-code share **24.65% → 3.8% (≈−85%)**, copy/paste **8.66% → 15.7% (≈+81%)**. Baseline — Appendix A1 повного звіту 2025; endpoint — public summary 2026 (moved YTD, copy/paste H1). Різні випуски/вибірки не утворюють єдину панель і не встановлюють причинний ефект AI.

**Churn на екрані:** 2021→2024 **3.27% → 5.67% (+73%) фактично**; **6.87% (+110% проти 2021) — прогноз 2025**; **+15% — індекс 2023→2026**, абсолютного endpoint 2026 немає. Визначення: рядки, суттєво переглянуті або reverted за два тижні. Calls **343→223 / 1k changed lines** та duplicated blocks **≈+81%** зберігають період 2023→2026. Детальні розрахунки, обмеження й відмінності знаменників — у notes та [джерельній нотатці](evidence/gitclear.md).
...
**GitClear: повний 2025 PDF + public summary 2026.** Екран показує 2021/2026 snapshots для moved та copy/paste, а churn розділяє на спостереження, прогноз та індекс із іншою базою. Для moved 2023 перекриття випусків уже відрізняється: 15.88% у A1 2025 проти 13% у summary 2026. Не конструювати з них точний ряд 2021→2026. Function calls і duplication залишаються окремими signals за 2023→2026. Проксі не є автоматичним вердиктом якості або причинності. [Визначення, таблиця та періоди](evidence/gitclear.md).
...
```pptx-slide
{
  ...
  "envelopeHeading": "MORE COMBINATIONS MAKE SPECIFICATION AND COVERAGE HARDER",
  "regionCaption": "Semantic limits are harder to make unambiguous",
  "regionBoundaryCases": "Boundary cases need contextual judgment",
  "regionExamples": [
    "Answer",
    "Clarify",
    "Escalate"
  ],
  "regionInner": "Acceptable outputs / actions",
  "regionOutside": "Unauthorized\ndisclosure",
  "regionAxis": "Schematic space, not a measured probability or distance",
  "oldCoverage": "Expected states and branches\nanchor the acceptance checks.",
  "oldFoot": "× Potential violations of the specified path",
  "inputScope": "INPUT: intents, context, data and permissions",
  "complexity": "Inputs × context × outputs create more ways to miss a requirement.",
  "productionRisk": "Known scenarios can pass while unseen combinations fail in production."
}
```

### Поточна композиція та зміст, v18

Слайд повертає загальне порівняння зі старої презентації, PDF page 5 **Designing the Space of Possibilities**. Зліва — визначений сценарій: за того самого стану натискання A має відкрити B; відхилення й перевірки прив’язані до заданих станів, переходів та гілок. Справа — простір припустимих входів, контекстів, відповідей і дій, дозволені варіанти та заборонений наслідок. Пунктирна semantic boundary показує складність вичерпного опису й перевірки. Це схема, не карта ймовірностей або universal semantic distance.

Поєднання inputs × context × outputs створюють більше потенційних прогалин у вимогах та coverage. Непередбачені комбінації можуть дійти до production. Не виводимо частоту дефектів із геометрії і не стверджуємо автоматичне зростання error rate. Детерміновані системи також можуть мати складні простори станів; permissions, schemas і invariants залишаються точними обов’язками.

**Стандарти і практичний пробіл:** ISO/IEC 25059 — quality model та vocabulary; ISO/IEC TS 25058 — evaluation guidance. Команда має перетворити їх на product-specific semantic criteria, приклади, контрприклади, заборони та перевірки. «Стандартів немає» і «семантичну межу взагалі неможливо задати» не є тезами цього слайда. Не існує готової межі конкретного продукту лише завдяки посиланню на загальну модель якості.

**Ролі:** Product / PO та BA формують цінність і критерії; QA / Dev та domain experts допомагають операціоналізувати й перевірити; Project Manager забезпечує рішення, експертів, час і dependencies. Ризик погоджує уповноважений owner. Попередні IT-assistant value/safety/cost/fallback приклади збережено у notes. Основна видима теза — семантичну прийнятність важче описати й перевірити, ніж конкретний fixed transition; наступний слайд пояснює дефект та evaluation evidence.

...
- Roboto Regular / Bold за наступним прямим запитом maintainer. Повні накреслення вбудовані у PPTX; ліцензія SIL OFL 1.1, вихідний шрифт і точні статичні версії збережені в artwork/roboto-fonts.zip. Перед authoring встановити обидва TTF із цього архіву. Символ → використовує стандартний font fallback, оскільки Roboto не містить цього гліфа. Текст залишається редагованим, без перетворення на криві.
...
```pptx-slide
{
  ...
  "newHeading": "ACCEPTABLE BEHAVIOR REGION",
  "boundaries": [
    [
      "ALLOW",
  ...
  "envelopeHeading": "BUSINESS TOLERANCE CONTRACT",
  "envelope": [
    "Allowed variation: wording",
    "Required meaning: approved steps",
    "Limits: rework, cost and latency",
    "Escalation: owner + fallback"
  ],
  "specification": "A prompt influences behavior. Examples and acceptance rules define the contract.",
  "regionCaption": "Many valid outputs inside the same agreed limits",
  "regionExamples": [
    "Short explanation",
...
**Композиція:** Дві розділені колонки. Ліва Button A → Window B збережена. Праворуч native область допустимих відповідей із двома варіантами всередині, умовою CLARIFY та PROHIBIT поза нею. Це концептуальна межа умов, не числова карта semantic distance. Нижче — business tolerance contract, мовний розрив і starting points.
...
```pptx-slide
{
  ...
  "chartAxis": "Count in the sample (n = 200)"
}
```

**Час:** 3:00.

**Композиція:** Ліворуч native bar chart частот 196 / 3 / 1 з точною шкалою 0–200 і частками 98% / 1,5% / 0,5%. Праворуч об’єднана частота 4/200 = 2%, Wilson 95% interval ≈0,8%–5,0% і тяжкість наслідків. Нижче Golden Set / Eval Gate, калібрування та відповідальні особи. Це навчальна вибірка.
...
```pptx-slide
{
  ...
  "table": [
    ...
    [
      ...
      "Tolerances + evaluation plan\nComplete control design, including people\nOwners, authority and fallback\nCapacity, quality, cost, latency budgets",
      "Product / BA + Architect\nPM resolves dependencies"
    ],
    [
      "DONE\nDoD",
      "Code review + tests\nAcceptance verified",
      "Versioned model / prompt / data\nCalibrated evals + coverage limits\nWorking controls + human rehearsal\nRecovery, bypass and failure evidence",
      "Developer + QA verify\nDone does not authorize release"
    ],
    [
      "RELEASE\nOPERATE",
      "Release approval\nDeployment + operations",
      "Scoped approval + live monitoring\nReady responders + stop triggers\nProduction incidents revise regression,\nEval Gates and monitoring rules",
  ...
  "behavior": "Behavior changes with prompts, models or retrieval, even when application code stays the same.",
  "syntax": "Valid JSON can still contain an incorrect instruction."
}
```
...
```pptx-slide
{
  ...
  "architectureHeading": "SOURCES OF UNCERTAINTY",
  "architecture": [
    "Token prices + control cost",
    "Opaque provider changes",
    "Latency + fallible evaluators"
  ],
  "veto": "If control or viable cost cannot be demonstrated, narrow the scope or stop this AI path.",
  ...
  "trialRule": "Unknown load: a bounded, authorized trial needs working controls and stop limits.",

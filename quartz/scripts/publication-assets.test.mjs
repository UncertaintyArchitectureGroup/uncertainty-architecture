...
test("standalone publication exposes all eight canonical Mermaid figures", async () => {
  ...
  assert.deepEqual(
    figures.map((figure) => figure.number),
    [1, 2, 3, 4, 5, 6, 7, 8],
  );
...
test("platform PNG rasterization uses Chromium for Mermaid foreignObject labels", () => {
  const plan = svgRasterizationPlan(
    '<svg viewBox="0 0 400 200"><foreignObject width="200" height="80"><div>Visible label</div></foreignObject></svg>',
    1600,
  );
  assert.deepEqual(plan, {
    ...
    width: 1600,
    height: 800,
    contains_foreign_object: true,
    background: "#ffffff",
  });
});

test("publication hero omits source-updated dates until publication_date is explicit", () => {
  const svg = buildHeroSvg(
    { key: "test", filename: "test.png", width: 1200, height: 627 },
    {
      title:
  ...
  assert.match(svg, /Vitalii Oborskyi · CC BY 4\.0/);
  assert.doesNotMatch(svg, /GMT|Coordinated Universal Time|Fri Aug/);
});

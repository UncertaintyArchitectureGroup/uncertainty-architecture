...
export async function finalizePublicationPair(
  ...
  const backupDirectory = await mkdtemp(
    path.join(path.dirname(outputPdf), ".ua-publication-backup-"),
  )
  ...
  try {
    ...
    if (hadManifest && (await exists(manifestBackup))) {
      await rename(manifestBackup, outputManifest)
    }
    if (hadPdf && (await exists(pdfBackup))) {
      await rename(pdfBackup, outputPdf)
    }
    throw error
  } finally {
...
async function main() {
  ...
  const buildDirectory = await mkdtemp(path.join(outputRoot, ".ua-publication-build-"))

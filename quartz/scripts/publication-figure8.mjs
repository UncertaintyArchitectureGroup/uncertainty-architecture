...
function textBlock(x, y, lines, { size = 14, weight = 500, fill = "#314852", anchor = "middle", line = 18 } = {}) {
  const tspans = lines
    .map((value, index) => `<tspan x="${x}" dy="${index === 0 ? 0 : line}">${escapeXml(value)}</tspan>`)
    .join("")
...
export function assertFigure8SemanticSource(mermaid) {
  const missing = figure8SemanticMarkers.filter((marker) => !mermaid.includes(marker))
  if (missing.length > 0) {
    throw new Error(`Canonical Figure 8 changed; publication rendition requires review. Missing semantic markers: ${missing.join(", ")}`)
  }
}

export function buildFigure8DecisionSvg() {
  const lifelines = [170, 500, 850, 1180]
    .map((x) => `<line x1="${x}" y1="145" x2="${x}" y2="860" stroke="#c7d1d6" stroke-width="1.5" stroke-dasharray="6 8"/>`)
    .join("")

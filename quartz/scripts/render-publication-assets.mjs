...
export const publicationAssetsRoot = path.join(repoRoot, "dist", "publication", "thinking-systems")
export const publicationSvgRoot = path.join(publicationAssetsRoot, "figures", "svg")
export const publicationPngRoot = path.join(publicationAssetsRoot, "figures", "png")
...
async function pinnedMermaidCliVersion() {
  const contractPath = path.join(repoRoot, ".github", "policy", "supply-chain-contract.json")
...
function parseFigureCaption(tail) {
  const match = /^\s*(?:<!--[\s\S]*?-->\s*)*\*\*Figure\s+(\d+)([AB])?\s+—\s+([^*\n]+?)\.\*\*/.exec(
    tail,
  )
...
export function classifyFigureWidth(mermaid) {
  ...
  const density = lines.length + edges + Math.ceil(nodes / 2) + Math.ceil(text.length / 180)
...
function minimumSvgFontPx(svg) {
  const values = [...String(svg).matchAll(/font-size(?:=|:)\s*["']?([0-9.]+)(?:px)?/gi)]
...
function viewBoxDimensions(svg) {
  const match = /viewBox=["']\s*[0-9.-]+\s+[0-9.-]+\s+([0-9.]+)\s+([0-9.]+)\s*["']/i.exec(
    String(svg),
  )
  if (!match) return null
...
export function svgRasterizationPlan(svg, pngWidth) {
  ...
  if (!dimensions) throw new Error("Publication SVG is missing a valid viewBox")
  return {
    renderer: "chromium",
    width: pngWidth,
    height: Math.max(1, Math.ceil((pngWidth * dimensions.height) / dimensions.width)),
...
async function rasterizeSvgWithChromium(svgBuffer, pngWidth) {
  ...
  try {
    ...
    if (plan.contains_foreign_object) {
      const labelsVisible = await page.locator("#ua-frame foreignObject").evaluateAll((nodes) =>
        nodes.some((node) => {
      ...
      if (!labelsVisible) {
        throw new Error("Chromium did not expose Mermaid foreignObject labels for rasterization")
      }
    }
    ...
    if (metadata.hasAlpha) {
      throw new Error("Platform PNG must be opaque after white-background flattening")
    }
...
async function renderMermaidSvg(mermaid, version) {
  const temporary = await mkdtemp(path.join(os.tmpdir(), "ua-publication-assets-"))
  try {
    ...
    await writeFile(
      puppeteerPath,
      // Reuse the browser installed by pdf:setup/CI. Mermaid's transitive
      // Puppeteer package may expect a separate Chrome cache that is absent.
      JSON.stringify({
    ...
    await new Promise((resolve, reject) => {
      ...
      child.once("exit", (code) => {
        if (code === 0) resolve()
        else reject(new Error(`mermaid-cli exited with code ${code}: ${detail.trim()}`))
      })
    })
...
async function emitFigure({ figure, svgBuffer, pngWidth, sourcePath, semanticFingerprint = null }) {
  ...
  const normalizedSvg = Buffer.from(`${svgBuffer.toString("utf8").trim()}\n`, "utf8")
  ...
  return {
    ...
    desktop_no_zoom_floor_met:
      figure.panel && desktopLabel !== null ? desktopLabel >= desktopMinimumLabelPx : null,
...
export function buildHeroSvg(specification, publication) {
  ...
  const subtitleLines = wrapText(secondary, specification.width >= 1800 ? 42 : 31)
  const padX = Math.round(specification.width * 0.075)
  const titleSize = specification.width >= 1800 ? 66 : specification.width >= 1500 ? 56 : 47
...
async function renderHeroAssets(source) {
  ...
  const publication = {
    ...
    license: String(source.data.license || "CC BY 4.0").replace(/^CC-BY-4\.0$/i, "CC BY 4.0"),
  }
  const assets = []
...
async function renderFigures(source) {
  ...
  const canonicalFingerprint = assertCanonicalFigure8Fingerprint(located.mermaid, located.caption)
  const figure8Panels = [
    {
      number: 8,
      panel: "A",
    ...
    },
    {
      ...
      basename: "figure-8b-capability-family-axis-and-orthogonality-relationship",
  ...
  for (const panel of figure8Panels) {
    ...
    if (asset.projected_desktop_minimum_label_px === null) {
      throw new Error(`Figure 8${panel.panel} SVG has no measurable label size`)
    }
...
export async function buildPublicationAssetsManifest(source, { now = new Date() } = {}) {
...
async function main() {
  ...
  if (!before.equals(after)) {
    throw new Error("Canonical Markdown changed while rendering publication assets")
  }
  for (const asset of [...manifest.figures, ...manifest.heroes]) {
    ...
    if (!info.isFile() || info.size < 1024) {
      throw new Error(`Generated publication asset is missing or too small: ${relative(target)}`)
    }
  }
  console.log(

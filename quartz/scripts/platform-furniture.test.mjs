...
import { buildFurnitureMarkdown, buildFurnitureHtml, insertMarkdownFurniture, insertHtmlFurniture } from "./render-platform-furniture.mjs";
...
test("publication furniture carries author identity and six-stage UA research path", async () => {
  ...
  assert.match(p.author_furniture.bio, /creator and principal author of Uncertainty Architecture/);
  assert.equal(p.research_path.items.length, 6);
  assert.deepEqual(p.research_path.items.map((item) => item.stage), [
    "Practical precursor",
    "Early UA framework",
...
test("LinkedIn furniture uses LinkedIn navigation and Medium furniture prefers Medium navigation", async () => {
  ...
  assert.match(medium, /medium\.com\/data-science-collective\/architecting-uncertainty/);
});

test("furniture inserts once immediately before Resources in Markdown and HTML", async () => {
  const p = await profile();
  const md = insertMarkdownFurniture("# Article\n\nBody\n\n## Resources\n\n- Repo\n", buildFurnitureMarkdown(p, "linkedin"));
  assert.ok(md.indexOf("## About the author") < md.indexOf("## Resources"));
  assert.throws(() => insertMarkdownFurniture(md, "again"), /already contains author furniture/);
  const html = insertHtmlFurniture("<main><p>Body</p><h2>Resources</h2></main>", buildFurnitureHtml(p, "medium"));
  assert.ok(html.indexOf("publication-furniture") < html.indexOf("<h2>Resources</h2>"));
  assert.throws(() => insertHtmlFurniture(html, "again"), /already contains publication furniture/);
});

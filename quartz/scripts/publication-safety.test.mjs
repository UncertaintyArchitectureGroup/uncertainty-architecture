...
import {
  ...
  resolveSourceCommit,
} from "./publication-provenance.mjs"
...
test("declared source refs resolve to immutable full commit SHAs", async () => {
  const resolved = await resolveSourceCommit("HEAD")
...
test("strict provenance rejects modified bytes for a tracked source", async () => {
  ...
  assert.match(preview.sourceCommit, /^[0-9a-f]{40}$/)

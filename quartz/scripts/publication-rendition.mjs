...
export const currentArticleSource = "content/research/notes/thinking-systems-publication-draft.md"
...
function buildTitlePage({
  ...
  const version =
    data.edition || data.version || (data.draft === true ? "Draft" : "Unversioned")
  ...
  const canonical = data.canonical_url
    ? `<br/><a href="${escapeHtml(data.canonical_url)}">Canonical publication</a>`
    : ""
  const authorLine =
    authors.length > 0 ? authors.join(" · ") : "Author not declared in source metadata"
  ...
  const sourceRows = dirty
    ...
    <div><dt>Committed baseline</dt><dd><code>${escapeHtml(sourceCommit.slice(0, 12))}</code></dd></div>
    <div><dt>Working-tree blob</dt><dd><code>${escapeHtml(provenance.workingBlob.slice(0, 12))}</code></dd></div>`
    : `<div><dt>Source commit</dt><dd><code>${escapeHtml(sourceCommit.slice(0, 12))}</code></dd></div>`
...
function locateCanonicalFigure8(content) {
...
export function splitFigure8(content, { verifyFingerprint = true } = {}) {
  ...
  if (!located) return { content, split: false }
  ...
  const canonicalCaption = caption
    ...
    .replace(/\*\*/, "")
    .trim()
  const panelA = `<section class="ua-pdf-static-figure ua-pdf-static-figure--8a">
${buildFigure8DecisionSvg()}
<p><strong>Figure 8A — Decision-ownership model.</strong> Publication rendition of canonical Figure 8; continue with Figure 8B.</p>
</section>`
  const panelB = `<section class="ua-pdf-static-figure ua-pdf-static-figure--8b">
${buildFigure8CapabilitySvg()}
...
export function assertCurrentArticleFigure8Rendition(rendition) {
  ...
  const canonicalEight = rendition.canonicalFigures.filter((figure) => figure.number === 8)
  const renderedEight = rendition.renditionFigures.filter((figure) => figure.number === 8)
...
export async function loadPublicationSource(sourcePath) {
  ...
  if (!isInside(contentRoot, absolute)) {
    throw new Error("Publication source must be a Markdown file under content/")
  }

...
- All slide titles, substantive notes and load-bearing evidence numbers are present. The current slide 4 acceptance fixture uses NBER’s September 2026 revision (25.5× / 3.4× / 1.3×), published marketplace shares on the slide, plus supporting DORA perceptions and Agarwal’s unrounded-to-integers Table 2 percentages in notes; mutation checks reject obsolete NBER and rounded Agarwal replacements. DORA chart readings retain approximation signs.
...
The subsequent maintainer instruction to apply the substantive review authorizes v15 changes to slides 4 and 8–14. Slide 4 prioritizes three evidence contrasts, preserving auxiliary metrics and arithmetic in notes. Slides 8–12 follow an illustrative access-recovery assistant; slide 12 separates a model proposal, permission/required-approval checks before delivery/execution, and post-action feedback. It also exposes proportional control and the architectural veto. Slide 13 shows coordination outcomes rather than a participant list. Titles, prior note prefixes, slides 1–3/5–7/15, shared resources and the illustrative chart/workbook are preserved. The authorized freeze refresh covers only source sections and slide/notes XML for 4/8–11 after inspection. Supporting-number mutation fixtures follow the moved evidence into notes, and renderer word-fit fixtures follow the new Request / Verify user labels. This content/layout revision uses the existing independent render, source-preservation comparison and portable package verification as its regression evidence; subjective narrative quality is not encoded as a new CI rule.

The next direct maintainer correction authorizes v16 only on 4/10/11/13. Slide 4 restores the v14 two-column layout and all evidence blocks, with an explicit missing-baseline caveat for reported +15% churn and notes distinguishing the older projected doubling. Slide 10 uses single-line count/share native text boxes with explicit transparent fill/outline and sufficient width, while custom and automatic chart labels are disabled; portable mutation checks reject joined labels, missing transparency and duplicate values. Slide 11 restores the old talk's statistical readiness/completion/release comparison with cost inside the conditions, operational control/HITL and production feedback. Slide 13 makes the combined HOW/WHAT changes and operating-model evolution primary, with the expertise gap as one PM dependency example. Unaffected slides and all earlier notes remain exact. The freeze refresh is limited to reviewed 4/10/11 source and slide/notes parts plus chart label XML; workbook values are preserved.

The subsequent v17 request authorizes **slide 4 only** to show GitClear 2021/2026 cross-edition snapshots and separate observed 2021–2024 churn, the 2025 projection, and the 2023–2026 index. Full 2025 Appendix A1 and the 2026 author's index chart correct the earlier unspecified-baseline wording: +15% has a 2023 baseline but no published absolute 2026 churn endpoint. The source, notes and evidence owner record sample/denominator limits. The right column retains DORA with closer spacing; all evidence blocks remain visible. Only reviewed source/slide/notes 4 may change in the freeze; all other slides, shared dependencies, prior note prefixes and titles/order remain exact.

The v18 direct correction authorizes **slide 9 only** to restore the fixed-path versus possibility-space requirements comparison. Native diagrams show A→B and scoped inputs / acceptable output variants / prohibited outcomes, with dashed semantic boundaries. Visible copy separates coverage complexity from an unproven universal production failure-rate increase and describes the roles of ISO/IEC 25059 and ISO/IEC TS 25058. Prior note prefixes remain exact. The existing rendered-label regression now checks Press A / Window B. Only reviewed source/slide/notes 9 may refresh the freeze; all other slides and shared dependencies remain exact.

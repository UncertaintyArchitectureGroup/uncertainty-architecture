...
The central Markdown now owns the maintainer-requested **slides 1–8 freeze**, established after the GitClear explanation on slide 4. Read that scope before editing. The portable validator enforces its observable subset through `frozen-slides.json`: exact source-section hashes and normalized package-part hashes for the protected slides, notes and dependency closure, including shared presentation settings, layouts, masters, themes and cover bytes. Generated relationship/creation IDs are normalized by meaning; content, geometry, relationship targets and asset bytes are not ignored. The explicit September 22 request authorizes the new distribution explanation on slide 8. That earlier refresh changed only slide 8. The next explicit September 22 request authorizes a font-only change to Arial across all 14 slides: protected source sections, text, notes and geometry remain exact; font references and the removal of embedded Roboto are reviewed before refreshing the package baseline. Slides 9–14 remain editable. The freeze record is itself a manifest input; generation cannot silently replace it. There is no auto-refresh or bypass flag. This protects against accidental changes, not an independent human-authorization boundary.

...
The PMDay PowerPoint path reads its central Markdown under `assets/presentations/`, creates native editable slide objects in the configured authoring runtime, and commits a checked PPTX/manifest pair at the maintainer's request. `npm run pptx:verify` and the ordinary publication regression glob check that snapshot without the authoring backend. PDF/site generation is unchanged.

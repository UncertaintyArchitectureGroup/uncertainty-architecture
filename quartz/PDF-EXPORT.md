...
`pdf:setup` installs the Playwright Chromium binary used by the renderer. On Ubuntu CI or a fresh Linux machine that also needs browser system packages, use `npx playwright install --with-deps chromium`. Visual verification additionally requires Poppler (`poppler-utils` on Ubuntu).
...
The final PDF and its provenance manifest form one rollback-capable publication pair. Both candidates are completed before installation. If either final rename fails, the previous PDF and previous matching manifest are restored together rather than leaving a new PDF beside stale provenance.
...
Regular figure PNGs target 1800 px width. Dense Figure 8A targets 3200 px; Figure 8B targets 2400 px. SVG remains available for website/PDF-quality reuse. PNG is the common platform rendition for Medium and LinkedIn.

The public asset command builds and validates the complete bundle in a staging directory, then replaces the previous bundle as one rollback-capable directory transaction. Figure 8A and Figure 8B ship with `figure-08-shared-caption.md`; the two panels are one logical figure and must be published together with that complete canonical caption. Requested PNG/cover dimensions are verified so stale, partial, or undersized assets do not silently enter a bundle.
...
Build Integrity runs unit/regression tests and the ordinary draft-filtered Quartz build on every pull request. The Chromium/Poppler end-to-end publication render runs on pushes to `main`, manual workflow runs, and pull requests whose diff touches the publication sources or publishing/Quartz rendering surfaces. The end-to-end job renders and verifies the standalone article PDF, its platform asset bundle, and the living working-paper PDF.
